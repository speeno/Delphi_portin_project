object Sobo48: TSobo48
  Left = 71
  Top = 120
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = '출판사관리(설정)'
  ClientHeight = 533
  ClientWidth = 901
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = '굴림'
  Font.Style = [fsBold]
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 12
  object CornerButton1: TCornerButton
    Left = 4
    Top = 3
    Width = 21
    Height = 47
    Cursor = crArrow
    BevelColor = clRed
    BevelWidth = 1
    MouseDownColor = clRed
    Transparent = False
    CornerOptions = [cnTopLeft, cnBottomLeft]
    CornerSize = 10
  end
  object CornerButton2: TCornerButton
    Left = 4
    Top = 52
    Width = 21
    Height = 447
    Cursor = crArrow
    BevelColor = clBlue
    BevelWidth = 1
    MouseDownColor = clBlue
    Transparent = False
    CornerOptions = [cnTopLeft, cnBottomLeft]
    CornerSize = 10
  end
  object CornerButton9: TCornerButton
    Left = 4
    Top = 501
    Width = 21
    Height = 30
    Cursor = crArrow
    BevelColor = clBlack
    BevelWidth = 1
    MouseDownColor = clBlack
    Transparent = False
    CornerOptions = [cnTopLeft, cnBottomLeft]
    CornerSize = 10
  end
  object Label301: TmyLabel3d
    Left = 9
    Top = 12
    Width = 13
    Height = 26
    Caption = '조'#13#10'회'
    Font.Charset = HANGEUL_CHARSET
    Font.Color = clRed
    Font.Height = -13
    Font.Name = '굴림'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    AShadeLTSet = False
  end
  object Label302: TmyLabel3d
    Left = 9
    Top = 224
    Width = 13
    Height = 52
    Caption = '검'#13#10'색'#13#10'자'#13#10'료'
    Font.Charset = HANGEUL_CHARSET
    Font.Color = clBlue
    Font.Height = -13
    Font.Name = '굴림'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    AShadeLTSet = False
  end
  object Label309: TmyLabel3d
    Left = 10
    Top = 505
    Width = 12
    Height = 24
    Caption = '상'#13#10'태'
    Font.Charset = HANGEUL_CHARSET
    Font.Color = clBlack
    Font.Height = -12
    Font.Name = '굴림'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    AShadeLTSet = False
  end
  object Panel001: TFlatPanel
    Left = 24
    Top = 4
    Width = 873
    Height = 45
    Color = 14416873
    TabOrder = 0
    UseDockManager = True
    object dxButton1: TdxButton
      Left = 392
      Top = 8
      Width = 63
      Height = 30
      OnClick = Button709Click
      Caption = '검색'
      TabOrder = 0
      Glyph.Data = {
        
        6767676767676767674C67663B66673E67665466666566666666}
    end
  end
  object Panel004: TFlatPanel
    Left = 784
    Top = 53
    Width = 113
    Height = 81
    Color = clInfoBk
    Visible = False
    TabOrder = 2
    UseDockManager = True
    object RadioButton6: TFlatRadioButton
      Left = 16
      Top = 16
      Width = 73
      Height = 17
      Caption = '전체선택'
      TabOrder = 0
      OnClick = Button200Click
    end
    object RadioButton7: TFlatRadioButton
      Left = 16
      Top = 48
      Width = 73
      Height = 17
      Caption = '전체해체'
      TabOrder = 1
      OnClick = Button200Click
    end
  end
  object Button300: TFlatButton
    Left = 784
    Top = 450
    Width = 113
    Height = 49
    Color = clInfoBk
    Caption = '저장'
    Glyph.Data = {
      
      0003737FFFFFFFFF7F7330099999999900333777777777777733}
    Layout = blGlyphLeft
    NumGlyphs = 2
    ParentColor = False
    TabOrder = 3
    Visible = False
    OnClick = Button300Click
  end
  object Panel007: TFlatPanel
    Left = 24
    Top = 502
    Width = 873
    Height = 29
    Color = clInfoBk
    TabOrder = 4
    UseDockManager = True
    object ProgressBar0: TFlatProgressBar
      Left = 528
      Top = 5
      Width = 336
      Height = 19
      Min = 0
      Max = 100
    end
    object Panel008: TFlatPanel
      Left = 8
      Top = 5
      Width = 97
      Height = 19
      Caption = '레코드'
      ParentColor = True
      TabOrder = 0
      UseDockManager = True
    end
    object Panel009: TFlatPanel
      Left = 112
      Top = 5
      Width = 97
      Height = 19
      ParentColor = True
      TabOrder = 1
      UseDockManager = True
    end
    object Panel010: TFlatPanel
      Left = 424
      Top = 5
      Width = 97
      Height = 19
      Caption = '검색진행'
      ParentColor = True
      TabOrder = 2
      UseDockManager = True
    end
    object ProgressBar1: TProgressBar
      Left = 528
      Top = 5
      Width = 335
      Height = 18
      Min = 0
      Max = 100
      TabOrder = 3
    end
  end
  object Panel002: TFlatPanel
    Left = 24
    Top = 53
    Width = 873
    Height = 446
    ParentColor = True
    TabOrder = 1
    UseDockManager = True
    object DBGrid101: TDBGridEh
      Left = 1
      Top = 1
      Width = 871
      Height = 444
      Align = alClient
      BorderStyle = bsNone
      DataSource = DataSource1
      FooterColor = 16311512
      FooterFont.Charset = ANSI_CHARSET
      FooterFont.Color = clWindowText
      FooterFont.Height = -12
      FooterFont.Name = '굴림'
      FooterFont.Style = [fsBold]
      ImeName = '한국어 입력 시스템 (IME 2000)'
      Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgAlwaysShowSelection, dgCancelOnExit, dgMultiSelect]
      TabOrder = 0
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -12
      TitleFont.Name = '굴림'
      TitleFont.Style = [fsBold]
      Columns = <
        item
          EditButtons = <>
          FieldName = 'GCODE'
          Footers = <>
          Title.Alignment = taCenter
          Title.Caption = '출판사코드'
          Width = 80
        end
        item
          EditButtons = <>
          FieldName = 'GNAME'
          Footers = <>
          Title.Alignment = taCenter
          Title.Caption = '출판사명'
          Width = 424
        end
        item
          Checkboxes = True
          EditButtons = <>
          FieldName = 'CHEK3'
          Footers = <>
          KeyList.Strings = (
            'O;On'
            'N;Off')
          Title.Alignment = taCenter
          Title.Caption = 'SET재고적용'
          Width = 100
          OnUpdateData = DBGrid101Columns2UpdateData
        end
        item
          Checkboxes = True
          EditButtons = <>
          FieldName = 'YESNO'
          Footers = <>
          KeyList.Strings = (
            'O;On'
            'N;Off')
          Title.Alignment = taCenter
          Title.Caption = '출고정지유무'
          Width = 100
          OnUpdateData = DBGrid101Columns3UpdateData
        end>
    end
  end
  object DataSource1: TDataSource
    DataSet = T1_Sub81
    OnDataChange = DataSource1DataChange
    Left = 34
    Top = 126
  end
  object DataSource2: TDataSource
    Left = 66
    Top = 126
  end
  object T1_Sub81: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 34
    Top = 158
    object T1_Sub81ID: TFloatField
      FieldName = 'ID'
    end
    object T1_Sub81GDATE: TStringField
      FieldName = 'GDATE'
      EditMask = '!0000.!90.90;1; '
      Size = 10
    end
    object T1_Sub81GCODE: TStringField
      FieldName = 'GCODE'
      Size = 10
    end
    object T1_Sub81GNAME: TStringField
      FieldName = 'GNAME'
      Size = 100
    end
    object T1_Sub81GTELS: TStringField
      FieldName = 'GTELS'
      Size = 30
    end
    object T1_Sub81CODE1: TStringField
      FieldName = 'CODE1'
      Size = 1
    end
    object T1_Sub81CODE2: TStringField
      FieldName = 'CODE2'
      Size = 1
    end
    object T1_Sub81CODE3: TStringField
      FieldName = 'CODE3'
      Size = 1
    end
    object T1_Sub81CODE4: TStringField
      FieldName = 'CODE4'
      Size = 1
    end
    object T1_Sub81CODE5: TStringField
      FieldName = 'YESNO'
      Size = 1
    end
    object T1_Sub81GOQUT: TFloatField
      FieldName = 'GOQUT'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T1_Sub81GSQUT: TFloatField
      FieldName = 'GSQUT'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T1_Sub81GSSUM: TFloatField
      FieldName = 'GSSUM'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T1_Sub81CHEK1: TStringField
      FieldName = 'CHEK1'
      Size = 5
    end
    object T1_Sub81CHEK2: TStringField
      FieldName = 'CHEK2'
      Size = 5
    end
    object T1_Sub81CHEK3: TStringField
      FieldName = 'CHEK3'
      Size = 5
    end
    object T1_Sub81CHEK4: TStringField
      FieldName = 'CHEK4'
      Size = 5
    end
    object T1_Sub81CHEK5: TStringField
      FieldName = 'CHEK5'
      Size = 5
    end
  end
end
