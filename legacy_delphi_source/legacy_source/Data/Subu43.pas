...
//if Base10.Seek_Ggeo(Edit107.Text)='X' Then Exit;
  ...
  //nSqry.FieldByName('Gname').Value:=Base10.Seek_Hname(nSqry.FieldByName('Gcode').AsString,nSqry.FieldByName('Hcode').AsString);
    ...
    S4301:=S4301+nSqry.FieldByName('Gssum').AsFloat;
    nSqry.Post;
    nSqry.Next;
  end;
...
procedure TSobo43.DBGrid101TitleClick(Column: TColumn);
begin
  Base10.ColumnSX(nSqry,Column);
end;

procedure TSobo43.DBGrid201TitleClick(Column: TColumn);
begin
...
begin
  StBar101.Panels.Items[2].Text:=FormatFloat('###,###,##0',S4301);
