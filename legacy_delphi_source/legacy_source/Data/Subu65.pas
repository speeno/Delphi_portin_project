...
uses
  ...
  ToolEdit, CornerButton, dxCore, dxButtons, DBGridEh;
...
//if Base10.Seek_Ggeo(Edit107.Text)='X' Then Exit;
  ...
  {-S1_Ssub-}
  Sqlen :=
  'Select Hcode,Scode,Gubun,Pubun,Gjisa,Sum(Gsqut)as Gsqut,Sum(Gssum)as Gssum '+
  'From S1_Ssub Where '+D_Select+St1+
  'Group By Hcode,Scode,Gubun,Pubun,Gjisa ';
  ...
  List1:=List1+1;
    ...
    if nSqry.Locate('Gcode',St3,[loCaseInsensitive])=False then begin
      ...
      nSqry.Append;
      nSqry.FieldByName('Gcode').AsString:=SGrid.Cells[ 0,List1];
...
{ Sqlen := St3+' Group By Gcode ';
  ...
  List1:=List1+1;
    ...
    if nSqry.Locate('Gcode;Gjisa',VarArrayOf([St3,'']),[loCaseInsensitive])=False then begin
      St3:=Base10.Seek_Code(St3,St2,Edit107.Text);
      nSqry.Append;
      nSqry.FieldByName('Gcode').AsString:=SGrid.Cells[ 0,List1];
...
//if Base10.Seek_Ggeo(Edit107.Text)='X' Then Exit;
  ...
  if(T00=1)Then
  Button101.Caption:='' else
  Button101.Caption:=nSqry.FieldByName('Gname').AsString;
  ...
  List1:=List1+1;
    ...
    St6:=SGrid.Cells[ 4,List1];

    if mSqry.Locate('Gcode;Gjisa',VarArrayOf([St3,St6]),[loCaseInsensitive])=False then begin
      if St2='X' then begin
...
begin
  ...
  if(Edit104.Focused=True)and(Panel102.Caption='출판사명')Then begin
       Edit103.Text:='';
  ...
  if(Edit106.Focused=True)and(Panel102.Caption='출판사명')Then begin
       Edit105.Text:='';
...
procedure TSobo65.DBGrid101TitleClick(Column: TColumnEh);
begin
  Base10.ColumnS9(nSqry,Column);
end;

procedure TSobo65.DBGrid201TitleClick(Column: TColumnEh);
begin
