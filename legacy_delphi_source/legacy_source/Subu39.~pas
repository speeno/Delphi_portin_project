...
//if Base10.Seek_Ggeo(Edit108.Text)='X' Then Exit;
  ...
  { if nSqry.FieldByName('Gjisa').AsString<>'' then begin
    ...
    if nSqry.FieldByName('Gjisa').AsString<>'' then begin
      ...
      if Base10.H2_Gbun.Locate('Hcode;Scode;Gcode;Gname;Jubun',VarArrayOf(['','X',nSqry.FieldByName('Gcode').AsString,
      ...
      if Copy(nSqry.FieldByName('Gcode').AsString,1,1)='9' then
      if Base10.H2_Gbun.Locate('Hcode;Scode;Gcode;Gname;Jubun',VarArrayOf([nSqry.FieldByName('Gcode').AsString,'X',nSqry.FieldByName('Gcode').AsString,
      Base10.Seek_Jisa(nSqry.FieldByName('Gjisa').AsString,'3'),Base10.Seek_Jisa(nSqry.FieldByName('Gjisa').AsString,'4')]),[loCaseInsensitive])=true then
        St8:=Base10.H2_Gbun.FieldByName('Gdate').AsString;
    end;
    if St8='지방물류' then St9:='2';
  ...
  { Sqlen := 'Select Gname From T1_Gbun Where '+D_Select+
    ...
    if nSqry.FieldByName('Gjisa').AsString<>'' then begin
      ...
      if Copy(nSqry.FieldByName('Gcode').AsString,1,1)='9' then
      if Base10.H2_Gbun.Locate('Hcode;Scode;Gcode;Gname;Jubun',VarArrayOf([nSqry.FieldByName('Hcode').AsString,'X',nSqry.FieldByName('Gcode').AsString,
      Base10.Seek_Jisa(nSqry.FieldByName('Gjisa').AsString,'3'),Base10.Seek_Jisa(nSqry.FieldByName('Gjisa').AsString,'4')]),[loCaseInsensitive])=true then
        St4:=Base10.H2_Gbun.FieldByName('Gdate').AsString;
    end;
  ...
  { Sqlen :=
    ...
    if Base10.T4_Ssub.Locate('Hcode;Gcode;Gdate;Gjisa;Jubun',VarArrayOf([T3_Sub91Hcode.AsString,T3_Sub91Gcode.AsString,
...
procedure TSobo39.Button509Click(Sender: TObject);
var St1,St2,St3,St4,St8,St9: String;
begin
...
procedure TSobo39.DBGrid101TitleClick(Column: TColumnEh);
begin
  Base10.ColumnS9(mSqry,Column);
end;

procedure TSobo39.DBGrid201TitleClick(Column: TColumnEh);
begin
...
procedure TSobo39.T3_Sub91AfterPost(DataSet: TDataSet);
begin
//
end;

procedure TSobo39.T3_Sub91BeforePost(DataSet: TDataSet);
begin
...
procedure TSobo39.T3_Sub91BeforeClose(DataSet: TDataSet);
begin
  With T3_Sub91 do
  if(State in dsEditModes)Then Post;
end;

procedure TSobo39.DateEdit1ButtonClick(Sender: TObject);
begin
  DateEdit1.Date :=StrToDate(Edit101.Text);
end;

procedure TSobo39.DateEdit1AcceptDate(Sender: TObject; var ADate: TDateTime; var Action: Boolean);
begin
  Edit101.Text :=DateToStr(ADate);
end;

procedure TSobo39.Button701Click(Sender: TObject);
begin
    Seak80.Edit1.Text:='';
    Seak80.FilterTing('');
    if Seak80.Query1.RecordCount=1 Then begin
...
procedure TSobo39.Button702Click(Sender: TObject);
begin
    Seak80.Edit1.Text:='';
    Seak80.FilterTing('');
    if Seak80.Query1.RecordCount=1 Then begin
...
procedure TSobo39.Srart_39_01(Sender: TObject);
begin
  Screen.Cursor:=crHourGlass;
...
{ Sobo39.StBar201.Panels.Items[2].Text:=FormatFloat('###,###,##0',T01);
  Sobo39.StBar201.Panels.Items[3].Text:=FormatFloat('###,###,##0',T02);
  Sobo39.StBar201.Panels.Items[4].Text:=FormatFloat('###,###,##0',T03);
  Sobo39.StBar201.Panels.Items[5].Text:=FormatFloat('###,###,##0',T04); }
  Screen.Cursor:=crDefault;
end;

procedure TSobo39.DBGrid201DrawColumnCell(Sender: TObject;
