...
//if Base10.Seek_Ggeo(Edit108.Text)='X' Then Exit;
  ...
  Sqlen := 'Select Hcode,Scode,Gcode,Gname,Jubun,Gdate From H2_Gbun ';
  ...
  Sqlen := 'Select Hcode,Gcode,Gjisa,Jubun,Gname From T1_Gbun ';
  Base10.Socket.RunSQL(Sqlen);
  Base10.Socket.busyloop;
  if Base10.Socket.body_data <> 'ERROR' then
  ...
  Sqlen := 'Select Hcode,Gcode,Gdate,Gjisa,Jubun,Gqut1,Gqut2,Gqut3 From T4_Ssub '+
           'Where Gdate>=''@Gdate'' and Gdate<=''@Cdate'' ';
  ...
  Sqlen := 'Select Gcode,Gname,Gtel1,Gtel2,Jumin,Giqut,Gisum,Gosum,Gssum From G7_Ggeo ';

  if (Edit104.Text<>'') Then
  Sqlen:=Sqlen+' Where '+
         'Gcode'+'>='+#39+Edit102.Text+#39+' and '+
         'Gcode'+'<='+#39+Edit104.Text+#39;
...
procedure TSobo45.Button509Click(Sender: TObject);
...
begin
  ...
  Sqlen := 'Select S.Hcode,S.Gcode,S.Gjisa,S.Jubun,S.Bcode,Y.Grat8,S.Gsqut,Substr(S.Gdate,9,2) From S1_Ssub S, G4_Book Y '+
...
procedure TSobo45.Button801Click(Sender: TObject);
...
procedure TSobo45.Button802Click(Sender: TObject);
var St1,St2,St3,St4: String;
begin

  Button509Click(Self);

  Sqlen:='Select Substr(Gdate,9,2),Gcode,Jubun,Gjisa,Sum(Gsqut) From S1_Ssub Where '+
         ...
         'Gjisa'+'<>'+#39+'방문출고'+#39+
         ' Group By Gdate,Gcode,Jubun,Gjisa';
  ...
  Sqlen:='Select Gdate,Gcode,Gname,Name1,Name2,Gssum From T1_Ssub Where '+
         'Gdate'+'>='+#39+mSqry.FieldByName('Gdate').AsString+'.00'+#39+' and '+
         'Gdate'+'<='+#39+mSqry.FieldByName('Gdate').AsString+'.99'+#39+' and '+
...
procedure TSobo45.Button803Click(Sender: TObject);
var St1: String;
begin
  Sqlen:='Select Hcode,'+
...
procedure TSobo45.Button901Click(Sender: TObject);
begin
  Edit201.Value:=0;
  Edit202.Value:=0;
  Edit203.Value:=0;
...
procedure TSobo45.Button902Click(Sender: TObject);
var St1: String;
begin
  Sqlen:='Select Max(Gdate) From T2_Ssub Where '+
  ...
  Sqlen:='Select Hcode,'+
         ...
         'Sum31,Sum32,Sum33,Sum34,Sum35,Sum36,Sum37,Bigo1,Bigo2,Yesno From T2_Ssub Where '+
         'Gdate'+'>='+#39+St1+#39+' and '+
         'Gdate'+'<='+#39+St1+#39+' and '+
         'Hcode'+' ='+#39+mSqry.FieldByName('Hcode').AsString+#39;
...
procedure TSobo45.Button903Click(Sender: TObject);
begin
  Sqlen:='Select Hcode,'+
         'Sum01,Sum02,Sum03,Sum04,Sum05,Sum06,Sum07,Sum08,Sum09,Sum10,'+
         'Sum11,Sum12,Sum13,Sum14,Sum15,Sum16,Sum17,Sum18,Sum19,Sum20,'+
  ...
  List1:=List1+1;
    ...
    Edit238.Text :=SGrid.Cells[38,List1];
    Edit239.Text :=SGrid.Cells[39,List1];
    if SGrid.Cells[40,List1]='1' then
...
begin
  ...
  if nSqry.Active=True Then begin
    ...
    if Base10.Seek_Name(Sqlen)='' Then begin
      ...
      '  Sum35, Sum36, Sum37, Bigo1, Bigo2, Yesno ) VALUES ';
      Sq1 :=
      '(''@Gdate'',''@Hcode'', @Sum01 , @Sum02 , @Sum03 , @Sum04 , '+
      '   @Sum05  ,  @Sum06  , @Sum07 , @Sum08 , @Sum09 , @Sum10 , ';
      Sq2 :=
      ...
      '   @Sum23  ,  @Sum24  , @Sum25 , @Sum26 , @Sum27 , @Sum28 , '+
      '   @Sum29  ,  @Sum30  , @Sum31 , @Sum32 , @Sum33 , @Sum34 , '+
      '   @Sum35  ,  @Sum36  , @Sum37 ,''@Bigo1'',''@Bigo2'',''@Yesno'' )';
      ...
      Translate(Sq3, '@Bigo1', Edit238.Text);
      Translate(Sq3, '@Bigo2', Edit239.Text);

      if CheckBox1.Checked=True then
      Translate(Sq3, '@Yesno', '1') else
      Translate(Sq3, '@Yesno', '' );
      ...
      Sq4 :=
      'Sum36=  @Sum36  ,Sum37=  @Sum37  ,Bigo1=''@Bigo1'',Bigo2=''@Bigo2'',Yesno=''@Yesno''  '+
      ...
      TransAuto(Sq3, '@Sum34', FloatToStr(Edit234.Value));
      TransAuto(Sq3, '@Sum35', FloatToStr(Edit235.Value));
      TransAuto(Sq4, '@Sum36', FloatToStr(Edit236.Value));
      TransAuto(Sq4, '@Sum37', FloatToStr(Edit237.Value));
      Translate(Sq4, '@Bigo1', Edit238.Text);
      Translate(Sq4, '@Bigo2', Edit239.Text);
      Translate(Sq4, '@Gdate', Label003.Caption);
      Translate(Sq4, '@Hcode', Label001.Caption);
  ...
  if nSqry.Active=True Then begin
    ...
    if Base10.Seek_Name(Sqlen)='' Then begin
      ...
      'Gsqut=  @Gsqut  ,Gssum=  @Gssum  ,Yesno=''@Yesno'' '+
      'WHERE Gdate=''@Gdate'' and Hcode=''@Hcode'' ';
...
begin
  ...
  Sqlen := 'Select Hcode,'+
           ...
           'Sum11,Sum12,Sum13,Sum14,Sum15,Sum16,Sum17,Sum18,Sum19,Sum20,'+
           'Sum21,Sum22,Sum23,Sum24,Sum25,Sum26,Sum27,Sum28,Sum29,Sum30,'+
           'Sum31,Sum32,Sum33,Sum34,Sum35,Sum36,Sum37,Bigo1,Bigo2 '+
...
procedure TSobo45.DateEdit1ButtonClick(Sender: TObject);
begin
  DateEdit1.Date :=StrToDate(Edit101.Text+'.01');
end;

procedure TSobo45.DateEdit1AcceptDate(Sender: TObject; var ADate: TDateTime; var Action: Boolean);
begin
  Edit101.Text :=Copy(DateToStr(ADate),1,7);
end;

procedure TSobo45.Button701Click(Sender: TObject);
begin
    Seak80.Edit1.Text:='';
    Seak80.FilterTing('');
    if Seak80.Query1.RecordCount=1 Then begin
...
procedure TSobo45.Button702Click(Sender: TObject);
begin
    Seak80.Edit1.Text:='';
    Seak80.FilterTing('');
    if Seak80.Query1.RecordCount=1 Then begin
...
procedure TSobo45.DBGrid201DrawColumnCell(Sender: TObject;
...
procedure TSobo45.DBGrid101DblClick(Sender: TObject);
begin
...
procedure TSobo45.DBGrid201MouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var St1: String;
begin
  St1:='';
  DBGrid201.Hint := '';
