...
begin
  ...
  if nSqry.RecordCount >= 1 then begin
    Sqlen := 'Select Time3,Time4 From S1_Ssub Where '+D_Select+
    ...
    if Base10.Socket.Body_Data <> 'NODATA' then begin
      ...
      if (Base10.Socket.GetData(1, 1)<>'') or (Base10.Socket.GetData(1, 2)<>'') then begin

        MeDlg := MessageDlg('재출력 명세서 입니다. 출력하시겠습니까?', mtConfirmation, [mbYes, mbNo], 0);
        case MeDlg of
        mrYes: begin
        end;
        mrNo: begin
...
//if(nSqry.FieldByName('Hcode').AsString='122')and
  if(Base10.Database.Database='book_05_db')or
    (Base10.Database.Database='book_07_db')then begin
  Seek_Post(Self);
...
begin
  ...
  Sqlen := 'Select Grat9 From G1_Ggeo Where '+D_Select+'Hcode=''@Hcode'' and Gcode=''@Gcode'' ';
...
procedure TSobo21.DBGrid101TitleClick(Column: TColumnEh);
begin
  Base10.ColumnS9(nSqry,Column);
end;

procedure TSobo21.DBGrid201TitleClick(Column: TColumnEh);
begin
...
procedure TSobo21.Button801Click(Sender: TObject);
begin
  if nSqry.Active=True Then begin
...
procedure TSobo21.Button802Click(Sender: TObject);
var St1: String;
begin
  if nSqry.Active=True Then begin
...
procedure TSobo21.Button803Click(Sender: TObject);
begin
  if nSqry.Active=True Then begin
  Cpost:='21';
...
procedure TSobo21.StaticText4Click(Sender: TObject);
begin
  StaticText4.Visible:=False;
  Edit208.SetFocus;
end;

procedure TSobo21.T2_Sub11GSQUTChange(Sender: TField);
begin
...
procedure TSobo21.DateEdit1ButtonClick(Sender: TObject);
begin
  DateEdit1.Date :=StrToDate(Edit101.Text);
end;

procedure TSobo21.DateEdit1AcceptDate(Sender: TObject; var ADate: TDateTime; var Action: Boolean);
begin
...
procedure TSobo21.Button702Click(Sender: TObject);
var Str,St1: String;
begin
  Seek10.Edit1.Text:=Edit105.Text;
...
procedure TSobo21.Seek_Post(Sender: TObject);
begin
  nSqry.BeforePost:=nil;
  DataSource1.Enabled:=False;
  nSqry.First;
