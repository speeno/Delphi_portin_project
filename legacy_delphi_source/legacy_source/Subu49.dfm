object Sobo49: TSobo49
  Left = 71
  Top = 120
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = '세금계산서발행'
  ClientHeight = 533
  ClientWidth = 901
  Color = clBtnFace
  Font.Charset = HANGEUL_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = '굴림'
  Font.Style = [fsBold]
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 12
  object CornerButton1: TCornerButton
    Left = 4
    Top = 3
    Width = 21
    Height = 79
    Cursor = crArrow
    BevelColor = clRed
    BevelWidth = 1
    MouseDownColor = clRed
    Transparent = False
    CornerOptions = [cnTopLeft, cnBottomLeft]
    CornerSize = 10
  end
  object CornerButton2: TCornerButton
    Left = 4
    Top = 84
    Width = 21
    Height = 415
    Cursor = crArrow
    BevelColor = clBlue
    BevelWidth = 1
    MouseDownColor = clBlue
    Transparent = False
    CornerOptions = [cnTopLeft, cnBottomLeft]
    CornerSize = 10
  end
  object CornerButton9: TCornerButton
    Left = 4
    Top = 501
    Width = 21
    Height = 30
    Cursor = crArrow
    BevelColor = clBlack
    BevelWidth = 1
    MouseDownColor = clBlack
    Transparent = False
    CornerOptions = [cnTopLeft, cnBottomLeft]
    CornerSize = 10
  end
  object Label301: TmyLabel3d
    Left = 9
    Top = 27
    Width = 13
    Height = 26
    Caption = '조'#13#10'회'
    Font.Charset = HANGEUL_CHARSET
    Font.Color = clRed
    Font.Height = -13
    Font.Name = '굴림'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    AShadeLTSet = False
  end
  object Label302: TmyLabel3d
    Left = 9
    Top = 248
    Width = 13
    Height = 52
    Caption = '검'#13#10'색'#13#10'자'#13#10'료'
    Font.Charset = HANGEUL_CHARSET
    Font.Color = clBlue
    Font.Height = -13
    Font.Name = '굴림'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    AShadeLTSet = False
  end
  object Label309: TmyLabel3d
    Left = 10
    Top = 505
    Width = 12
    Height = 24
    Caption = '상'#13#10'태'
    Font.Charset = HANGEUL_CHARSET
    Font.Color = clBlack
    Font.Height = -12
    Font.Name = '굴림'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
    AShadeLTSet = False
  end
  object Panel001: TFlatPanel
    Left = 24
    Top = 4
    Width = 873
    Height = 78
    Color = 14416873
    TabOrder = 0
    UseDockManager = True
    object Label101: TmyLabel3d
      Left = 217
      Top = 13
      Width = 15
      Height = 19
      Caption = '~'
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -19
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
      Visible = False
      AShadeLTSet = False
    end
    object Label102: TmyLabel3d
      Left = 329
      Top = 45
      Width = 15
      Height = 19
      Caption = '~'
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -19
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
      Visible = False
      AShadeLTSet = False
    end
    object Panel101: TFlatPanel
      Left = 24
      Top = 28
      Width = 90
      Height = 22
      Caption = '조회일자'
      ParentColor = True
      TabOrder = 6
      UseDockManager = True
    end
    object Panel102: TFlatPanel
      Left = 24
      Top = 44
      Width = 90
      Height = 22
      Caption = '출판사명'
      ParentColor = True
      Visible = False
      TabOrder = 7
      UseDockManager = True
    end
    object Button101: TFlatButton
      Left = 840
      Top = 16
      Width = 25
      Height = 25
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      NumGlyphs = 2
      TabOrder = 8
      Visible = False
      OnClick = Button101Click
    end
    object Button201: TFlatButton
      Left = 840
      Top = 40
      Width = 25
      Height = 25
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      NumGlyphs = 2
      TabOrder = 9
      Visible = False
      OnClick = Button201Click
    end
    object Edit101: TFlatMaskEdit
      Left = 120
      Top = 12
      Width = 97
      Height = 22
      ColorFlat = clWhite
      AutoSize = False
      BorderStyle = bsNone
      Color = clWhite
      EditMask = '!9999.!99.99;1; '
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = '굴림'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 0
      Text = '1999.01.01'
      Visible = False
      OnChange = Edit101Change
      OnKeyDown = Edit101KeyDown
      OnKeyPress = Edit111KeyPress
      ImeName = '한국어(한글) (MS-IME98)'
    end
    object Edit102: TFlatMaskEdit
      Left = 232
      Top = 12
      Width = 97
      Height = 22
      ColorFlat = clWhite
      AutoSize = False
      BorderStyle = bsNone
      Color = clWhite
      EditMask = '!9999.!99.99;1; '
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = '굴림'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 1
      Text = '1999.01.01'
      Visible = False
      OnChange = Edit101Change
      OnKeyDown = Edit111KeyDown
      OnKeyPress = Edit111KeyPress
      ImeName = '한국어(한글) (MS-IME98)'
    end
    object Edit104: TFlatEdit
      Left = 120
      Top = 44
      Width = 209
      Height = 22
      ColorFlat = clWhite
      ImeName = '한국어(한글) (MS-IME98)'
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      Visible = False
      OnChange = Edit101Change
      OnKeyDown = Edit111KeyDown
      OnKeyPress = Edit114KeyPress
    end
    object Edit106: TFlatEdit
      Left = 344
      Top = 44
      Width = 209
      Height = 22
      ColorFlat = clWhite
      ImeName = '한국어(한글) (MS-IME98)'
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
      Visible = False
      OnChange = Edit101Change
      OnKeyDown = Edit102KeyDown
      OnKeyPress = Edit114KeyPress
    end
    object Edit103: TFlatEdit
      Left = 120
      Top = 44
      Width = 57
      Height = 22
      ColorFlat = clWhite
      ImeName = '한국어(한글) (MS-IME98)'
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      Visible = False
    end
    object Edit105: TFlatEdit
      Left = 344
      Top = 44
      Width = 57
      Height = 22
      ColorFlat = clWhite
      ImeName = '한국어(한글) (MS-IME98)'
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      Visible = False
    end
    object Edit100: TFlatComboBox
      Left = 120
      Top = 28
      Width = 97
      Height = 23
      Color = clWindow
      DropDownCount = 9
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ItemHeight = 15
      Items.Strings = (
        '2020.12'
        '2020.11'
        '2020.10'
        '2020.09'
        '2020.08'
        '2020.07'
        '2020.06'
        '2020.05'
        '2020.04'
        '2020.03'
        '2020.02'
        '2020.01'
        '2019.12'
        '2019.11'
        '2019.10'
        '2019.09'
        '2019.08'
        '2019.07'
        '2019.06'
        '2019.05'
        '2019.04'
        '2019.03'
        '2019.02'
        '2019.01'
        '2018.12'
        '2018.11'
        '2018.10'
        '2018.09'
        '2018.08'
        '2018.07'
        '2018.06'
        '2018.05'
        '2018.04'
        '2018.03'
        '2018.02'
        '2018.01'
        '2017.12'
        '2017.11'
        '2017.10'
        '2017.09'
        '2017.08'
        '2017.07'
        '2017.06'
        '2017.05'
        '2017.04'
        '2017.03'
        '2017.02'
        '2017.01'
        '2016.12'
        '2016.11'
        '2016.10'
        '2016.09'
        '2016.08'
        '2016.07'
        '2016.06'
        '2016.05'
        '2016.04'
        '2016.03'
        '2016.02'
        '2016.01'
        '2015.12'
        '2015.11'
        '2015.10'
        '2015.09'
        '2015.08'
        '2015.07'
        '2015.06'
        '2015.05'
        '2015.04'
        '2015.03'
        '2015.02'
        '2015.01'
        '2014.12'
        '2014.11'
        '2014.10'
        '2014.09'
        '2014.08'
        '2014.07'
        '2014.06'
        '2014.05'
        '2014.04'
        '2014.03'
        '2014.02'
        '2014.01'
        '2013.12'
        '2013.11'
        '2013.10'
        '2013.09'
        '2013.08'
        '2013.07'
        '2013.06'
        '2013.05'
        '2013.04'
        '2013.03'
        '2013.02'
        '2013.01'
        '2012.12'
        '2012.11'
        '2012.10'
        '2012.09'
        '2012.08'
        '2012.07'
        '2012.06'
        '2012.05'
        '2012.04'
        '2012.03'
        '2012.02'
        '2012.01'
        '2011.12'
        '2011.11'
        '2011.10'
        '2011.09'
        '2011.08'
        '2011.07'
        '2011.06'
        '2011.05'
        '2011.04'
        '2011.03'
        '2011.02'
        '2011.01'
        '2010.12'
        '2010.11'
        '2010.10'
        '2010.09'
        '2010.08'
        '2010.07'
        '2010.06'
        '2010.05'
        '2010.04'
        '2010.03'
        '2010.02'
        '2010.01'
        '2009.12'
        '2009.11'
        '2009.10'
        '2009.09'
        '2009.08'
        '2009.07'
        '2009.06'
        '2009.05'
        '2009.04'
        '2009.03'
        '2009.02'
        '2009.01'
        '2008.12'
        '2008.11'
        '2008.10'
        '2008.09'
        '2008.08'
        '2008.07'
        '2008.06'
        '2008.05'
        '2008.04'
        '2008.03'
        '2008.02'
        '2008.01'
        '2007.12'
        '2007.11'
        '2007.10'
        '2007.09'
        '2007.08'
        '2007.07'
        '2007.06'
        '2007.05'
        '2007.04'
        '2007.03'
        '2007.02'
        '2007.01')
      ParentFont = False
      TabOrder = 10
      Text = '2010.12'
      ItemIndex = -1
      OnChange = Edit101Change
    end
    object dxButton1: TdxButton
      Left = 790
      Top = 15
      Width = 65
      Height = 50
      OnClick = Button101Click
      Caption = '검색'
      TabOrder = 11
      Glyph.Data = {
        
        6767676767676767674C67663B66673E67665466666566666666}
      Layout = blGlyphTop
    end
  end
  object Panel002: TFlatPanel
    Left = 24
    Top = 85
    Width = 745
    Height = 414
    ParentColor = True
    TabOrder = 1
    UseDockManager = True
    object DBGrid101: TDBGridEh
      Left = 1
      Top = 1
      Width = 743
      Height = 412
      Align = alClient
      BorderStyle = bsNone
      DataSource = DataSource1
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = '굴림'
      Font.Style = [fsBold]
      FooterColor = 16311512
      FooterFont.Charset = HANGEUL_CHARSET
      FooterFont.Color = clRed
      FooterFont.Height = -12
      FooterFont.Name = '굴림'
      FooterFont.Style = [fsBold]
      FooterRowCount = 1
      ImeName = '한국어 입력 시스템 (IME 2000)'
      ParentFont = False
      SumList.Active = True
      TabOrder = 0
      TitleFont.Charset = HANGEUL_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -12
      TitleFont.Name = '굴림'
      TitleFont.Style = [fsBold]
      OnKeyDown = DBGrid101KeyDown
      OnKeyPress = DBGrid101KeyPress
      OnTitleClick = DBGrid101TitleClick
      Columns = <
        item
          EditButtons = <>
          FieldName = 'GCODE'
          Footer.Value = '합계'
          Footer.ValueType = fvtStaticText
          Footers = <>
          Title.Alignment = taCenter
          Title.Caption = '코드'
          Width = 45
        end
        item
          EditButtons = <>
          FieldName = 'GNAME'
          Footers = <>
          Title.Alignment = taCenter
          Title.Caption = '출판사명'
          Width = 210
        end
        item
          EditButtons = <>
          FieldName = 'SDATE'
          Footers = <>
          Title.Alignment = taCenter
          Title.Caption = '발행일자'
          Width = 78
        end
        item
          EditButtons = <>
          FieldName = 'GOSUM'
          Footer.ValueType = fvtSum
          Footers = <>
          Title.Alignment = taCenter
          Title.Caption = '금액'
          Width = 87
        end
        item
          EditButtons = <>
          FieldName = 'GBSUM'
          Footer.ValueType = fvtSum
          Footers = <>
          Title.Alignment = taCenter
          Title.Caption = '세액'
          Width = 87
        end
        item
          Color = 11861755
          EditButtons = <>
          FieldName = 'GSSUM'
          Footer.ValueType = fvtSum
          Footers = <>
          Title.Alignment = taCenter
          Title.Caption = '발행금액'
          Width = 87
        end
        item
          Alignment = taCenter
          EditButtons = <>
          FieldName = 'CODE2'
          Footers = <>
          PickList.Strings = (
            '영수'
            '청구')
          Title.Alignment = taCenter
          Title.Caption = '구분'
          Width = 55
        end
        item
          Alignment = taCenter
          Checkboxes = True
          EditButtons = <>
          FieldName = 'CODE1'
          Footers = <>
          KeyList.Strings = (
            '1;On'
            '0;Off')
          Title.Alignment = taCenter
          Title.Caption = '저장'
          Width = 55
          OnUpdateData = DBGrid101Columns7UpdateData
        end>
    end
  end
  object Panel007: TFlatPanel
    Left = 24
    Top = 502
    Width = 873
    Height = 29
    Color = clInfoBk
    TabOrder = 2
    UseDockManager = True
    object ProgressBar0: TFlatProgressBar
      Left = 528
      Top = 5
      Width = 336
      Height = 19
      Min = 0
      Max = 100
    end
    object Panel008: TFlatPanel
      Left = 8
      Top = 5
      Width = 97
      Height = 19
      Caption = '레코드'
      ParentColor = True
      TabOrder = 0
      UseDockManager = True
    end
    object Panel009: TFlatPanel
      Left = 112
      Top = 5
      Width = 97
      Height = 19
      ParentColor = True
      TabOrder = 1
      UseDockManager = True
    end
    object Panel010: TFlatPanel
      Left = 424
      Top = 5
      Width = 97
      Height = 19
      Caption = '검색진행'
      ParentColor = True
      TabOrder = 2
      UseDockManager = True
    end
    object ProgressBar1: TProgressBar
      Left = 528
      Top = 5
      Width = 335
      Height = 18
      Min = 0
      Max = 100
      TabOrder = 3
    end
  end
  object Panel003: TFlatPanel
    Left = 768
    Top = 85
    Width = 129
    Height = 414
    ParentColor = True
    TabOrder = 3
    UseDockManager = True
    object myLabel3d1: TmyLabel3d
      Left = 58
      Top = 53
      Width = 15
      Height = 19
      Caption = '~'
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -19
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
      Visible = False
      AShadeLTSet = False
    end
    object Label100: TmyLabel3d
      Left = 30
      Top = 15
      Width = 68
      Height = 16
      Caption = '검색일자'
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clRed
      Font.Height = -16
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
      Visible = False
      AStyle3D = Resit3d
      AShadeLTSet = False
    end
    object Button100: TFlatButton
      Left = 12
      Top = 344
      Width = 105
      Height = 25
      Color = clInfoBk
      Caption = '선택출력(F2)'
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      Layout = blGlyphLeft
      NumGlyphs = 2
      ParentColor = False
      TabOrder = 0
      OnClick = Button100Click
    end
    object Button200: TFlatButton
      Left = 12
      Top = 376
      Width = 105
      Height = 25
      Color = clInfoBk
      Caption = '전체출력(F3)'
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      Layout = blGlyphLeft
      NumGlyphs = 2
      ParentColor = False
      TabOrder = 1
      OnClick = Button200Click
    end
    object Edit201: TFlatMaskEdit
      Left = 16
      Top = 36
      Width = 97
      Height = 22
      ColorFlat = clWhite
      AutoSize = False
      BorderStyle = bsNone
      Color = clWhite
      EditMask = '!9999.!99.99;1; '
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = '굴림'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 2
      Text = '1999.01.01'
      Visible = False
      OnChange = Edit101Change
      OnKeyDown = Edit101KeyDown
      OnKeyPress = Edit111KeyPress
      ImeName = '한국어(한글) (MS-IME98)'
    end
    object Edit202: TFlatMaskEdit
      Left = 16
      Top = 68
      Width = 97
      Height = 22
      ColorFlat = clWhite
      AutoSize = False
      BorderStyle = bsNone
      Color = clWhite
      EditMask = '!9999.!99.99;1; '
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = '굴림'
      Font.Style = [fsBold]
      MaxLength = 10
      ParentFont = False
      TabOrder = 3
      Text = '1999.01.01'
      Visible = False
      OnChange = Edit101Change
      OnKeyDown = Edit101KeyDown
      OnKeyPress = Edit111KeyPress
      ImeName = '한국어(한글) (MS-IME98)'
    end
    object RadioButton4: TFlatRadioButton
      Left = 40
      Top = 96
      Width = 41
      Height = 81
      Caption = '전체저장'
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      OnClick = RadioButton4Click
    end
    object RadioButton5: TFlatRadioButton
      Left = 40
      Top = 192
      Width = 41
      Height = 81
      Caption = '전체해제'
      Font.Charset = HANGEUL_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = '굴림'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
      OnClick = RadioButton5Click
    end
  end
  object DataSource1: TDataSource
    DataSet = T4_Sub91
    OnDataChange = DataSource1DataChange
    Left = 42
    Top = 126
  end
  object DataSource2: TDataSource
    OnDataChange = DataSource2DataChange
    Left = 74
    Top = 126
  end
  object T4_Sub91: TClientDataSet
    Aggregates = <>
    Params = <>
    BeforeClose = T4_Sub91BeforeClose
    BeforePost = T4_Sub91BeforePost
    AfterPost = T4_Sub91AfterPost
    AfterCancel = T4_Sub91AfterCancel
    BeforeScroll = T4_Sub91AfterScroll
    AfterScroll = T4_Sub91AfterScroll
    OnNewRecord = T4_Sub91NewRecord
    Left = 42
    Top = 158
    object T4_Sub91SDATE: TStringField
      FieldName = 'SDATE'
      EditMask = '!0000.!90.90;1; '
      Size = 10
    end
    object T4_Sub91GDATE: TStringField
      FieldName = 'GDATE'
      EditMask = '!0000.!90.90;1; '
      Size = 10
    end
    object T4_Sub91GUBUN: TStringField
      FieldName = 'GUBUN'
      Size = 10
    end
    object T4_Sub91GCODE: TStringField
      FieldName = 'GCODE'
      Size = 10
    end
    object T4_Sub91GNAME: TStringField
      FieldName = 'GNAME'
      Size = 100
    end
    object T4_Sub91GIQUT: TFloatField
      FieldName = 'GIQUT'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GISUM: TFloatField
      FieldName = 'GISUM'
      EditFormat = '#########0'
    end
    object T4_Sub91GOQUT: TFloatField
      FieldName = 'GOQUT'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GOSUM: TFloatField
      FieldName = 'GOSUM'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GJQUT: TFloatField
      FieldName = 'GJQUT'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GJSUM: TFloatField
      FieldName = 'GJSUM'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GBQUT: TFloatField
      FieldName = 'GBQUT'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GBSUM: TFloatField
      FieldName = 'GBSUM'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GPQUT: TFloatField
      FieldName = 'GPQUT'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GPSUM: TFloatField
      FieldName = 'GPSUM'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GSUSU: TFloatField
      FieldName = 'GSUSU'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GSQUT: TFloatField
      FieldName = 'GSQUT'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GSSUM: TFloatField
      FieldName = 'GSSUM'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GSUMX: TFloatField
      FieldName = 'GSUMX'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91GSUMY: TFloatField
      FieldName = 'GSUMY'
      DisplayFormat = '###,###,###0'
      EditFormat = '#########0'
    end
    object T4_Sub91CODE1: TStringField
      FieldName = 'CODE1'
      Size = 4
    end
    object T4_Sub91CODE2: TStringField
      FieldName = 'CODE2'
      Size = 4
    end
    object T4_Sub91CODE3: TStringField
      FieldName = 'CODE3'
      Size = 4
    end
  end
  object T4_Sub92: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 74
    Top = 158
  end
end
