...
| 2026-05-30 (account-menu-fxx-rbac C/D/E) | **4-key Fxx fetch + 화면 caps 게이트**. ① `fetch_fxx_matrix` → `hcode+gname+gcode` 3-key (`LIMIT 1` 제거). ② `infer_login_profile` F51~55 우세 지문. ③ `/auth/refresh` → `refresh_user_claims_from_db` 재도출 + JWT `gname`/`rdb`. ④ 프론트 `useScreenCaps`·`WriteGate`/`PrintGate`·사이드바 `canAccessScreen`. 감사: [`analysis/audit/account-menu-fidelity.md`](../analysis/audit/account-menu-fidelity.md). 회귀: `test_auth_fxx_4key_fetch`·`test_screen_caps_static`·`test_account_menu_fidelity_coverage`. |
| 2026-04-22 (3차 사이클) | **DEC-056 분기 0 (Wave B) 신설 + DEC-058 정산 변형사 DB 컬럼 어댑터**. 사용자 보고("정산 데이터 조회 안됨 / admin 메뉴 일부 안 보임 / 통계 화면 403"). ① **분기 0** — `auth_service._has_admin_role_mapping(user_id)` 헬퍼 + 동기/비동기 `_resolve_role_and_permissions{,_async}` 양쪽 첫 분기에서 호출 (LSP 일관). admin role 매핑된 사용자는 hcode='99999' / Id_Logn Fxx 가 operator 합성하더라도 즉시 admin/['*']. 통계 화면(`stats/customer-analysis`, `stats/sales-period` 등) 403 도 분기 0 발효 후 admin = `*` permission 으로 자동 통과. ② **A2 — tax_invoice 변형사 컬럼 어댑터** (DEC-058 보강): `_t2_columns(server_id)` 캐시 + `_build_sql_list_tax(cols)` / `_build_sql_count_tax(cols)` 동적 SELECT. `T2_Ssub.Chek3 / Sdate / Yesno` 부재 시 정적 리터럴(`'0' AS Chek3`) 로 1054(Unknown column) → HTTP 500 차단. `t5_ssub_adapt` 패턴 1:1 재사용(신규 패턴 0). ③ **신규 테스트 5종 + 디버그 도구 2종**: `test_admin_resolver_branch0_priority` 5/5 + `test_admin_settlement_full_access` 4/4 + `test_settlement_billing_no_inline_correlated_subquery` 1/1 + `test_settlement_tax_invoice_chek3_optional` 6/6 + `test_settlement_cash_status_sdate_response_shape` 2/2 = 18/18 PASS. `debug/probe_settlement_endpoints.py` (정산 7화면 라이브 점검) 4 서버 × 7 endpoint = **28/28 OK**. `debug/show_jwt_claims.py` (JWT 클레임 진단). ④ 인접 회귀 무영향 — `test_admin_superuser_safety_net` 11/11 PASS, `test_c10_admin_phase1::test_P_01..05` 5/5 PASS. ReadLints 0. **운영 가이드**: 분기 0 적용 후 모든 admin 사용자는 한 번 재로그인 필요(또는 토큰 만료 대기 — 15분). |
