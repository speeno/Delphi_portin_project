...
요약: **C9·C3 각 T1~T8 완료**. C9 단일 계약 [`migration/contracts/master_data.yaml`](../migration/contracts/master_data.yaml) (`도서물류관리프로그램/backend/app/routers/masters.py` 등). C3 [`migration/contracts/inbound_receipt.yaml`](../migration/contracts/inbound_receipt.yaml)·`routers/inbound.py`. C9 **Sobo17/38/39 등 부분 쓰기·4변종 통합** 등은 phase2·JSON `deferrals`·[`docs/phase2-next-work-queue.md`](phase2-next-work-queue.md) §4 후속으로 분리.
