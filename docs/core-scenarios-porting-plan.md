...
각 시나리오의 진행 상태는 [`dashboard/data/porting-screens.json`](../dashboard/data/porting-screens.json)에 머신리더블로 동기화됩니다. 아래는 **주간 회의용 한 장 요약**입니다.

> **§4 스냅샷 (2026-05-08):** `T1`~`T8` 상태 문자열·노트·계약·테스트 경로는 항상 [`porting-screens.json`](../dashboard/data/porting-screens.json)의 해당 `scenarios[]` 객체를 정본으로 본다. 본 절은 동일 날짜 기준 요약이며, 수치·pytest 행 수·hotfix ID는 JSON을 따른다.

### 4.1 단계 0 — C1 로그인

요약: **T1~T8 완료** (phase1 동결). 계약 [`migration/contracts/login.yaml`](../migration/contracts/login.yaml), 테스트 [`migration/test-cases/login.json`](../migration/test-cases/login.json), 제품 [`도서물류관리프로그램/backend/app/routers/auth.py`](../도서물류관리프로그램/backend/app/routers/auth.py)·[`도서물류관리프로그램/frontend/src/app/(public)/login/`](../도서물류관리프로그램/frontend/src/app/(public)/login/). D-LOGIN-1~4 등 1차 범위 외는 DEC·YAML `out_of_scope`로 명시.

### 4.2 단계 1 — C6 거래/잔액 조회 (읽기 전용)

요약: **T1~T8 완료**. 계약 [`migration/contracts/sales_inquiry.yaml`](../migration/contracts/sales_inquiry.yaml), 테스트 [`migration/test-cases/sales_inquiry.json`](../migration/test-cases/sales_inquiry.json), 라우터 `transactions` / `inventory` / `reports`. 레이아웃·회귀: `Sobo21`·`Sobo31`·`Sobo61`·`Sobo62` 및 `test/test_c6_inquiry_phase1.py` (JSON `evaluation_report` 참조).

### 4.3 단계 2 — C9 마스터 등록 → C3 입고 접수

요약: **C9·C3 각 T1~T8 완료**. C9 단일 계약 [`migration/contracts/master_data.yaml`](../migration/contracts/master_data.yaml) **v1.4.0** — Sobo11/14 전 CRUD + **Sobo17/38/39 전 CRUD**(book-code thin alias·discount `variant`)·`require_permission('master.write')` (`도서물류관리프로그램/backend/app/routers/masters.py` 등). C3 [`migration/contracts/inbound_receipt.yaml`](../migration/contracts/inbound_receipt.yaml)·`routers/inbound.py`. 상세 회귀·probe 매트릭스는 [`analysis/regression/c9_master_writes.md`](../analysis/regression/c9_master_writes.md) 및 [`docs/phase2-next-work-queue.md`](phase2-next-work-queue.md) §4 (완료 처리).

### 4.4 단계 3 — C2 출고(+C11) → C4 반품 → C5 정산(+C12)

요약: **C2·C4·C5 각 T1~T8 완료** (C4 phase2·C5 phase2·DEC-031/032/034/035/036·058 등은 JSON·계약 본문 참조). 계약 [`outbound_order.yaml`](../migration/contracts/outbound_order.yaml)·[`return_receipt.yaml`](../migration/contracts/return_receipt.yaml)·[`settlement_billing.yaml`](../migration/contracts/settlement_billing.yaml).

### 4.5 단계 4 — C7 인쇄 → C8 바코드

요약: **C7·C8 완료** (C7 Phase 1·2-α·B4 PoC·계약 `print_invoice.yaml` / `print_label.yaml` 등, C8 [`barcode_scan.yaml`](../migration/contracts/barcode_scan.yaml)·`POST /api/v1/scan/match`). 미리보기·PDF·라벨 라우트는 제품 `print`·`settlement`·호출 화면 내 통합 — 엔드포인트 표는 JSON 시나리오 `C7`·`C8`.

### 4.6 단계 5 — C10 권한 관리 (+C13/C15)

요약: **T1~T8 완료**. 계약 [`admin_permissions.yaml`](../migration/contracts/admin_permissions.yaml)·[`admin_web_platform.yaml`](../migration/contracts/admin_web_platform.yaml), UI **`/admin/id-logn`** (통합), 백엔드 `core/deps.py`·`id_logn_service.py`·`d_select.py` 등 — 세부는 JSON 시나리오 `C10`.

**확장 라인 (C11·C13_stats·C14·C15_cutover):** 동일하게 `porting-screens.json` 내 해당 `id` 블록의 `tasks`가 정본이다.
...
- 사람용 한 장 요약: 본 문서 §4 (스냅샷 날짜·요약 문단; 행별 T1~T8 정본은 [`porting-screens.json`](../dashboard/data/porting-screens.json))
