...
### 3.8 계정별 컬러 테마 텍스트 대비 (WCAG AA)

`frontend/src/app/themes.css` 의 모든 `[data-theme]` 테마는 텍스트가 올라가는 토큰 쌍
(본문/보조/버튼/링크/사이드바/destructive 등)에서 **대비 4.5:1 이상**을 유지한다.
색조(hue)는 테마 정체성이므로 유지하고 **명도만** 조정한다. 밝은 액센트(라임·오렌지·틸)
위 텍스트는 흰색 대신 **어두운 잉크**를 쓴다(§3.7 Vivid Lime CTA 와 동일 패턴).
가드: `test/test_theme_contrast_guard.py` 가 전 테마·전 쌍을 자동 검증한다. (2026-07-04)
...
## 7.3 셸 기본 디자인 — 2026-08-25 목업 1차 적용 (DEC-199)

사용자 제공 기본 화면 목업(대시보드)에서 **토큰·셸 구조 수준**으로 옮길 수 있는 부분을 기본
디자인(`:root`)으로 적용했다. 화면 페이지는 손대지 않고 토큰 회전으로만 영향받는다(§8-4 OCP).

| 영역 | 목업 | 적용 |
|------|------|------|
| 헤더 | 최상단 전폭 흰 바 · 좌 워드마크 · 우 「(주)교문사 \| 경리부」 칩 | `layout.tsx` 헤더 우선 배치, `header.tsx` 워드마크 + 사용자 칩(보조 뱃지는 드롭다운) |
| 사이드바 | 진회색(#4A4A4A 계열) · 상단 접기 박스(«) · 밝은 글자 | `--sidebar` L0.40 / `--sidebar-foreground` #F5F5F5 / `--sidebar-header` L0.33 / `--sidebar-accent` L0.50 |
| 탭 바 | 회색 한 줄 · 활성 탭 라임 pill | `--tabbar` #D9D9D9, `--tab-active: var(--vivid-lime)` — 탭 줄과 도구 줄을 한 줄로 합침 |
| 콘텐츠 | 연회색 캔버스 + 흰 카드 | `--background` #F5F5F5(Neutral Gray_02), `--card` White 유지 |

- **Vivid Lime 예외**: §8-1 은 CTA/알림 외 사용을 막지만, 목업의 «현재 화면» 표시(활성 탭 1곳)는
  사용자 지시로 허용한다. `--tab-active` 토큰 한 곳에만 배치하고 `--primary`/`--sidebar-primary`
  에는 여전히 두지 않는다.
- 사이드바 대비: Inverse Text on L0.40 ≈ 8:1, 활성 행(L0.50) 위 흰 글자 ≈ 4.9:1 — `test_theme_contrast_guard`.
- **사이드바 플라이아웃**(2026-08-25 13:03 목업): 대메뉴 클릭 → 오른쪽 서브메뉴 패널. 「현재 화면」
  (포커스 창)이 속한 대메뉴·항목은 `--nav-active`(라임), 열려 있는 다른 창은 ✓. 메뉴 순서·구성은
  `MENU_GROUPS`/`SIDEBAR_LAYOUTS` 그대로(사용자: 순서 변경 금지). 활성 탭도 같은 `--nav-active`.
- **워드마크**(13:01 이미지): 배경·격자 제거한 투명 PNG `public/brand/bukioworks-wordmark.png`
  (Bukio Black 잉크) — 앱 헤더·로그인. 마케팅 라임 바탕 PNG 는 랜딩·공지·풋터에 그대로.
- **시계·날씨 위젯**(13:04 요청): 검정 박스+라임 글로우 → Bukio Black/Secondary 텍스트로 흰 헤더에
  맞춤. 13:12 「테두리 제거」— 카드 윤곽선·그림자 없이 헤더 위에 텍스트만 얹힌다(`border`·`shadow` 금지).
...
### 7.5 표(목록)·섹션 헤더 공통 — DEC-203 (2026-08-25 「도서별 수불원장 결과」「거래처 원장」 목업)

모든 화면의 표에 같은 기준으로 적용(공용 `DataGrid` + `list-table-card.tsx` 상수 + 인라인 표 프레임 일괄 제거).

- **표 프레임 없음**: 회색 캔버스 위 흰 행, 가로 구분선(`border-border/70`)만. 둥근 모서리·테두리·그림자 금지.
- **헤더행**: `--table-head`(#EDEDED) 배경, Bukio Black 14px semibold. 세로 구분선 없음.
- **선택 행**: `--row-selected`(민트 #C9F5E6). **키보드 포커스 행**: 표가 포커스를 가질 때 `--row-focus`(파랑)
  2px 안쪽 테두리(`group-focus-within/dg`).
- **합계행**: `bg-muted` 회색 + 굵게(종전 Pale Lime 은 목업으로 대체). 전일재고/전일미수 행은 옅은 회색.
- **섹션 헤더**(`SectionHeader`): 표 위에 굵은 제목(18px) + 회색 메타(코드·기간·선택 일자), 오른쪽에
  「내용 전체 보기」 체크(상단 표를 스크롤 없이 모두 펼침 — 분할 off) · 「엑셀 다운로드」 · 「출력」
  알약형 외곽선 버튼. 기존 화면의 표 제목 `h2` 는 굵기·크기만 상향(text-base/lg bold).
- **엑셀 다운로드**: 전용 라우트가 없는 조회 화면은 `lib/table-export.ts::exportTableXlsx` → 범용
  `POST /api/v1/export/table-xlsx`(화면 표시 컬럼 순서 그대로). **출력**: `printTable` — 표만 새 창에 그려
  브라우저 인쇄.
- 숫자 정렬은 오른쪽 유지(목업은 가운데 — 회계 관례 우선, 원하면 토글 가능).
- **밀도(DEC-211, 2026-08-26)**: 띠 `py-1.5`·제목 20px·부제 미표시(툴팁), 페이지 루트 `gap-3 pb-4`,
  섹션 헤더 `pb-1`·16px, 표 셀 `py-2` — 하단 목록이 더 많이 보이도록 상단 높이를 최소로.
- **페이저**(DEC-205): 표 하단 가운데 번호형 « ‹ 1 2 3 4 5 … N › » — 현재 페이지는 라임 원(`nav-active`),
  나머지는 텍스트. 상단엔 건수·「페이지당」 select 유지.
...
- 2026-08-25 — §7.3 셸 기본 디자인 1차 적용(DEC-199): 어두운 사이드바·전폭 헤더·회색 탭 바·연회색 캔버스.

| 일자 | 버전 | 변경 |
|------|------|------|
| 2026-05-11 | v1.0 | 최초 작성 — PDF 8 page 기반 토큰·매핑·금지 사항 정리 |
| 2026-07-04 | v1.1 | §3.8 신설 — 테마 텍스트 대비 4.5:1 규칙 + 가드 테스트 (테마 10종 36개 토큰 교정) |
