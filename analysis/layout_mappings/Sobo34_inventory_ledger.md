...
## 5-b. 행 집합 = 기간 거래 ∪ 재고 스냅샷 (DEC-183)

누적(nSqry)에 없는 도서를 **스냅샷 결과셋(sSqry)** 에서 찾아 행으로 추가한다
(L1046~1055). 이걸 빠뜨리면 조회 기간이 짧을수록(예: 시작일=기준일) 재고만 있고
그 기간 거래가 없는 도서·분류가 통째로 사라진다 — 2026-08-22 사용자 리포트
"분류 수가 갑자기 3개로 줄어든다" 의 직접 원인.

## 5-c. 합계 행 (DEC-182)

두 그리드 하단 「합계」 = `DataGrid` `totals` 프롭. 상단은 검색 결과 전체 합계,
하단은 **선택 분류** 의 도서 합계(= 그 분류의 상단 행 값). 정가는 합산 대상이 아니다.

## 6. 남은 결정

1. ~~**재고(반)** 를 1차에 포함할지~~ → **포함**(운영 요청 2026-08-22). `_fetch_return_stock_asof` 신설.
2. ~~상단 축~~ → **레거시 준수**(운영 확인): 상단=분류 / 하단=도서(도서코드·정가 표시).
3. (미해결) `Subu34.pas` L463 `chul_09_db` 폐기 분기가 공통 경로에 남아 있다. 해당 테넌트
   폐기 수치가 어긋나면 `migration/contracts/` `customer_variants` 로 옮긴다(DEC-033).
4. (미확인) **라이브 대사 없음** — 전·현재고는 DEC-138 검증 로직 재사용이지만, 재고(반)와
   분류 롤업은 운영 데이터와 아직 대조하지 않았다.
