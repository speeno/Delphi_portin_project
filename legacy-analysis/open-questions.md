...
- **영향 범위**: C3 Phase 2 — 자동 수신 기능 활성화 시점. 1차는 사용자 파일 업로드 우회(DEC-027) 로 핵심 기능 보존.
- **상태**: **후속 이관** — 운영 인프라/계정 합의 후 별도 결정으로 채널/스케줄 동결.
- **후속 작업**: (1) 운영 FTP 자격증명 인계 또는 SFTP 전환 합의 (2) 파일 포맷 sample 1건 수집 (3) `inbound_service.import_from_file` 가 받는 입력 포맷과 동일하게 통일 (4) 스케줄러(매일 N회) 정의.

---
*최종 업데이트: 2026-04-19 — OQ-IN-1 신규 (C3 입고 1차 FTP 우회 결정 후속).*
