...
---
*최종 업데이트: 2026-06-14 — DEC-065 + P4 보강 (거래명세서 Sobo21 화면 내 신규추가 — outbound create_order 재사용 + 단가/비율/금액 패리티 + 직전거래가 G7_Ggeo 게이트 + 키보드 전용 in-grid / P4: 거래현황(상세) Subu24 검색 다이얼로그 — bcode/pubun 라인 필터 + 듀얼그리드 + 키보드 전용 모달). 직전: 2026-06-05 — DEC-064 §Idnum 정합 (Sobo21 6축 합성키 + 좌·우 한 화면 + 도서 lookup + DataGrid 키보드 네비).*
