...
- **미결(사용자 확인 대기)**
  - 교문사 도서코드는 1팀 `1~3418`(1,898건) + `80000~89011`(144건), 2팀 `90001~99801`(1,366건).
    신규 1팀 코드가 `3419`(본 계열 다음)인지 `89012`(8만번대 다음)인지 — 현재 제안은 `89012`.
  - 자동 채번은 `9` 접두를 예약으로 배제하므로 **2팀(90001~) 신규 도서는 자동 코드가 나오지 않는다**
    (수기 입력). 2팀도 자동 채번이 필요하면 거래처 접두 스킴처럼 팀별 대역 채번이 필요하다.
