...
- **요청**: "북이오웍스 로그인은 이메일 아이디/비번. 하단 [위러브솔루션→북이오웍스 계정 전환하기] → 전환 페이지에서
  ...
  델파이는 그대로". 메일 = Brevo SMTP 무료 티어(smtp-relay.brevo.com:587), 발신자 newoneseek@buk.io(2026-09-03 — bukio.works 미등록·bukio.com 타인 파킹으로 확인되어 실 운영 도메인으로 교체).
...
### DEC-237 — 기초관리 필터 띠 버튼: 역할 4단 위계 + 높이 통일 + 사용자 언어 (2026-09-05)

- **배경** — 기초관리 12개 화면의 필터 영역 버튼이 역할은 같은데 스타일이 제각각이었다.
  생성 CTA 가 `brand-primary`/`default`/`secondary`/`outline` **4종**, 저장이 `default`/`secondary`
  **2종**, 조회는 색만 같고 높이(h-7/h-8)와 위치(`ml-auto` 유무)가 달랐다.
- **결정 — 역할이 스타일을 정한다 (사용자 확정 2026-09-05)**

  | 계층 | variant | 높이 | 대상 |
  |---|---|---|---|
  | ① 생성 CTA (화면당 1개) | `brand-primary` + `font-semibold shadow-sm` | h-8 | 신규 거래처/도서/저자/입고처/도서분류/거래처구분/입력 |
  | ② 주 실행 | `default` (Bukio Black) | 띠 h-8 · 패널 sm | 조회 · 저장 · 등록 |
  | ③ 도구 | `outline` | 띠 h-8 · 패널 sm | 찾기 · 컬럼 · 엑셀 업로드 · 엑셀 저장 · 신규 입력(폼 비우기) |
  | ④ 되돌리기 | `ghost` | 띠 h-8 · 패널 sm | 초기화 · 취소 · 선택 해제 · 접기 |
  | ⑤ 삭제 | `destructive` | sm | 삭제 |

- **높이는 한 줄 안에서 같게** — 필터 띠 버튼을 전부 h-8 로 올렸다(클릭 타깃 28→32px).
  공용 트리거 3종(`GridColumnSettings`·`MasterImportButton`·`MasterLookupButton`)의 `size="sm"`
  을 제거했으므로 이 컴포넌트를 쓰는 **다른 메뉴 화면(약 49개)의 띠도 함께 정렬**된다 —
  조회는 원래 h-8 이었으므로 어긋나 있던 쪽을 맞춘 것이다. 패널 내부는 `sm` 유지(DEC-231 결).
- **초기화는 `outline` → `ghost`** — 조회 바로 옆이라 오조작 위험이 있고, 아웃라인 도구 5개가
  나란히 서면 눈이 훑을 곳을 못 찾았다. 무게를 낮춰 위계를 만들고 도구 묶음과 시각적으로 분리.
- **조회 위치** — `PageHeader` 가 이미 `justify-end` 라 `ml-auto` 는 불필요.
  도서관리에만 남아 있던 `ml-auto` 를 제거해 12개 화면의 조회 위치를 맞췄다.
- **문구를 사용자 말로**
  - 공용 `MasterLookupButton` 기본 라벨 `"검색 팝업"` → **`"찾기"`** ("팝업"은 구현 용어).
    직접 지정하던 3개 화면의 `title="검색 팝업"` 을 제거해 기본값을 쓰게 했다.
    거래명세서의 `"도서 검색"` 도 `"도서 찾기"` 로 어휘 통일.
  - 거래처관리 검색 placeholder 오기 수정: `"예: A0001 또는 도서"` → `"예: A0001 또는 거래처명"`
    (거래처 화면인데 예시가 도서였다).
- **남은 것** — 출판사·도서코드 두 화면만 띠에 생성 CTA 가 없다(생성 경로가 패널의 「신규 입력」).
  띠 CTA 를 새로 넣을지는 기능 추가라 별도 판단.

### DEC-238 — 입고관리 필터 띠·목록표: 역할 위계 적용 + 초기화(Enter 무간섭) + 엑셀 저장 공용화 (2026-09-05)

- **배경** — DEC-237 의 역할 4단을 입고관리 7개 메뉴에 적용하려고 전수 조사한 결과:
  ① 공용 `TransactionStatusScreen` 의 조회가 `secondary`(회색) — 입고·출고·반품·폐기 현황 5축
  전부에서 조회만 회색이었다. ② 생성 CTA(입고 접수 「신규 등록」)가 검정. ③ 「파일 업로드」가 회색.
  ④ 초기화 버튼이 5개 화면 전부에 없음. ⑤ 엑셀 저장이 하나도 없음. ⑥ 입고명세서 부제가
  "조회·출력"인데 출력 버튼 없음.
- **결정**
  1. 조회 = 검정(`default`) 전 화면 예외 없음 — 현황 공용 컴포넌트 수정으로 5축 동시 정리.
  2. 생성 CTA 라벨은 **대상을 붙인다** — 「신규 입고」(기초관리의 「신규 거래처/도서」와 같은 꼴).
  3. **초기화는 Enter 이동 스톱(`FILTER_STOP_IDS`)에 넣지 않는다** (사용자 요구 "엔터 사용법이 간섭
     받지 않도록"). 구조적 근거: `advanceFilterOnEnter` 는 스톱 목록만 따라가고 마지막 스톱(조회)만
     자동 클릭한다 → 초기화는 절대 Enter 로 도달·실행되지 않는다. DOM 순서도 조회 → 초기화라
     `advanceAfterSelect`(선택 확정 후 다음 포커서블)의 결과도 바뀌지 않는다. 라벨 「초기화」는
     자동실행 판별 `QUERY_BUTTON_RE(/조회|검색/)` 에 걸리지 않는다. 되돌린 뒤 첫 필터칸으로
     포커스를 복귀시켜 키보드 흐름을 잇는다. 스타일 = ④ `ghost`.
  4. **엑셀 저장은 공용 컴포넌트로** — `components/data-grid/grid-export-button.tsx` +
     `lib/table-export.ts#gridRowsToExport`. 화면 표시 컬럼(순서·숨김 = `useGridPrefs`)을 그대로
     내보내는 2026-08-25 규칙을 코드로 고정. 새 백엔드 없이 기존 `POST /export/table-xlsx` 재사용.
     JSX 셀은 `DataGridColumn.exportValue` 로 값을 지정(전표번호 5-pad·합성키 일자·거래처명
     코드 폴백·상태 라벨). 그리드 툴바의 「컬럼」 옆 `sm`, 띠에서는 h-8.
  5. 부제는 기능을 앞서지 않는다 — 입고명세서 "조회·출력" → "조회".
- **적용 범위** — 입고 접수 · 입고 파일 업로드(변경 없음) · 일별/기간별 입고내역서 · 입고명세서 ·
  입고 현황(목록/상세/요약) · 신간발행. 공용 컴포넌트 경유로 출고·반품·폐기 현황에도 조회 검정 ·
  초기화 · 엑셀 저장이 함께 들어갔다(제품 저장소 e2fc55e).
- **남은 것** — 엑셀은 **현재 로드된 페이지 행**만 내보낸다(참조 구현 재고현황과 동일). 전체 결과
  내보내기가 필요하면 화면별 전용 export 라우트(`resolve_export_columns`)로 승격한다.

### DEC-239 — 전표 입력 화면(신규 입고 접수·신규 출고 주문): 골격 1벌 + 라인 편집기 1벌, 축 데이터로 분기 (2026-09-06)

- **배경** — 사용자: "신규입고접수, 신규출고주문 화면이 거의 동일한 기능인데 화면 스타일이 너무 다르다."
  원인은 취향이 아니라 **입고 신규가 1차 단순화 상태에서 멈춰 있었던 것**(파일 머리말: "OrderLineGrid 를
  복제하지 않고 텍스트 표를 직접 보유 … 향후 InboundLineGrid 로 분리 가능"). 그 뒤 InboundLineGrid 가
  생겼지만 상세·팝업만 쓰고 신규 화면은 620줄 인라인 `<table>` 그대로였다. 결과 라인 편집기 4벌
  (OrderLineGrid 566 · ReturnLineGrid 607 · InboundLineGrid 307 · 입고 신규 인라인) — 셀 규격 두 계열
  (px-2 py-1 / px-4 py-3), 컬럼 설정·합계·비율 자동·재고 참조 유무가 제각각, `DraftLine` 타입 4곳 복사.
- **결정 (사용자 확정 1안)** — 현황 화면(`TransactionStatusScreen` 5축)과 같은 방식으로 통합.
  1. 골격 `components/transactions/slip-entry-layout.tsx` — 띠(제목·「목록」·헤더 폼·저장 검정) →
     오류/저장 배너(「전표번호 N 저장 · 이어서 입력」+ 보기 링크 + 축 고유 후속: 출고 「바로출고」) →
     「라인」 카드 → 참조 패널 → 부가(메모). 두 화면은 슬롯 내용만 다르다.
  2. 라인 편집기 `SlipLineGrid`(= 종전 OrderLineGrid 를 in-place 일반화, 파일 유지) + **축 데이터**
     `SlipLineAxis` { 구분 목록, 컬럼 순서, 비율 라벨, 도서 확정 후 이동 칸, legacy-id, prefs 키 }.
     `OrderLineGrid` 는 출고 축 고정 얇은 래퍼(주문 상세·편집 팝업·DEC-209 가드 호환).
     입고 고유 동작은 코드 분기가 아니라 축 값·부모 콜백으로만: 구분 8종(Subu22 DBGrid101Enter),
     입고처 기본 비율·구분(`loadVendorDefaults` → `defaultPubun` + 모든 구분→같은 비율 맵, 출고 지사
     비율과 같은 꼴), 도서 확정 → 수량(출고는 공급율), Enter 리듬 입고처→구분→도서→수량.
  3. **어휘** — 일자 라벨은 두 화면 모두 「거래 일자」(띄어쓴 표기, 사용자 확정). 상대명은 입력칸 아래
     「이름(코드)」 보조 텍스트로 통일(입고의 별도 읽기전용 Input 제거, Edit23 id 는 보조 텍스트에 보존).
  4. **기능 승계** — DEC-218 가로 스크롤 힌트(HScrollBox)를 인라인 표에서 공용 그리드로 옮겨 출고도
     얻는다. 입고 신규는 합계 행·컬럼 설정·재고 참조(customerPreview gubun=입고; 입고처는 G1 거래처가
     아니라 프로필은 비고 재고·메모만)를 새로 얻는다.
- **가드** — 라인 편집기 4벌 중 2벌 통합(반품 `ReturnLineGrid`·입고 상세 `InboundLineGrid` 는 후속).
  허브 정적 가드 6종은 검사 위치를 페이지 → 축 데이터/그리드/골격으로 옮겨 갱신
  (`test_inbound_new_receipt_grid_keys.py` 전면 재작성, dec200/dec209/dec218/menu_opens/shipment_lookup).
  dfm 위젯 id 0 누락. 제품 저장소 1fa7905.
...
### DEC-242 — 「출판사관리(설정)」 메뉴 전 계정 감춤: 저장 플래그를 웹이 읽지 않는다 (2026-09-06)

- **배경** — 사용자: "이 메뉴가 필요한가? 사용하는 메뉴인가?" → 조사 결과 보고 후 "감춰주세요".
- **조사** — Sobo48 = `G7_Ggeo`(출판사 마스터)의 `Chek3`(SET재고적용 Y/N)·`Scode`(출고정지유무) 부분 저장 화면.
  - 레거시: 빌드 매트릭스상 **D-KBT(한국도서유통)·WH-MS(MS북스) 전용**. `Chek3` 소비처는 통합 거래처원장
    `Subu32_1.pas` 한 곳(`'True'` → `G4_Book.Grat8` 로 SET 재고), `Scode`(출고정지)는 **읽는 폼 없음**.
  - 웹: 통합 거래처원장 포팅은 `Chek3` 미적용, 출고 접수·거래명세서는 출고정지 미검사. `G7_Ggeo.Chek3` 를
    읽는 유일한 곳은 직전거래가 정책(`sales_statement_create_service.load_price_reuse_config`)인데 **회사
    자기 행**을 `'grat1'/'grat2'` 어휘로 읽어 이 화면의 `Y/N` 과 무관. → **이 화면에서 무엇을 저장해도 웹
    동작은 바뀌지 않는다.**
  - 활동: 앞선 35 테넌트 DB 조사에서 이 계정의 `G7_Ggeo` 접근 0건. 플래그 컬럼엔 날짜가 없어 최근 변경은
    DB 로 판정 불가.
- **결정** — 제작명세서/제작현황(DEC-2xx 감춤)과 같은 방식으로 `hiddenFromMenu` + `hiddenReason`. 라우트
...
### DEC-244 — 사이드바 플라이아웃은 뷰포트 안에 맞춘다 (긴 하위 메뉴 잘림 해소) (2026-09-06)

- **배경** — 사용자 스크린샷: 발송비/입금처럼 아래쪽 대메뉴의 플라이아웃(DEC-199)이 행 top 에 고정으로 열려
  하위 항목이 화면 아래로 잘리고 클릭할 수 없었다. 높이 제한도 없었다.
- **결정** — 열릴 때(`useLayoutEffect`) 목록 높이를 재서 ① 뷰포트에 들어가면 top 을 위로 끌어올려 전부 보이게,
  ② 그래도 길면 `max-height = 뷰포트 − 16px` + 세로 스크롤(`overscroll-contain`). 창 크기 변경 시 재계산.
  메뉴 순서·구성·닫힘 규약(바깥 클릭·Esc·blur)은 불변. 제품 저장소 1252f3e.

### DEC-245 — 내정보 화면을 4개 구역으로 나눠 왼쪽 탭으로 접근 (2026-09-06)

- **배경** — 사용자: "내정보 화면이 너무 복잡하다. 정리하여 메뉴별 구분 및 접근이 가능하도록." 카드 7개가
  한 줄로 이어졌고, 저장/업로드 결과 메시지는 맨 아래 JSON 카드에만 떠서 조작한 자리에서 보이지 않았다.
- **결정** — 구역 4개: **계정**(로그인 ID·Hcode·접속 서버·계정 유형) · **화면**(테마+스와치, 위치 안내 초기화) ·
  **거래명세서 인쇄**(대표 로고 → 공급자·계좌 → 삼련 도장, 자동출력 링크) · **고급**(커스터마이징 JSON —
  일반 사용엔 불필요함을 명시). 왼쪽 세로 탭(모바일은 상단 가로, `components/ui/tabs`), `?tab=print` 딥링크,
  결과 메시지는 활성 구역 아래 `role="status"` 상태줄. 기능·API·상태·핸들러는 그대로.
  가드(테마 스와치 짝 토큰·위치 권한 저장값·prefs DB 영속)는 문자열 위치 불변으로 통과. 제품 저장소 653309b.

### DEC-246 — 테마 재정비: 기본 톤 토큰까지 테마가 덮게 하고 다크 5종 추가 (2026-09-06)

- **배경** — 사용자: "기본 UI/UX 톤엔매너가 수정되어 다시 검토하여 다양하게 선택할 수 있도록."
  북이오 기본 톤(라임 CTA·활성 메뉴·표 머리·탭 바)이 도입되며 `:root` 토큰이 56개로 늘었는데,
  기존 테마 10종은 **27개만 덮고** 있었다. 결과: 어떤 테마를 골라도 CTA 버튼·활성 메뉴·활성 탭은 라임,
  표 머리·탭 바·선택 행은 기본 회색 → 「초록 테마인데 버튼만 라임」 같은 어긋남.
- **결정**
  1. 10종 전부에 CTA·크롬 토큰 추가: `vivid-lime/-hover/-sky`, `nav-active-foreground`, `sidebar-header`,
     `tabbar`, `table-head`, `row-selected`, `row-focus`, `chart-5`, `brand-warm(+fg)`, `accent-red`.
     CTA 는 각 테마 강조색을 밝게 파생해 **본문색이 AA(4.5:1)** 로 얹히도록 계산.
     `--nav-active: var(--vivid-lime)`·`--tab-active: var(--nav-active)` 체인 덕에 활성 메뉴·탭은 자동 추종.
     **상태색(`--status-*`)과 차트 1~4 는 의미색이라 테마와 무관하게 공통**(초록=정상, 주황=경고…).
  2. **다크 5종 신설** — 북이오 다크·미드나이트 블루·카본·딥 포레스트·플럼 더스크. 종전 10종은 전부
     "밝은 캔버스 + 어두운 사이드바" 한 결이라 어두운 화면 선택지가 없었다.
  3. CTA 버튼(`brand-primary`) 전경을 `text-foreground` → `text-[var(--nav-active-foreground)]`.
     기본 테마에선 같은 색(#282828)이라 변화 0, 다른 테마에서 글자가 파묻히지 않는다.
     (종전 CTA 는 전경이 짝 토큰이 아니라 테마 CTA 색을 바꾸면 읽을 수 없게 되는 구조였다.)
  4. 내정보 「화면」 탭의 select+범용 스와치 → **테마 카드 그리드**(밝은/어두운 그룹, `radiogroup`).
     카드 **안쪽에만** `data-theme` 을 걸어 그 테마의 실제 색을 그대로 보여준다(문서 전체 적용 아님).
  5. 미리보기용 `[data-theme="default"]` 블록 추가 — 실제 기본은 `:root` 이고 `applyTheme("default")` 는
...
### DEC-248 — 테마 카드 미리보기가 전부 같은 CTA 색으로 보이던 문제: 체인 토큰을 테마가 직접 선언 (2026-09-06)

- **증상** — 사용자: "테마를 선택하면 테마 컬러 자체도 변경되어 테마선택에 문제가 발생된다."
  내정보 테마 카드의 CTA 띠가 카드마다 다른 게 아니라 **현재 적용 중인 테마 색으로 전부 같아져서**
  어떤 테마인지 구분할 수 없었다(기본 상태에선 16장 모두 라임).
- **원인** — `:root` 의 `--nav-active: var(--vivid-lime)` 은 **선언된 요소에서 var() 가 치환**된 뒤 그
  결과값이 상속된다(CSS 사양). 테마 카드는 미리보기를 **하위 요소**에 `data-theme` 으로 그리는데,
  하위에서 `--vivid-lime` 을 재정의해도 `--nav-active` 는 재계산되지 않는다. 실제 적용(:root 에
  data-theme)은 같은 요소라 정상이었다 — **미리보기 전용 함정**.
- **결정** — 16개 테마 블록이 `--nav-active`·`--tab-active`·`--tab-active-foreground` 를 **직접 선언**해
  자립시킨다. `:root` 의 체인 선언은 그대로 둔다(DEC-199 가드). 가드 추가:
  `test_theme_contrast_guard.test_themes_declare_chained_tokens_directly`.
- **일반 규칙** — 하위 요소에 테마를 걸어 미리보기를 그릴 때는 `var()`/`color-mix` 체인 토큰이 따라오지
  않는다. 새 체인 토큰을 `:root` 에 추가하면 테마 블록에도 직접 선언을 넣어야 한다. 제품 저장소 5555513.

### DEC-249 — 자동출력 모니터: 별도 창(팝업) + 헤더 상시 접근 (2026-09-06)

- **배경** — 사용자: "팝업이나 사용자가 접근하기 편한 방식으로 변경이 필요하다. 현재는 해당 링크 또한
  쉽게 접근하기 어렵다." 진입점이 내정보 > 거래명세서 인쇄 안쪽의 작은 텍스트 링크 하나뿐이었다.
- **성격** — 이 모니터는 클라우드 백엔드가 현장 프린터에 직결할 수 없어(OQ-002) **브라우저 창이 프린터
  드라이버**다(DEC-111). 워크스페이스 탭이 아니라 **떠 있는 별도 창**이어야 다른 화면으로 옮겨도 인쇄가
  끊기지 않는다.
- **결정** — `lib/auto-print-window.ts`:
  1. `openAutoPrintMonitor()` — **이름 붙인 창**(`bls-auto-print-monitor`)으로 열어 여러 번 눌러도 창이
     늘지 않고 기존 창에 포커스. `?embed=1`(560×760)로 사이드바·헤더 없이 모니터만. 팝업 차단 시 새 탭 폴백.
  2. **헤더 「자동출력」 버튼** — 자동출력 ON 계정에만 노출, 어느 화면에서든 한 번에 열기/이동.
     ON/OFF 정본은 계정 설정이고 노출 판단용으로만 localStorage 캐시 + 같은 탭 `CustomEvent`·다른 탭
     `storage` 구독으로 즉시 반영(`storage` 는 다른 탭에만 오므로 둘 다 필요).
  3. 내정보의 묻힌 링크 → 안내 문구가 붙은 버튼 블록("창을 닫지 마세요").
  4. `ThemeSync` 가 프로필 로드 시 플래그를 캐시 — 이미 전 화면에서 `/me/profile` 을 읽으므로 추가 요청 0.
- **가드** — `test_dec249_auto_print_access`(이름 붙인 창·embed·폴백·같은 탭 통지·헤더/내정보 진입점).
  제품 저장소 5e14925.

### DEC-250 — 로그인 화면 목업 반영: 단일 카드 + 「오른쪽 위 모서리만 둥근」 모티프, 공지는 소개 메인화면으로 (2026-09-07)

- **배경** — 사용자 목업 제공("100% 동일하게") + "게시판은 일단 로그인 이전 소개 메인화면으로 이동".
  이어서 "비율을 확인해줘. 지금 화면 스크롤이 발생한다"(설계 뷰포트 **1440×1024**).
- **치수 환산(중요)** — 제공된 목업 캡처는 **1482×1054 = 1440×1024 의 1.0292배**다. 픽셀을 그대로
  옮기면 화면이 3% 커지고 세로가 넘친다. **÷1.0292** 로 환산해 적용:
  카드 603×604 · radius `0 44px 0 0` · 내용폭 483(좌우 60) · 입력 h49 r13 · 로그인 h48 · 전환 h52 · 로고 h27.
  검증(1024 높이 모사): 카드 top 142·h 604, 아이디 283, 비번 378, 로그인 635, 전환 바닥 908
  (목업 141/604/282/374/635/908 — 최대 4px).
- **스크롤 해소** — 카드를 `flex-1 + max-h-[604px] + min-h-[420px]` 로 둔다. 창이 낮으면 **카드 안 여백만
  줄고 페이지는 스크롤되지 않는다**(806px 뷰포트에서 문서 높이 = 뷰포트). 고정 높이로 두면 창이 조금만
  낮아도 바로 스크롤이 생긴다.
- **모티프** — 「오른쪽 위 모서리만 크게 둥근」 사각형(카드 44px, 입력·버튼 13px). 색은 전부 토큰:
  캔버스·입력 `muted`(Subtle Gray), 카드 흰색, 로그인 `foreground`(Bukio Black), 전환 `input`.
  TSX 하드코딩 hex 0(주석 포함 — 가드가 주석도 잡는다).
- **기능 보존** — 회사 선택 콤보·소속 선택(ORG_SELECT_REQUIRED)·모호 라우팅 안내·계정 안내(ACCT_*)·
  전환 완료 배너·에러는 카드 안(목업의 넓은 여백 자리)에 그대로 둔다. 콤보는 레거시 ID 로그인이 켜져
  있고 아이디에 `@` 가 없을 때만 보이므로 **목업이 그린 이메일 전용 상태에서는 나타나지 않는다**.
  링크는 목업대로 「비밀번호 찾기 / 신규가입 문의하기」 둘만.
- **게시판 이동** — 좌측 공지 패널이 사라지면서 공지 fetch·표시를 `components/marketing/LandingNoticeBoard`
  (클라이언트)로 옮겨 소개 메인화면에 배치. backoff 재시도와 환경별 실패 안내(dev `uvicorn` /
  prod `BLS_API_PROXY_TARGET`)도 승계. 로그인에서 빠진 「공지」·「내 계정 찾기」 진입점은 이 게시판에 둔다.
  가드 `test_login_portal_retry` 는 fetch 가 옮겨간 파일을 보도록 경로만 갱신. 제품 저장소 d2da9e4.

### DEC-251 — 계정 전환 화면 목업 반영: 로그인과 같은 카드 껍데기 + 단계 점 (2026-09-07)

- **배경** — 사용자 목업 2장(1단계 기존 계정 확인 / 2단계 이메일 입력). 로그인(DEC-250)과 같은 결.
- **공통화** — `components/account/AuthCardShell.tsx` 신설. 로그인 전 화면의 조판을 한 곳에 모은다:
  캔버스 `muted` · 좌상단 로고 h27 · 카드 603폭 `max-h-[604px]` flex(창이 낮으면 카드 여백만 줄고 페이지는
  스크롤되지 않음) · **오른쪽 위 모서리만 44px** · 카드 아래 보조 CTA(`AuthCardBelow`) · 단계 점(`AuthStepDots`,
  활성 = 브랜드 라임). 치수는 목업 캡처가 설계 뷰포트의 1.03배인 점을 환산한 값(DEC-250 과 동일 기준).
- **카드 상단** — 뒤로(‹) · 「계정 전환하기」 · 닫기(✕) + 4단계 점. 뒤로는 1단계 `/login`, **2단계에서는
  `restart()`**(티켓·확인 결과까지 초기화 — 다른 계정으로 갈아타는 경로), 그 외 이전 단계.
  종전 3칸 단계 칩(`<ol>`)과 레거시 요약 카드는 제거.
- **단계별** — 1단계: 사용자(회사 콤보를 목업 문구로) · 아이디 · 비밀번호(**눈 토글 신설**) · 「다음 ›」.
  2단계: 확인된 계정을 **라임 알약**(「… 님, 환영합니다.」)으로 보여주고 라벨 없는 이메일 입력 + 「인증번호 발송」.
  3·4단계도 같은 입력·버튼 조판. 카드 아래 「기존 정보가 기억나지 않습니다.」 + 회색 「문의하기」(→ `/signup`).
- **불변** — ACM-INV-2 델파이 병행 안내 문구 유지, 검증 API 계약(`verify-legacy`) 불변. 첫 필드는 자유
  입력이 아니라 **회사 콤보**다(계약이 라우팅 힌트를 요구하고, 비워 두면 자동 결정). 하드코딩 hex 0.
- **미검증** — 2단계 라임 알약은 유효한 레거시 자격이 있어야 도달해 화면 확인은 하지 못했다.
  제품 저장소 cd7521a.

### DEC-252 — 로그인 전 화면 공통 알림 팝업(AuthAlertDialog) (2026-09-07)

- **배경** — 사용자 목업(「계정 전환하기2.5」): "암호 오류시 팝업창 스타일입니다. 100% 동일하게."
- **규격(설계 뷰포트 1440 환산)** — 상자 **401×200**, **오른쪽 위 모서리만 24px**, 좌하단 말풍선 꼬리
  20×20(clip-path 삼각형), 좌/우 여백 41/26, 제목 top 42, 본문 top 77, 「확인」 101×37 가운데, 딤 **검정 50%**.
  접근성: `role="alertdialog"`·`aria-modal`, 열리면 「확인」 포커스, Esc·딤 클릭·X 로 닫힘.
- **적용 범위** — 한 문장으로 끝나는 알림만: 계정 전환 1단계 자격 오류, 인증번호 발송 완료(목업 문구 그대로),
  로그인 화면의 자격 오류(인라인 빨간 배너 제거).
  **팝업을 쓰지 않는 곳**: 계정 안내(`ACCT_*`)·소속 선택·이미 전환됨 — 링크와 선택지가 붙어 조작이
  필요하므로 카드 안 인라인으로 남긴다. 제품 저장소 a9b28fe.

### DEC-253 — 계정 전환 3단계(인증번호 입력) 목업 반영 (2026-09-07)

- 안내 문구 + **코드 입력 한 줄**(입력칸 + 오른쪽 만료 카운트다운 칸 101×49, mm:ss, `text-link`).
  두 칸 사이 2px 흰 틈, 오른쪽 칸만 top-right 13px — 행 전체가 카드 모티프를 따른다.
  카운트다운은 `sentAt + expiresInMin` 에서 계산해 재발송 틱과 같은 주기로 갱신(`role="timer"`).
- 「인증번호 재발송」은 알약(rounded-full 31px) 가운데. 60초 쿨다운 표시 유지.
- 주 버튼 라벨 → 목업의 「확인」. 종전 메일 안내 카드(마스킹 이메일)는 목업에 없어 제거,
  개발용 `debugCode` 는 dev 분기로 유지.
- **보류** — 새 비밀번호 입력은 목업에 없지만 전환 완료에 필수라 코드 아래에 같은 조판으로 남겼다.
  목업의 4번째 점이 이 단계일 수 있어 **별도 화면 분리는 해당 목업을 받은 뒤**로 미룬다. 제품 저장소 b601e93.

### DEC-254 — 계정 전환 4단계(비밀번호 설정) 분리 (2026-09-07)

- **배경** — 목업 「계정 전환하기4」. DEC-253 에서 "4번째 점이 비밀번호 설정일 수 있어 분리는 목업을 받은 뒤"로
  미뤄 뒀던 것이 확정됐다.
- **결정** — `Step` 1|2|3|4 → **1|2|3|4|5**(4 = 비밀번호 설정, 5 = 완료).
  - 3단계는 인증번호만. 「확인」은 비밀번호가 필요하면 4단계로, `link`/`relink` 모드(비번 불필요)는 종전대로
    바로 완료(코드 6자리 자동 제출도 이 모드에서만 유지).
  - 4단계: 인증된 이메일 **라임 알약** + 2줄 안내 + 비밀번호/확인(각각 눈 토글) + 「완료」. 목업 문구 그대로.
    비밀번호 조건 실시간 표시(`PasswordRules`)는 유지 — 정책이 화면마다 갈리지 않게.
- **코드 오류 처리** — API 는 코드와 비밀번호를 **한 번에** 검증한다(`complete`). 그래서 코드가 틀리면
  4단계에서 오류가 나는데 거기엔 고칠 입력이 없다. `ACCT_CODE_INVALID` 는 **3단계로 되돌리고** 팝업으로
  알린다. 제품 저장소 0b2fd15.

### DEC-255 — 계정 전환 완료 화면 목업 반영 (2026-09-07)

- 완료(5단계)는 **되돌아갈 곳이 없는 종착 화면**이라 카드 상단의 뒤로(‹)·닫기(✕)와 단계 점을 감추고
  제목만 남긴다(목업 그대로).
- 안내 2줄 「… 님의 북이오웍스 계정 전환이 완료되었습니다. / 로그인 후 이용해 주세요.」 +
  주 버튼 「로그인 하러 가기」 → `/login?switched=1`(로그인 화면의 전환 완료 배너로 이어짐).
- 종전 체크 아이콘·이메일 표기는 목업에 없어 제거. **서버 완료 메시지(`doneMessage`)는 전환/연결 구분과
  델파이 병행 안내(ACM-INV-2)를 함께 담으므로 보조 줄로 유지**한다 — 정적 문구로 대체하면 연결(link)
  케이스의 안내가 사라진다. 제품 저장소 8fc2edf.

이로써 계정 전환 흐름 5개 화면(DEC-251·253·254·255)과 공통 팝업(DEC-252)이 목업과 맞춰졌다.

### DEC-256 — 로그인 화면 입력 검증 규칙 (2026-09-07)

- **배경** — 사용자 목업 「오류 메시지 유형」 4개 규칙. "꼭 필요한 부분에 대해서 처리해줘".
- **결정** (`app/(public)/login/page.tsx`)
  1. **이메일 형식** — `@` 가 들어올 때까지 아이디 칸 아래 「이메일 주소 형식에 맞지 않습니다」.
  2. **비밀번호 8자** — 8자를 채우면 사라진다. 웹 계정 정책(`passwordPolicyOk`)과 같은 기준.
  3. **로그인 실패** — 비밀번호 칸 아래 **인라인** 한 문장
     「등록되지 않은 아이디이거나 비밀번호를 잘못 입력하셨습니다」. 아이디/비밀번호를 구분해 알리지
     않는 것이 계정 열거 방지와도 맞는다. 401 만 이 문구로 덮고 나머지 오류는 서버 문구를 그대로 쓴다.
  4. 조건 충족 전 「로그인」 버튼 비활성(`canSubmit`).
- **규칙 1·2·4 는 `emailMode` 에서만 건다** — 레거시 델파이 비밀번호(`Gpass`)에는 길이 정책이 없어,
  break-glass 로 `BLS_LEGACY_ID_LOGIN` 을 켠 동안 짧은 레거시 비밀번호를 막아 버리면 안 된다.
  이메일 전용 운영(ACM-DEC-06)에서는 항상 켜진 것과 같다.
- 로그인 화면에 한해 **DEC-252 팝업을 쓰지 않는다** — 목업이 인라인으로 못박았고, 고칠 입력 바로
  아래 오류가 붙는 편이 조작에 가깝다. 팝업은 계정 전환 쪽에 남는다.
- 가드: `test/test_dec256_257_auth_screen_rules.py`.

### DEC-257 — 계정 전환하기 1단계 입력 검증 규칙 (2026-09-07)

- **배경** — 사용자 목업 「오류 메시지 유형」(계정전환하기). "꼭 적용해야할 부분 적용하기".
- **결정** (`components/account/SwitchWizard.tsx`)
  1. **모든 입력 필드가 채워질 때까지 「다음」 비활성**(`canVerify`) — 사용자(회사) 콤보도 입력 필드로
     본다. 다만 공개 콤보 API 가 실패해 옵션이 0개면 고를 수 없는 값을 요구하게 되므로 그때만 면제
     (`orgRequired = orgOptions.length > 0`). 버튼이 비활성이어도 Enter 제출은 남아 있어 `verify()`
     안에서 한 번 더 막는다.
  2. **일치하는 계정이 없으면 팝업**(DEC-252) 「입력하신 정보와 일치하는 계정이 없습니다」.
     `ACCT_LEGACY_INVALID`(401) 만 이 문구로 덮는다 — 소속 선택(`ACCT_ORG_HINT_REQUIRED`)·이미 전환됨
     (`ACCT_ALREADY_SWITCHED`)처럼 **조작이 필요한 안내는 종전대로 카드 안 인라인**이라야 링크를 누를 수 있다.
- 콤보가 필수가 되면서 첫 옵션 문구의 「(모르면 자동)」는 옵션이 없을 때만 남긴다.
  회사를 고르면 확인이 최대 1분에서 크게 줄어드는 부수 효과도 있다.
- 화면 검증(로컬 dev + Chrome): 세 칸을 다 채워야 「다음」이 켜지고 하나를 비우면 다시 꺼짐,
  없는 아이디로 제출 시 목업 문구 팝업 확인.
- 가드: `test/test_dec256_257_auth_screen_rules.py`.

### DEC-258 — 계정 전환하기 2단계(이메일 입력) 검증 규칙 (2026-09-07)

- **배경** — 사용자 목업 「오류 메시지 유형」(계정전환하기 2단계). "필수적으로 적용할 내용을 반영".
- **결정** (`components/account/SwitchWizard.tsx`, 2단계)
  1. **형식 오류 인라인** — `@` 가 나올 때까지 입력칸 바로 아래 「이메일 주소 형식에 맞지 않습니다」
     (`text-xs text-destructive`, `role="alert"`, `aria-invalid`/`aria-describedby` 연결).
     로그인 규칙 1(DEC-256)과 같은 기준(`includes("@")`)이라 두 화면 문구·시점이 어긋나지 않는다.
  2. **팝업 「아이디 입력 오류 / 이미 사용 중인 이메일 주소입니다.」** — 서버가 주소 점유로 거절할 때
     (`ACCT_EMAIL_TAKEN`/409). 목업(2026-09-07 6:21)이 이 건만 팝업으로 못박았다 — 형식 오류(규칙 1)는
     고쳐 쓸 입력이 바로 위에 있어 인라인, 점유는 **다른 주소를 써야 하는 판단**이라 흐름을 끊는다.
     **다만 현재 백엔드는 이 거절을 하지 않는다** — 아래 보류 참조.
  3. **유효한 주소가 되기 전까지 「인증번호 발송」 비활성**(`emailValid`). 버튼이 꺼져 있어도 Enter
     제출이 남아 있어 `sendCode()` 안에서 한 번 더 막는다.
- 2단계 오류는 **팝업이 아니라 인라인** — 고칠 입력이 같은 화면에 있어 DEC-252 팝업 기준(한 문장으로
  끝나고 조작이 필요 없는 알림)에 해당하지 않는다.
- **보류 — 목업 규칙 2와 기존 설계의 충돌.** 지금은 이미 등록된 이메일이 오류가 아니라 **연결
  (`link`/`relink`) 모드**다: 한 사람이 여러 소속을 이메일 하나에 붙이는 흐름으로, 1단계 안내
  「다른 소속은 나중에 같은 절차로 추가 연결할 수 있습니다」와 DEC-254 의 "연결 모드는 비밀번호 설정
  건너뜀"이 여기에 기대고 있다. 목업대로 **하드 차단하려면 이 기능을 없애야 하므로** 문구·표시
  자리만 먼저 배선하고 차단 여부는 사용자 확인 뒤로 미룬다. 차단으로 정하면
  `account_switch_service.send_code` 에서 `purpose == switch and account` 일 때
  `SwitchError(409, "ACCT_EMAIL_TAKEN", …)` 을 올리는 것만으로 이 화면은 그대로 동작한다.
  (이미 전환된 계정을 다시 전환하는 진짜 중복은 1단계 `ACCT_ALREADY_SWITCHED` 가 이미 막는다.)
- 화면 검증(로컬 dev + Chrome): `gyo` → 오류 표시·버튼 꺼짐, `gyo@`·`gyo@buk` → 오류 사라지되 버튼 꺼짐,
  `gyo@buk.io` → 버튼 켜짐.
- 가드: `test/test_dec256_257_auth_screen_rules.py::TestSwitchEmailStepRules`.

**DEC-257 보강 (2026-09-07)** — 「계정 찾기 오류」 팝업 목업 확정. 자격 불일치 팝업은 제목
**「계정 찾기 오류」** + 본문 **「입력하신 정보와 일치하는 계정이 없습니다.」** 두 줄로 띄운다
(종전에는 제목 한 줄뿐이었다). `AuthAlertDialog` 의 `title`/`description` 조판(DEC-252)을 그대로 쓴다.

### DEC-259 — 계정 전환하기 3단계(인증번호) 오류 · 공통 토스트 (2026-09-07)

- **배경** — 사용자 목업 「오류 메시지 유형」(3단계). "인증시간 초과시 **토스트 팝업**으로".
- **공통 토스트 도입** (`AuthCardShell.tsx`) — 로그인 전 화면 공통. `useAuthToast()` 컨텍스트로
  카드 안 어디서나 띄운다. 목업 실측(설계 1440 기준): 높이 49 · 좌우 여백 30 · 시안 띠 ·
  오른쪽 위로 삐져나온 28×24 닫기(오른쪽 아래 꼬리). **카드 위 71px 여백에 absolute 로 얹어**
  토스트가 떠도 카드가 밀리지 않는다. 6초 뒤 자동 소멸 + 닫기 버튼.
- **팝업(DEC-252)과의 구분** — 고칠 입력이 화면에 그대로 있고 흐름을 끊을 이유가 없는 알림은 **토스트**,
  다음 행동을 정해야 하는 알림(계정 찾기 실패·이메일 점유)은 **팝업**.
- **색** — 띠는 `color-mix(in srgb, var(--vivid-sky) 70%, white)` 로 **불투명 고정**. 반투명으로 두면
  다크 테마에서 바탕이 비쳐 띠가 어두워지고 글자 대비가 무너진다. 같은 이유로 글자·닫기는
  `text-foreground` 가 아니라 브랜드 위 전경색 `--nav-active-foreground`.
- **두 문구** — 만료 「인증 시간이 초과되었습니다. 인증번호를 재발송해 주세요.」 /
  불일치 「인증번호가 일치하지 않습니다. 다시 입력해 주세요.」
  백엔드는 코드 유효성 오라클을 막으려 **만료와 불일치를 `ACCT_CODE_INVALID` 하나로 합쳐** 준다
  (`account_switch_service` 의 `generic`). 그래서 **만료 판정은 화면 카운트다운(`expiresLeft`)으로**
  가른다 — 서버 응답을 쪼개지 않고도 목업의 두 문구가 나온다. 불일치 시 입력한 코드는 비운다.
- 카운트다운이 **1분 이하로 남으면 붉게**(`text-destructive`) — 목업의 `00:01` 표기.
  만료 순간에는 3단계에 머무는 동안 **한 번만** 토스트를 띄우고, 재발송하면 다시 무장된다.
- 종전 `ACCT_CODE_INVALID` → 팝업(DEC-254)은 이 토스트로 대체. 4단계에서 코드 오류가 나면
  3단계로 되돌리는 동작은 그대로다.
...
### DEC-261 — 전환 완료(5단계) 화면 목업 실측 정합 (2026-09-07)

- **배경** — 사용자 목업(6:37). DEC-255 에서 만든 완료 화면을 실측과 맞춘다.
- **결정** (`SwitchWizard.tsx` 5단계, `AuthCardShell.tsx`)
  - **카드 아래 보조 CTA(「문의하기」)를 감춘다** — 완료는 되돌아갈 곳이 없는 종착 화면이라
    카드 밖에 다른 행동이 남아 있으면 다음 할 일이 둘로 보인다. 껍데기에 `useHideAuthCardBelow`
    컨텍스트를 두어 **화면이 바뀌면 자동으로 되살아나게** 했다(다른 단계는 그대로 「문의하기」가 보인다).
  - **여백 실측 반영** — 목업 카드는 603×603 정사각. 안내는 카드 위에서 152(`mt-[90px]`),
    버튼 위 493·아래 64(`mb-[34px]`). 브라우저 실측으로 1px 수준까지 일치 확인.
  - **보조 줄은 연결(link/relink)에서만** — 목업의 전환(new) 화면은 두 줄로 끝난다. 다만 연결은
    "무엇이 연결됐는지"가 서버 메시지에만 있어 그때만 남긴다(ACM-INV-2 델파이 병행 안내 포함).
    첫 줄도 모드에 따라 **전환/연결**로 갈라 문구가 사실과 어긋나지 않게 했다.
    → DEC-255 의 "서버 완료 메시지는 항상 보조 줄로 유지"를 이 범위로 좁힌다.
- 가드: `test/test_dec256_257_auth_screen_rules.py::TestSwitchDoneScreen`.

### DEC-262 — 마스터 코드 컬럼 폭 초과 = 조용한 잘림 → "저장은 됐는데 상세는 404" (2026-09-08)

- **배경** — 사용자 보고 ①: 기초관리 › 거래처관리 › **+신규거래처** 저장 직후 상단에
  `(404) customer not found`. "에러는 났지만 저장은 되었다." 도서(신규도서)에서도 동형으로
  `(404) book not found`.
- **원인(라이브 확인, remote_153 / 교문사 hcode=5019)**
  - `G1_Ggeo.Gcode` = **varchar(5)** (전 서버 동일. `G2_Ggwo`/`G3_Gjeo` 도 5, `G4_Book` 은 10).
  - 그런데 DEC-079 접두 채번은 `<접두><6자리>` = **7자**(`J000001`) 를 만들었다.
  - 레거시 MySQL(5.1.63, 비-strict)은 폭 초과 INSERT 를 **경고만 내고 잘라서** 저장 →
    실제 행은 `Gcode='J0000'`. 등록 응답은 요청 코드(`J000001`)를 그대로 돌려주므로
    프론트가 `/master/customer/J000001` 로 이동 → 그 코드로는 행이 없어 404.
    (증거: `SELECT * FROM G1_Ggeo WHERE Gcode='J0000'` → `Gname='교문사 테스트'`, `Gubun='J'`)
  - 채번 쿼리의 `LENGTH(Gcode)=7` 조건도 영영 매칭되지 않아 **매번 `<접두>000001`** 을 제안했고,
    중복검사(7자)도 잘린 행(5자)과 안 맞아 같은 `J0000` 이 반복 삽입될 수 있었다.
- **결정**
  1. 코드 자리수는 **실제 컬럼 폭에서 뽑는다** — `app/services/master_code_width.py`
     (`SHOW COLUMNS` → `varchar(N)`/`char(N)` 파싱, 서버·테이블별 캐시. MySQL 3.23 호환).
     접두 채번은 `<접두><폭-1자리>`(varchar(5) → `A0001`), 집계 조건도 `LENGTH(Gcode)=<폭>`.
     자리 소진·폭 미상 시엔 폭을 넘기지 말고 **빈 값(수기 입력 폴백)** 으로 물러난다.
  2. **등록 전 길이 가드** — `ensure_code_fits` 를 거래처·도서·입고처·저자·기타거래처·출판사·
     할인율 등록 경로에 건다. 초과면 INSERT 전에 `ValueError("MASTER_CODE_TOO_LONG:…")`,
     라우터(`_master_value_error_detail`)가 **422 + 한국어 메시지**로 변환한다
     (「거래처코드는 최대 5자입니다 (입력 7자: J000001).」). 잘린 행이 남지 않는다.
  3. **입력칸에서도 막는다** — `GET /masters/next-code` 응답에 `maxLength`(컬럼 폭)를 실어
     신규 화면 코드 입력칸의 `maxLength` 로 건다.
- **원칙** — 레거시 DB 는 비-strict 라 *길이 초과를 에러로 알려주지 않는다*. 사용자가 값을 넣는
  레거시 컬럼은 **쓰기 전에 폭을 확인**하거나, 최소한 등록 후 그 키로 다시 읽히는지 확인해야 한다.
- 가드: `test/test_master_code_width_guard.py`, `test/test_customer_code_prefix.py`.
- **잔여 데이터** — 사고로 생긴 `hcode=5019`, `Gcode='J0000'`(교문사 테스트) 행은 그대로 남아 있다.
  필요하면 `/master/customer/J0000` 에서 삭제.

### DEC-263 — 기초관리 신규 화면: 코드 직접 지정 · 등록 버튼 위치/명칭 통일 (2026-09-08)

- **배경** — 사용자 요청 ②③④⑥⑦(2026-09-08 피드백 문서).
- **결정**
  1. **거래처구분/입고처구분에 「직접입력」 선택지** — 고르면 자동채번을 끄고 코드칸을 비워
     사용자가 직접 코드를 넣는다(구분은 비워 저장). 코드칸을 **직접 고친 경우에도** 자동채번이
     덮어쓰지 않는다(`manualCode`). 다시 구분을 고르면 접두 채번이 되살아난다.
  2. **입고처관리(Sobo12) 배열을 거래처관리(Sobo11)와 동형으로** — 기본정보(구분/지역/코드/정지유무/
     정지사유 · 명/대표자/사업자등록번호/업태/종목/한도액 · 주소1+우편번호검색/유선전화/팩스/핸드폰/
     코드2 · 주소2) + 청구정보(비율 8칸 / 계산서·발행유무 / 담당자·연락처·비고1 / 비고2) 2단 섹션.
     라벨↔실컬럼 바인딩(DEC-172)은 그대로 두고 **배치만** 옮겼다.
  3. **도서 목록 제목** `도서(마스터)` → **`도서관리`**.
  4. **신규 등록 버튼을 제목 띠(우측 상단)로 통일** — 도서 신규/상세도 거래처와 같은 자리·모양
     (DEC-228 확장). 명칭은 거래처 「신규거래처 저장」 · 도서 「신규저장」 · 입고처 「신규입고처 저장」.
- 가드: `test/test_dec172_inbound_vendor_full.py`(바인딩 유지), 프론트 타입체크/eslint.

### DEC-264 — 전표 입력 띠: 거래일자·입고처(거래처)를 제목 옆(좌측)으로 (2026-09-08)

- **배경** — 사용자 요청 ①(2026-09-08 피드백 문서): 「입고관리 – 신규입고접수 "거래일자
  입고처코드" 위치가 너무 우측으로 치우쳐 있습니다. 좌측 상단으로 올 수 있게 이동 요청」.
  DEC-200 흰 띠는 **조회 화면 문법**(제목 좌 / 필터·조회 우)이라 전표 «입력» 화면에서도
  먼저 채우는 칸이 화면 오른쪽 끝에 붙어 시선·마우스 이동이 컸다.
- **결정** — `PageHeader` 에 `filtersAlign?: "start" | "end"`(기본 `end` = 종전 그대로).
  `"start"` 면 띠 필드는 제목 바로 옆(좌)에서 시작하고, 액션(저장 등)만 `ml-auto` 로 우측 끝.
...
- **배경** — 사용자 요청 ②(2026-09-08): 「입고현황 – 입고상세명세서 상세 "라인저장" 후 팝업이
  그대로 있습니다. 수정 저장이되면 팝업이 닫히도록 요청합니다. … 입고관리 – 입고명세서 –
  수정 완료 시에도 위와 같이 팝업이 안닫힙니다.」 이어서 **공통 규칙**으로 확정:
  「팝업 화면에서 수정이나 저장 동작을 수행한 이후에는 해당 팝업 창은 목적을 이루었으니
  동시에 자동으로 닫히도록 한다.」
- **결정** — 편집 팝업의 **성공 경로는 `onChanged?.()` → `onClose()` 로 끝난다.**
  종전의 「저장 완료 — 추가 n · 수정 n · 삭제 n」 배너 + 재조회(열린 채 유지)는 없앤다.
  저장 결과는 뒤에서 재조회되는 목록이 보여 준다.
  - `ReceiptDetailDialog`(입고현황·입고명세서·신간발행 공용) — 「라인 저장」 후 닫힘.
  - `OrderDetailDialog`(출고현황 등) — 「라인 저장」 후 닫힘. 일자 변경 경로와 동일해졌다.
    **주문 취소**(확정 동작)도 성공 시 닫는다.
  - 이미 이 규칙을 지키던 것: 거래명세서 편집/라인수정 팝업, 위젯 설정 시트, 자료불러오기,
    가입 승인, 엑셀 필드 선택.
  - **예외(의도)** — 「바로출고」는 편집 확정이 아니라 요청이고, 팝업의 안내문("자동출력 PC에서
    ≈5초 뒤 인쇄")이 유일한 피드백이라 팝업을 연 채로 둔다. `MasterImportButton` 의 결과창은
    «동작의 결과 보고»(오류 행 목록)라 자동으로 닫지 않는다.
...
### DEC-267 — 거래처구분 선택지: A~K 순서 · 접두 표시 · 구분 바꾸면 코드도 바뀐다 (2026-09-08)

- **배경** — 사용자 피드백(기초관리 – 거래처 신규등록, 참고 이미지 「신규 출판프로그램 반영 요청 건」):
  ① 「초기 작업 시 A~K 까지 순서대로 배열」 — 선택지가 등록 순서(10001…)라 스킴 순서와 달랐다.
  ② 「거래처구분과 거래처코드 매칭이 잘 되는지 확인(매칭이 안되는 부분이 있어)」.
  ③ 「거래처구분 "직접입력" 이 안 보인다」, ④ 「저장 후 에러메시지」.
- **확인 결과**
  - ② 재현됨 — 접두 스킴에 **없는** 구분(출판사·교문사 관련·X 거래종료·전자책·파프리카·저자)으로
    바꾸면 코드가 갱신되지 않고 **직전 접두 코드(A0001)가 그대로 남아** 구분과 어긋났다.
    프론트가 "접두 미매칭이면 코드칸이 비어있을 때만 채운다"(수기 입력 보존)였기 때문.
  - ③ 「직접입력 (코드 직접 지정)」은 DEC-263 으로 이미 (미지정) 바로 아래에 있다 — 사용자 화면은
    이전 배포본/오래 열어 둔 창의 낡은 번들로 보인다(현행 화면 확인 완료).
  - 접두 매핑 자체(구내서점 A … 기타거래처 K, I 제외)는 참고 이미지와 **일치**.
- **결정**
  1. `MasterGbunSelect` 는 접두 스킴이 켜진 화면(신규 거래처)에서 선택지를 **A→K 순서**로 정렬하고
     (스킴 밖 구분은 원래 순서대로 뒤에), 옵션 라벨에 접두문자를 붙인다 — 「구내서점 (A)」.
  2. 신규 거래처 화면은 자동채번 모드(`manualCode=false`)에서 **구분이 바뀌면 서버 제안 코드로 항상
     교체**한다. 수기 입력 보존은 `manualCode`(코드칸을 직접 고치거나 「직접입력」 선택) 가 맡는다.
- **자릿수 결정(2026-09-08 사용자 「기존 코드 체계 유지해주세요」 → 확장 미진행)** — 참고 이미지의 코드 예시는 `A000001`(접두+6자리=7자)인데 레거시
  `G1_Ggeo.Gcode` 는 **varchar(5)** 라 현재 채번은 `A0001`(접두+4자리, 구분당 9,999개)이다(DEC-262).
  7자로 가려면 거래처코드를 저장하는 **레거시 전 테이블·전 서버 컬럼 폭 확장 + 델파이 프로그램 병행
  검증**이 필요하다 — 별도 결정 사안. 사용자가 「7자 확장 검토」를 선택해 실측 조사를 수행했다:
  `docs/decision-customer-code-width-expansion.md`(폭·키·테넌트 DB 수·행수 실측, 위험, 단계 계획).
  요지 — `S1_Ssub` 2,375만 행 등 대형 테이블 ALTER 는 무중단 불가(야간 점검 창 필요), 테넌트 DB 50+,
  웹 코드 변경은 0(폭 자동 산출), 5자 유지 시에도 구분당 9,999개로 현 사용량(교문사 1,299개) 대비 여유.
  **조사 결과를 보고 사용자가 「기존 코드 체계 유지」로 확정** — 레거시 스키마 변경 없음, 채번은
  `<접두><4자리>`(`A0001`) 그대로. 조사 문서는 재조사 방지용 기록으로 보존.
- 가드: `test/test_customer_code_prefix.py`(GbunSelectOrderAndMatching).

### DEC-268 — 현황 화면 검색 필터를 띠 아래 줄(전폭)로 (2026-09-08)

- **배경** — 사용자 요청: 「출고 현황 화면의 검색 관련 여러 필터 입력 그룹을 아래부분으로 이동하면
  좋겠다」(캡처의 화살표: 띠 우측 끝 → 제목 아래). 필터가 거래처·도서코드·전표·시작일·종료일·
  도서구분·거래구분 **7묶음**이라 제목 오른쪽에 몰면 입력칸이 좁고(예: 거래처 w-24) 시선·마우스
  이동이 크다. DEC-266(전표 입력 줄을 카드 안으로)과 같은 방향의 후속.
- **결정** — `PageHeader` 에 `filtersBelow`(기본 false) 추가. true 면 필터 셀이 `md:col-span-2` 로
  **띠 두 번째 줄 전폭**을 차지하고 좌측에서 시작한다(액션이 있으면 그 줄 우측 끝 `ml-auto`).
  현황 공용 화면 `TransactionStatusScreen`(출고·입고·반품·폐기·신간발행 현황)이 이를 쓴다.
  - 필터는 **여전히 `PageHeader` children(띠 안)** 이다 — DEC-200 의 `.page-header` 인라인 라벨 CSS,
    DEC-233 팝오버 예외, `data-enter-scope`/`FILTER_STOP_IDS`(Enter=다음 칸→조회)가 그대로 산다.
  - 조회·초기화는 필터 뒤에 그대로 붙는다(마지막 필터→조회 근접, Enter 이동 순서와 일치).
  - 다른 조회 화면(원장·통계 등)은 기본값 유지 — 필터가 1~3묶음이라 제목 오른쪽이 낫다.
- **확대 (2026-09-09 사용자)** — 「재고현황, 청구서 관리 화면 등 여러 화면에서 조회용 필터 값 입력
  컴포넌트와 검색 버튼이 모두 오른쪽 정렬인데 **왼쪽 정렬**로, **화면이름 및 경로 라인 다음 라인**으로.
  기타 유사 케이스도 같은 규칙」 → `filtersBelow` **기본값을 true 로 승격**해 전 조회 화면에 적용.
  - 액션(엑셀 저장·엑셀 업로드·컬럼·「신규 …」 CTA)은 **제목 줄 우측 끝 유지**(DEC-228/263) —
    `data-slot="page-header-actions"` 로 제목 줄에 따로 그린다.
  - **액션만 있고 필터가 없는 화면**(도서분류·상세/신규 등)은 두 번째 줄을 만들지 않는다.
  - 화면 확인: 재고현황 · 청구서관리 · 출고/입고 현황 · 거래처현황(액션 있는 목록) · 도서분류(액션만).
  - 되돌릴 화면이 생기면 `filtersBelow={false}` — 다만 그런 화면이 없어야 한다(가드가 잡는다).
- 가드: `test/test_dec268_status_filter_row.py`, `test/test_dec200_page_header_band.py`.

### DEC-269 — 입고처 등록/상세를 거래처와 «같은 배열»로 · G2_Ggwo_Ext 사이드 테이블 (2026-09-09)

- **배경** — 사용자 요청(캡처 2장 비교): 「신규입고처 등록시 입력 컴포넌트 위치 및 배열을 첨부한
  거래처 기본정보 및 청구정보 배열과 동일하게 맞춰주세요.」 DEC-263 으로 큰 틀은 맞췄지만
  거래처에만 있는 칸(상세주소1/2 · 주소2 우편번호 · 연락처2 세트 · 담당관리자2/연락처2 · 메모)이
  빠져 3·4행과 청구정보 하단이 어긋나 있었다.
- **확인** — `G2_Ggwo` 는 `G1_Ggeo` 와 **컬럼 47개가 완전히 동일**하다(실측). 빠진 칸은 레거시
  컬럼 부족이 아니라 «거래처가 확장 테이블 `G1_Ggeo_Ext`(DEC-230)에 담고 있던 항목»이었다.
- **결정**
  1. **`G2_Ggwo_Ext` 사이드 테이블 신설** — `app/services/inbound_vendor_ext_service.py`.
     `G1_Ggeo_Ext`(DEC-230)·`G4_Book_Ebook`(DEC-068) 선례 그대로 **레거시 스키마 무변경**,
     `CREATE TABLE IF NOT EXISTS` 로 첫 사용 시 자동 생성(실패해도 빈값으로 화면은 뜬다),
     (Hcode, Gcode) PK, 모든 조회/쓰기 hcode 스코프. 델파이는 이 테이블을 모른다(병행 무영향).
     컬럼: Zip2 · Add1Detail · Add2Detail · Tel2 · Fax2 · Phon2 · Email2 · **Manager2 · Contact2** · Memo.
     거래처와 달리 `StopReason` 이 없다 — 입고처는 정지사유가 레거시 `Email` 컬럼(DEC-172).
     담당자 연락처도 레거시 `Gnum1`(Edit128) 이라 `Contact1` 대신 담당관리자2용 두 칸을 둔다.
  2. **폼 배열을 거래처와 1:1** — 기본정보 3행 「주소1(우편번호·검색·주소+상세주소)/유선전화/팩스번호/
     핸드폰번호/입고처코드2」, 4행 「주소2(동형)+유선전화2/팩스2/휴대전화2/이메일2」.
     청구정보 「담당관리자1·연락처·비고1 / 담당관리자2·연락처·비고2 / 메모」, 계산서 행도 같은 3열.
     - 남는 차이(의도) — 비율 줄은 입고처가 8칸(위탁~기타·**한도(율)**·신간수량, 레거시 Panel118),
       거래처는 7칸(한도는 기본정보). 계산서는 Sobo12 에 구분(Pubun) 입력이 없어 「계산서 거래처명」.
       거래처의 「이메일」 자리는 입고처에서 「입고처코드2」(Ocode) 가 쓴다.
  3. **DEC-172 바인딩은 그대로** — 담당자=Gpper · 정지사유=Email · 계산서 거래처명=Name2 ·
     한도(율)=Grat7 · 한도액=Gssum · 비고2=Name1 · 핸드폰번호=Gphon. 라벨만 거래처와 같은
     「담당관리자1」로 통일했다(값 바인딩 무변경).
- 확인 — 신규/상세 화면 실물 확인, `G2_Ggwo_Ext` 가 chul_09_db 에 12컬럼으로 자동 생성됨(행 0).
- 가드: `test/test_dec269_inbound_vendor_layout_parity.py`(확장 서비스 병합·hcode 격리·라우터 배선·
  모델 필드·폼 행 구성·DEC-172 바인딩), `test/test_dec172_inbound_vendor_full.py`(라벨↔컬럼).

### DEC-270 — 공용 코드 채번은 «기존 코드 체계»를 잇는다 (자리수는 실데이터에서, 2026-09-09)

- **배경** — 사용자 보고: 「도서재고를 맞추는 중, 기존 코드가 잘못 변경된 부분을 확인했다. 원래
  요청은 기존 코드는 그대로 두고 **추가되는 코드에만** 적용하는 것이었다.」 이어서 「도서코드가
  기존 코드가 아닌 다른 코드가 부여되어 있다」.
- **1차 확인 — 기존 코드는 바뀌지 않았다** (고객 실자료 재다운로드 대조로 최종 확정: 1팀·2팀 모두 동일)
  - `UPDATE G4_Book` 의 SET 대상에서 `Gcode` 는 읽기전용(`g4_book_adapt._BOOK_READONLY_COLS`),
    저장 모델(`_BookWritableFields`)에도 없다. 도서 엑셀 업로드는 `update_book` 만 호출(INSERT 없음).
    상세(수정) 화면 코드칸은 `readOnlyCode = !createMode`, 저장 payload 에 `gcode` 미포함.
  - 거래처(`G1_Ggeo`)·입고처(`G2_Ggwo`) 도 동형 — PATCH 대상에 `Gcode` 없음. DEC-267 의
    「구분 바꾸면 코드 재발급」은 **신규 화면 한정**이다.
  - → 웹이 기존 마스터 코드를 고쳐 쓰는 경로는 없다. 이 사실을 고정하는 가드가 없어 신설 대상.
- **2차 확인 — 그러나 «신규 채번» 로직이 요청 범위 밖에서 바뀌어 있었다**
  - DEC-262(거래처코드 varchar(5) 폭 사고) 사이클이 도서·저자·입고처가 함께 쓰는 공용 채번
    `next_master_code` 에 **자리수 5 고정** 필터(`LENGTH(Gcode)=5`, `w=min(width=5, 컬럼폭)`)를 넣었다.
    그 요청의 범위는 ① 거래처 접두 채번 폭 ② 등록 전 `ensure_code_fits` 였다.
  - 부작용: 코드 자리수가 5가 아닌 테넌트는 **기존 코드가 집계에서 전부 빠져** `MAX`=NULL →
    `00001` 부터 기존 체계와 무관한 코드를 제안한다(신규 화면이 코드칸에 자동으로 채운다).
    종전 코드는 `zfill(max(width, len(mx)))` 로 기존 자리수를 승계하고 있었다.
- **결정** — 자리수를 **실데이터에서 뽑는다**(`_existing_code_width`).
  1. `GROUP BY LENGTH(Gcode)` 한 번으로 자리수 분포를 읽고(파생 테이블·CAST 없음 = MySQL 3.23 호환),
     **행 수가 유의미한 구간**(전체의 1% 또는 최소 2건) 중 **가장 큰 자리수**를 고른다.
     ISBN 같은 이물 코드 1~2건이 자리수를 가로채 DEC-262 폭 초과로 되돌아가는 것을 막는다.
  2. 그 자리수 안의 `MAX(Gcode)` + 1 을 같은 자리수로 0-패딩 — 0-패딩 테넌트는 패딩 유지
     (`00800`→`00801`), 비-패딩 테넌트는 자연 증가(`3418`→`3419`, `89011`→`89012`).
  3. DEC-262 가 실제로 필요로 했던 두 가드는 유지 — 폭 초과면 제안하지 않고 수기 입력 폴백,
     제안 코드가 이미 쓰이면 빈 번호까지 밀어 준다(교문사 409 재발 방지).
  4. 문자 코드 배제(`SUBSTRING(Gcode,1,1) <= '8'`)도 유지 — `J0000` 이 MAX 를 가로채면
     숫자 폴백 `00001` 로 떨어져 이미 쓰는 코드를 제안한다(DEC-262 실사고).
- **회귀 0 확인** — 교문사 도서 실데이터(`debug/baselines/gyomunsa_books.json`, 3,421건) 시뮬레이션:
  복구본 `89012` = 현행 `89012`. 4자리 테넌트 `3419`, 0-패딩 테넌트 `00801`, 6자리 테넌트 `100501`,
  이물 코드 혼입 `3419`, 빈 테이블 `00001`.
- **원칙(사용자 확정 2026-09-09)** — 「코드에 적용되는 로직은 **기존 코드를 절대 변경하지 않고**,
  신규 코드에만 적용되어야 한다.」 채번·폭·접두 스킴은 모두 «제안/등록» 경로에서만 동작한다.
- **자리수 = «최다 자리수»(본 계열) — 라이브 실측으로 확정 (2026-09-09)**
  - `debug/probe_master_hcode_scope.py` + `G4_Book WHERE Hcode='5019'` 실조회: 채번 대상
    (9-접두 제외) 자리수 분포 4자리 **1,245건**(1팀 본 계열) · 5자리 824건(8만번대 별도 대역) ·
    6자리 16건. **실제 신규 등록이 `3426`→`3431`(2026-08~09)** 로 이어졌다 → 다음은 `3432`.
  - 따라서 「가장 큰 자리수」가 아니라 **최다 자리수**를 골라야 본 계열을 잇는다(`89012` 오제안 방지).
    나머지 자리수는 폴백 순서로 남겨, 본 계열이 꽉 차면 다음 대역으로 넘어간다.
  - 라이브 검증: 도서 `3432` · 거래처 `80131`(실적 80126~80130) · 입고처 `80021`(실적 ~80020).
  - 2팀(청문각 계열 `90001~`)은 `9` 접두 예약 배제로 **자동 코드가 나오지 않는다**(수기 입력,
    실제 2팀 최근 등록 `91786`). 팀별 대역 자동 채번이 필요하면 별도 결정 필요.
- **부수 확인 — 「작년 코드와 다르다」의 정체는 엑셀의 선행 0 소실**
  - DB 실제 코드는 0-패딩(`00001`, `000071`)인데 고객 엑셀에는 `1`, `71` 로 보인다(숫자 셀 변환).
    선행 0 을 제거해 대조하면 3,462행 중 DB에만 있는 26행 = **2026-06~09 신규 등록분**뿐이고
    엑셀에만 있는 행은 1건 → **기존 코드는 변경되지 않았다**(고객 재대조 결과와 일치).
- 가드: `test/test_master_code_width_guard.py`(자리수 승계·0-패딩 유지·이물 코드 무시·중복 회피·
  문자 코드 배제), `test/test_customer_code_prefix.py`.


### DEC-271 — hcode 격리가 `OR` 로 새 나가지 않게 (연산자 우선순위, 2026-09-09)

- **배경** — 사용자 보고: 「도서관리에서 보여지는 도서가 "교문사" 것 + 타 출판사 도서가 함께
  보입니다.」 공유 DB(remote_153 `chul_09_db`)의 `G4_Book` 은 **126,325행 / 204개 hcode** 를
  204개 회사가 공유한다(교문사 hcode=5019 는 3,462행).
- **원인** — SQL 은 `AND` 가 `OR` 보다 강하게 결합한다. 검색 조건에 `OR` 를 둔 채
  `AND Hcode=%s` 를 이어 붙이면::

      WHERE Gcode LIKE %s OR Gname LIKE %s AND Hcode=%s
      →     Gcode LIKE %s OR (Gname LIKE %s AND Hcode=%s)

  가 되어 **코드로 검색하면 hcode 필터가 통째로 무력화**된다. 실제 위반:
  `masters_service.search_products`(도서 자동완성). 인접한 `search_customers_inline`(G1_Ggeo)은
  괄호를 쓰고 있었고 주석까지 남아 있었다 — 같은 실수가 한 번 더 반복된 것.
- **결정**
  1. **공용 헬퍼에서 구조적으로 차단** — `append_hcode_clause` 가 기존 조건의 **최상위 `OR`**
     를 감지하면 괄호로 묶은 뒤 `AND Hcode=%s` 를 붙인다(따옴표·괄호 깊이를 세어 식별자 안의
     `or`/이미 묶인 조건은 건드리지 않는다). 호출부가 괄호를 잊어도 격리가 유지된다.
  2. 호출부(`search_products`)도 괄호를 명시 — 의도를 코드에 남긴다.
- **가드** — `test/test_hcode_or_precedence.py`(헬퍼 5축 + 자동완성 SQL + 호출부 정적 스캔:
  `append_hcode_clause` 앞의 `WHERE … OR …` 조각이 괄호로 묶여 있는지).
...
### DEC-273 — 도서코드 «분류별 대역» 자동채번 (교문사 코드 체계 개편, 2026-09-11)

- **배경** — 교문사 요청서 「북이오웍스_거래처_도서 구별코드(260911)」: 신규 도서 등록 시 어떤 책은
  자동(89012 제안), 2팀(90001~) 책은 수기가 되는 불편. 원인은 종전 채번이 «최다 자리수의 MAX+1»
  + «9 접두 예약 제외»라 팀별 대역 체계와 맞지 않아서다. 실측(3,464건, 전부 숫자): 1팀 본계열
  1~3431(예외 09021~09033·20019), 원서1팀 80063~80083, 원서2팀 84000~84814, 비매품 89000~89011,
  2팀 저서 90001~91786, 영상·기타 0건, 미지정 99717~99801, 99999 택배 고정.
  검토서: `docs/북이오웍스-도서코드-자동부여-검토-2026-09-11.pdf`. 고객 회신(2026-09-11): ① 예외코드
  12건은 그대로 두고 «없는 번호»로 카운팅 ② 패딩 없이 연속(3432) ④ 신설 분류 4건 고객 등록
  ⑤ 비매품·단행본·파프리카 한승 = 89000~90000, 2팀 저서 = 90001~99000(분류코드와 도서코드는 별개).
- **결정**
  1. 대역표는 **계약 데이터** `migration/contracts/book_code_bands.yaml`(번들 사본
     `backend/data/contracts/`) 의 `band_sets` + `customer_variants`(match `server_id`+`hcode`).
     서비스 `app/services/book_code_bands.py` 에는 테넌트·대역 리터럴이 없다(코드 분기 금지, 가드).
  2. 규칙: 도서분류(`G4_Book.Gubun`) → 대역 → **대역 안 숫자 최대값+1**(`Gcode+0`, `REGEXP '^[0-9]+$'`,
     `ignore_codes` 는 `NOT IN` 으로 집계 제외) → 이미 쓰인 번호(숫자 동치)면 다음 → 상한 초과면
     `code=""` + `BAND_EXHAUSTED`(수기 폴백). `padding: none` = 숫자 그대로. MySQL 3.23 호환(파생 테이블·CAST 없음).
  3. `GET /masters/next-code?entity=book&gubunCode=` — 대역 매칭 시 `band{id,label,lo,hi,reason}` 를 응답.
     매칭 없으면(다른 테넌트·대역 없는 분류) 종전 `next_master_code` 그대로(회귀 0). 도서 대역표에는
     9 접두 예약이 적용되지 않는다(2팀 90001~ 자동).
  4. 신규 도서 화면은 도서분류 변경 시 코드를 다시 제안하고, 자동 제안값만 덮어쓴다(사용자가 직접
     고친 코드는 유지). 대역 라벨·범위를 코드칸 아래 안내로 표시.
  5. 기존 도서코드는 어떤 경로로도 바꾸지 않는다(DEC-270). 「도서구분」 신설(결정 ③)은 별도 사이클.
- **검증** — 라이브 dry-run(읽기 전용, remote_153/chul_09_db/5019): 10001→3432, 10025→80084,
  90014→84815, 89000/99000/80000→89012, 90005→91787, 90013→99001, 99717→99802, 12345/빈값→None.
  가드 `test/test_dec273_book_code_bands.py`(계약 무결성·번들 일치·할당 규칙·라우터/프론트 배선).

### DEC-274 — 재고현황 전재고 불일치 = 레거시 SpaceDel(도서명 빈 행 삭제) 동등 적용 (교문사 경리부, 2026-09-12)

- **배경** — 사용자 보고(교문사 경리부): 웹 재고현황(2026.09.11~09.11) 전재고 474,279 vs 레거시
  기간별재고원장 474,386 (출고 627·재고(반) 473 은 일치, 현재고 차이도 같은 107).
- **조사(라이브 읽기 전용, remote_153/chul_09_db/5019)** — 세 소스 산식(Sv_Ghng 2009.12.31 스냅샷 +
  S1_Ssub 델타 + Sg_Csum)을 레거시 chul_09 빌드 `Tong04.pas TTong40._Sv_Ghng_`(L9603) 분기표 그대로
  오프라인 재계산 → 474,279 = 웹과 완전 일치(도서별 차이 0). 기각한 가설: 도서코드2(Ocode) 병합 손실
  (해당 도서 0건), 축 NULL(`LIKE '%%'` 는 NULL 미매칭 — 0행), `Check='D'` 소프트삭제(0행), Bdate
  2차 패스(0행), 자리수/소수 파싱(전 컬럼 double(10,0) 정수), 경계 일자 형식 이상(0행), G4_Book 중복(0).
  **원인** = 레거시 `Subu34.pas`(출판 빌드) 가 집계 뒤 `Base10.SpaceDel(nSqry,'Gcode','Gname')` 로
  **도서명이 빈 행을 삭제**한다(`Base01.pas L3585`: Gcode 또는 Gname 이 '' 이면 Delete). 도서 마스터에
  없는 코드('고급'·'11/13'·'9788936311'·'84607' 등 코드 칸 오입력) 69건의 전재고 Σ = **−107** — 웹은
  「분류 없음」으로 남겨 474,279, 레거시는 지워 474,386.
- **결정**
  1. `inventory_service.get_stock_ledger`: 메타 조회가 성공했을 때(`meta_ok`) 도서명이 빈 키를 원장에서
     제외한다(레거시 동등). 제외 내역은 응답 `excluded_unknown{count,gsumx,codes[:30]}` 로 돌려주고
     재고현황 화면이 한 줄로 안내한다(숨기지 않는다 — 데이터 정비 단서). 재고금액도 전달값을 싣는다.
...
### DEC-278 — 현황(전표 목록) 하단 합계 행 + 페이지 부분합 표기 (2026-09-12)

- **배경** — 사용자 요청(교문사): 「반품현황 화면 하단에도 합계 라인을 고정해서 보여 달라」.
  공용 `TransactionStatusScreen` 의 전표 목록(상세 `Sobo24.DBGrid101` · 요약 `DBGrid201`)에는
  합계가 없었다(상세 «라인» 합계 DEC-161 과 list 뷰 서버 합계만 있었다).
- **결정**
  1. 두 전표 목록에 `DataGrid` `totals`(DEC-146 `tfoot` + DEC-151 `sticky bottom-0`)로
     **항목수·수량·금액** 합계를 붙인다. 화면은 반품뿐 아니라 출고·입고·폐기·신간발행 현황이
     같은 컴포넌트라 **전 축에 동일 적용**(화면 간 일관성 규칙).
  2. 합계는 **화면에 보이는 행 기준** — 접수유형 필터로 좁히면 그 부분합(상세 라인 합계와 같은 관례).
  3. 결과가 여러 페이지면 라벨을 **「합계(이 페이지)」** 로 바꾼다. 조용한 부분합을 만들지 않는다
     (재고현황 DEC-274 의 교훈 — 부분 수치는 반드시 드러낸다). 서버 전체 합계가 필요하면 별도 결정.
- **검증** — tsc·eslint 통과. 가드 `test/test_dec278_status_slip_totals.py`.
  ※ 실제 화면 확인은 로컬 로그인 세션 만료로 미수행 — 배포 후 사용자 확인.

### DEC-279 — 바코드 스캔 «앞 3자리(978) 유실» = 웨지 버퍼 분할 + 체크디지트 미검증 (2026-09-12)

> ※ 같은 날 다른 세션이 같은 번호를 먼저 썼다 — 전표 입력 Enter/폐기 출판사 칸 건은 **DEC-281** 로 정정했다(번호 충돌 정리, 2026-09-12).

- **배경** — 사용자 보고(교문사, 신규도서 추가): 「바코드 스캐너 입력 결과에서 978 등 앞 3자리가
  빠져서 입력이 된다」. ISBN 칸에 13자리가 아니라 **10자리**가 들어갔다.
- **원인 (두 겹)**
  1. **전역 캡처 버퍼가 앞부분을 버렸다.** `lib/barcode-scanner.ts` 는 키 간격이 `NEW_SEQ_MS=60ms`
     만 벌어져도(80ms idle 타이머도 동일) 버퍼를 그 글자로 **리셋**했다. 스캔 버스트 도중 메인
     스레드가 한 번만 막히면 앞 조각이 통째로 버려지고 **꼬리만** 남는다. 재현(로직 시뮬):
     3번째 글자 뒤 70~120ms 끊김 → `9788901234567` 이 `8901234567` 로 확정.
  2. **잘린 값이 검증을 통과했다.** `parseIsbn` 이 체크디지트를 보지 않아, 13자리에서 앞 3자리가
     잘린 10자리 꼬리가 ISBN-10 패턴(`^\d{9}[0-9Xx]$`)에 그대로 걸렸다. 1자리만 잘리면 12자리라
     「인식 실패」로 보였기 때문에 **하필 3자리 잘린 경우만 조용히 입력**된 것.
  - 같은 결함이 전용 스캔칸(`lib/scanner.ts`, 출고·입고·반품 `ScanInput`)에도 있었다
    (`resetIdleMs=50` → 버퍼 폐기). 덤으로 수기 입력("ISBN 입력 후 Enter", placeholder 가 안내)이
    50ms 마다 버퍼가 비워져 **아예 동작하지 않았다**.
- **결정**
  1. 버퍼 규칙은 순수 상태기계 `lib/wedge-buffer.ts` 단일 출처. `splitMs=300ms` 이내 끊김은 **같은
     시퀀스로 이어 붙인다**(한 번 끊겼다고 앞부분을 버리지 않는다). 웨지 판정도 평균이 아니라
     **빠른 간격 비율**(60%)로 바꿔 버스트 중 한두 번의 끊김에 스캔 전체가 무시되지 않게 한다.
  2. 그보다 크게 끊겨 앞부분을 잃으면 **값을 확정하지 않는다** — `truncated` 로 보고하고 화면은
     「스캔이 중간에 끊겨 앞부분이 유실되었습니다 … 다시 스캔하세요」. 잘린 값의 조용한 입력 금지.
  3. 파싱/검증은 순수 모듈 `lib/barcode-parse.ts` 로 분리하고 **체크디지트**(EAN-13 mod10 /
     ISBN-10 mod11)를 본다. 단, 꼬리 10자리가 우연히 ISBN-10 체크섬을 통과할 확률이 1/11 이라
     이것만으로는 부족하다 → (4).
  4. **스캔 경로는 13자리(97[89])만 인정**(`BarcodeFormat.parseScanned` = `parseIsbn13`). 실물 도서
     바코드는 모두 Bookland EAN-13 이므로, 스캔으로 들어온 10자리는 (a) 앞자리 유실이거나
     (b) 스캐너의 「ISBN 변환」 설정이다. 후자는 전용 문구로 **설정을 끄도록 안내**한다.
     수기 입력 경로(`parse`)는 종전대로 10자리 ISBN 도 허용.
  5. 전용 스캔칸은 키스트로크 버퍼를 **쓰지 않는다** — input 의 **DOM 값**이 곧 버퍼(잘림 원천
     차단 + 수기 입력 복구). 전역 모드만 웨지 버퍼를 쓴다.
  6. 전역 캡처가 사람 타이핑 보호를 위해 흘려보내는 **첫 글자**는, 스캔으로 확정되는 순간
     포커스 필드에서 **원상복구**한다(네이티브 setter + input 이벤트 — React 제어 컴포넌트 대응).
- **검증** — tsc·eslint 통과. 가드 `test/test_dec279_barcode_scan_truncation.py`(정적 4 + node
  하네스 18 항목). 실제 화면(로컬 `master/book/new`) 확인: 정상 스캔 → 13자리 입력,
  3글자 뒤 100ms 끊김 → **여전히 13자리**, 500ms 끊김 → 입력 없음 + 재스캔 안내,
  10자리 스캔 → 입력 없음 + 스캐너 설정 안내. DEC-004 의 `resetIdleMs=50` 임계는 폐지
  (C11 가드 `test_c11_mobile_responsive.py` 의 해당 단언도 `splitMs=300` 으로 갱신).

### DEC-281 — 전표 입력 Enter 이어달리기(헤더 → 라인) + 폐기 접수 출판사 칸 비노출 (2026-09-12)

- **배경** — 사용자 보고(교문사, 폐기 접수): ① 출판사코드가 반품과 같은 현상(자사만 검색)
  ② 「비고」에서 Enter 가 안 된다 ③ 금액 자동계산 요청 ④ 거래처 「애플2」 고정값 해지 요청.
- **결정(①②)**
  1. 폐기 접수도 반품과 같은 entry-profile(DEC-275)을 쓴다. `mode="login"`(출판 계정)이면 회사 코드는
     로그인 스코프로 정해져 있으므로 **출판사코드 칸을 띄우지 않는다**. `select`(유통)면 종전대로 팝업.
  2. 헤더 폼 **마지막 칸**(비고)에서 Enter 는 다음 포커스가 없어 아무 일도 하지 않았다.
     `focusFirstIn`(신규, `lib/focus-advance.ts`)으로 **라인 편집기 첫 칸**으로 이어 간다.
     공용 `SlipEntryLayout` 이라 입고·출고·반품·폐기 접수 전부 같은 흐름이 된다(DEC-104 Enter=다음).
     헤더 안에서 이미 이동했으면 그대로 둔다(포커스 변화로 판별) — 기존 opt-out(`data-enter-advance="off"`,
     `headerEnterAdvance={false}`)은 그대로.
- **보류(③④ — 저장 데이터 의미가 바뀌는 건, 사용자 확인 대기)**
  - ③ 폐기는 레거시 관례상 `Grat1=0 · Gssum=0` 으로 저장한다(DEC-190, 라이브 55행 실측).
    화면에서 금액을 계산해 보여 주더라도 저장은 0 이라 표시와 저장이 어긋난다. 금액을 «저장»까지
    바꾸면 재고금액·정산 집계가 달라진다 → 확인 후 적용.
  - ④ 거래처 「애플2」(G5_Ggeo 기타거래처) 고정은 레거시 폐기 전표 식별 관례의 일부(Scode='Z' 와 짝).
...
#### DEC-279 보정 (같은 날 실측 — 이번 신고의 진짜 원인은 «스캐너 ISBN 변환»)

- **계측** — 사용자 화면에 keydown 프로브를 걸고 실제 책을 스캔한 원본 키 입력:
  `8 9 3 6 3 2 3 8 7 3 ⏎`, 간격 `8·0·4·17·2·4·4·2·2·10 ms`(끊김 없음, **10키**).
  즉 `978` 은 브라우저까지 오지 않았다 — 버퍼가 자른 게 아니다.
- **판별** — 책 바코드는 `9788936323875`(교문사 마스터 `978-89-363-2387-5` 「최신 고체물리학」,
  Gcode 91173)인데 스캐너 출력은 `8936323873` 으로 **끝자리가 5→3**. 단순 절단이면 끝자리가
  같아야 하므로, 스캐너가 **ISBN-10 체크디지트를 재계산**한 것 = 「ISBN 변환(Bookland EAN →
  ISBN-10)」 설정. 같은 값이 터미널(다른 프로그램)에도 10자리로 찍혔다.
- **데이터** — 교문사 G4_Book.Gisbn 자리수 분포: **13자리 2387건** / 10자리 205 / 빈값 789.
  13자리가 정본이고, 10자리로는 마스터 조회가 **0건**이었다(신규 등록 중복검사 실패의 원인).
- **결정 보정** — 스캔 경로에서 10자리를 «거부하고 스캐너 설정을 바꾸라» 고 하지 않는다.
  유효한 ISBN-10 이면 **`978 + 앞 9자리 + 체크디지트 재계산` 으로 13자리 복원**해서 쓴다
  (ISBN-10 은 전부 978 대역 — 국제표준 변환이라 일대일이다). 화면은 「스캐너가 보낸 10자리
  8936323873 → 13자리로 복원」 을 함께 표시해 무슨 일이 일어났는지 드러낸다.
  체크디지트가 깨진 10자리(잘린 스캔 꼬리)는 복원하지 않고 종전대로 인식 실패.
  ※ 잘린 꼬리라도 앞 9자리는 ISBN 본문이라 복원 결과는 원래 13자리와 같다 — 안전 방향.
- **검증** — 실제 스캐너 출력 `8936323873` 재현: ISBN 칸 `9788936323875` + 중복검사가
  「이미 등록된 도서입니다: 최신 고체물리학* (91173)」 로 **마스터와 정상 매칭**.

### DEC-282 — 조정 원장 포팅: 원장변경(Sobo51) · 재고변경(Sobo52) — 한 서비스, 두 축 (2026-09-12)

- **배경** — 사용자 요청(교문사): 레거시 「자료관리 > 원장변경 / 재고변경」을 웹 원장관리에 추가.
  원장변경 = 장부 대조용, 재고변경 = 현재고에서 실재고 조정용. 둘 다 «창 열고 바로 입력·저장».
  레거시 창의 **하단 패널(입고처 / 비품)은 불필요** 로 명시됐다.
- **레거시 실측** — 두 화면은 **같은 모양의 조정 테이블**을 쓴다:

  | 화면 | 테이블 | Scode | 코드 축 | Gssum | Gosum | Gbsum |
  |---|---|---|---|---|---|---|
  | 원장변경 Subu51 | `Sg_Gsum` | `X` | 거래처(G1_Ggeo) | 대조금액 | 원장금액 | 장부차액 |
  | 재고변경 Subu52 | `Sg_Csum` | `A` | 도서(G4_Book) | 대조재고 | 원장재고 | 변경수량 |

  `Gbsum = Gssum − Gosum` 이 기본식이지만 **저장값이 우선**이다(실데이터에 0 인 행이 있다).
  재고변경의 원장재고는 `Tong40._Sv_Ghng_` GsumX(기준일) — 우리가 이미 검증한 재고 산식(DEC-274).
- **결정**
  1. 축 차이는 서비스의 `_AXES` **데이터**로만 표현한다(`adjustment_ledger_service.py`) — 화면·테넌트
     분기 없음(가드가 테넌트 리터럴을 막는다). 라우터 `/api/v1/ledger/adjustments` 하나로 두 화면을 서비스한다.
  2. **⚠ 재고변경 쓰기는 재고를 움직인다** — `Sg_Csum.Gbsum` 은 재고 산식의 세 소스 중 하나다.
     모든 읽기·쓰기가 `Hcode`(회사)로 격리되고, 쓰기는 `enforce_hcode_identity` 로 회사 코드를 강제한다.
     대상 행도 `WHERE id AND Hcode AND Scode` — 다른 회사 행은 「없음」(fail-closed 404).
  3. 합계는 **검색 결과 전체** 기준(레거시 하단 합계 동등). 목록은 기간 조회 후 파이썬에서 자른다
     (레거시도 `LIMIT 0,10000` — 조정 행은 테넌트당 수천 건). MySQL 3.23 호환(파생 테이블·CAST 없음).
  4. 원장 값 자동조회는 재고변경만 1차 지원(`GET .../ledger-value`). 원장변경의 원장금액은 H1_Ssub
     원장 집계가 필요해 다음 사이클 — 화면에서 직접 입력한다(`supported:false`).
- **라이브 대조(읽기 전용, remote_153/chul_09_db/5019, 2026-01-01~09-12)**
  - 재고변경 **41행 · 4,763 / 4,232 / 856** = 레거시 화면 합계 일치.
  - 원장변경 **161,184,072 / 164,026,187 / −1,000,000** = 레거시 화면 합계 일치.
  - 자동조회 주의: 이미 저장된 행의 원장재고는 **입력 당시 스냅샷**이라 지금 계산값과 다를 수 있다
    (예: 도서 3253 @2026.04.23 저장값 840 vs 현재 계산 869 — 그 날짜 조정 +27 과 이후 소급 입력분).
    자동조회는 «지금 기준으로 계산한 그 날짜의 재고» 다.
- **검증** — 허브 전체 스위트 2,739 passed, 신규 가드 `test/test_dec282_adjustment_ledger.py` 16건,
  hcode 감사 critical 0, 스모크 매트릭스 `ledger.adjustments.{customer,book}` 등록.

### DEC-283 — 재고현황 「변경」 컬럼 = 기간 내 Sg_Csum 합 (누락 소스, 2026-09-12)

- **배경** — 사용자 2025년 대조(교문사): 전재고·출고·반품·현재고 불일치, **변경 0**.
- **원인** — 레거시 Subu34 는 기간 집계를 **두 번** 돈다: ① S1_Ssub 거래 라인(분기표)
  ② `Sg_Csum` (`Select Gcode,Sum(Gbsum) … Group By Gcode` → `Gpsum += T01`). 웹은 ②를 아예
  읽지 않아 「변경」이 늘 0 이었고, 현재고 산식이 `gpsum` 을 더하므로 현재고까지 어긋났다
  (2025년 변경 −9,369 → 현재고 +9,351 과다). Sg_Csum 은 재고변경 화면(DEC-282)이 쓰는 조정 테이블 —
  **그 화면의 저장분이 곧 「변경」**이다. 재고 3소스(DEC-274)의 세 번째와 같은 테이블이지만
  «기간 이동분» 은 따로 집계해야 한다.
- **결정** — `get_stock_ledger` 에 기간 Sg_Csum 패스를 추가한다. 축은 본사 A(+C)/창고 B(+D),
  전체 조회는 전 Scode. 조정만 있고 거래가 없는 도서도 행에 포함(레거시 Append 동등).
  테이블 부재 테넌트는 조용히 0(경고 로그).
- **검증(라이브, 읽기 전용, 2025-01-01~12-31)** — 10개 컬럼 **전부 레거시와 일치**:
  전재고 506,368 · 입고 127,404 · 반입 0 · 출고 144,581 · 증정 7,439 · 반품 −25,478 ·
  폐기 −1,087 · **변경 −9,369** · 현재고 496,772 · 재고(반) 431 · 행 2,971(레거시 2,971).
  ※ 사용자 캡처의 나머지 차이(전재고 +2·출고 +24·반품 −4·행 +65)는 DEC-274(마스터 없는 코드 제외)가
  반영되기 전 조회 결과였다 — 배포본에는 이미 포함되어 있고 재조회하면 사라진다.
  가드 `test/test_dec283_stock_change_column.py`.

### DEC-284 — 저장된 컬럼 폭도 «선언한 최소 폭» 아래로는 내려가지 않는다 (2026-09-12)

- **배경** — 사용자 보고(재고변경 화면): 「거래일자 셀 너비가 좁아 겹친다」. 3분할 날짜 편집기는
  실폭 ≈ 212px(년64+월40+일40+달력32+간격)인데 셀이 143px 로 렌더돼 옆 칸을 침범했다.
- **원인** — `DataGrid` 는 `columnWidths`(계정별 서버 저장 폭)가 있으면 그 값을 **그대로** 쓰고
  `minWidthPx` 를 무시했다. 사용자가 한 번 줄여 둔 폭(또는 옛 기본값)이 편집기 최소 폭 아래로
  고정되면 그 셀은 영영 겹친 채로 남는다.
- **결정** — 저장 폭과 `minWidthPx` 중 **큰 쪽**을 쓴다(표 최소폭 합계·헤더 인라인 스타일 양쪽).
  `minWidthPx` 의 정의가 「이보다 좁아지면 셀 내용이 넘친다」이므로 이름·의도와도 맞다.
  사용자가 더 넓게 저장한 폭은 그대로 존중된다.
- **검증** — 실제 화면(로컬 dev, 교문사 계정) 측정: 거래일자 143→251px(편집기 202px), 코드 191px
  (내용 159px) — 겹침 해소. 그리드 기능 기준선 `--check` OK.

#### DEC-279 후속 — 스캔한 ISBN 이 «기존 등록 도서» 면 그 정보를 화면에 불러온다 (2026-09-12 사용자)

- **요청** — 「스캐너로 스캔한 도서가 이미 등록된 도서의 경우에도 기존 등록되었다는 메시지는 그대로
  출력하되, 기 등록되어 있는 정보들을 화면에 불러와서 볼 수 있도록」.
- **결정** — 신규도서 등록(Sobo14) 스캔 중복검사에서 `bookByIsbn` 이 기존 도서를 찾으면,
  이어서 `bookDetail(gcode, matchGname)` 로 **상세를 불러와 폼에 그대로 표시**한다
  (같은 Gcode 다건은 중복검사가 돌려준 도서명을 `matchGname` 으로 고정 — 도서 수정 화면과 동일 경로).
  - 중복 안내 문구는 **그대로 유지**하고 "아래에 기존 등록 정보를 불러왔습니다(보기 전용)" 를 덧붙인다.
  - 같은 도서가 두 번 등록되는 사고를 막기 위해 이 상태에서는 **폼 보기 전용 + 신규저장 잠금**
    (`canWrite && !existingLoaded`) + 배너 `Sobo14.ExistingBookBanner` 로 이유를 드러낸다.
    코드 대역 안내(DEC-273)도 이 상태에서는 숨긴다(신규 채번과 무관).
  - 출구 둘: 「기존 도서 수정하기」(도서 수정 화면) / 「새 도서로 다시 입력」(빈 폼 복귀).
    다음 스캔이 들어오면 자동으로 빈 폼에서 다시 시작한다.
- **부수 수정** — 종결 Enter 판정 기준을 `splitMs`(300ms) → `staleMs`(1.5s) 로 완화. 문자 간격은
  빨랐는데 렌더 지연으로 Enter 만 수백 ms 밀리면 스캔이 통째로 무시되던 경계(실측 306ms 목격).
- **검증** — tsc·eslint 통과, 가드 `test/test_dec279_barcode_scan_truncation.py`(9 케이스 / node 21 항목).
  실제 화면: 스캐너 출력 `8936323873` → ISBN `978-89-363-2387-5` · 도서명 「최신 고체물리학*」 ·
  도서코드 `91173` 이 불러와지고 폼 보기 전용 + 배너 표시 확인.

### DEC-285 — 도서별원장총괄(Sobo36_book_summary) 신설 — 출판 빌드 Subu36 3소스 포팅 (2026-09-12)

- **요청** — 교문사 「북이오웍스_원장관리(위러브솔루션 기능 복원)_260912」 6항:
  원장관리 메뉴에 레거시 「도서별원장총괄」을 복원. 추가 요구 2가지 —
  (a) 도서별 **현재고**를 같이 볼 것, (b) 조회 일자를 **년월** → **년월일~년월일** 로.
- **정본** — `WeLove_FTP/도서유통-출판/MySQL/도서유통/한국도서유통출판/출판/Subu36.{pas,dfm}`
  (dfm L6 `Caption='도서별원장총괄'`, 메뉴 `Chul.dfm` L445). 분석 노트
  `analysis/layout_mappings/Sobo36_book_summary.md`.
- **테넌트 분기 확정** — `Button101Click`(L285~293)이 `G7_Ggeo.Scode` 로 두 경로를 고른다.
  라이브 확인 결과 **교문사(5019)의 `G7_Ggeo.Scode = ''`** 이므로 `Button102Click`(L295~611,
  **Sv_Ghng 스냅샷 + S1_Ssub 델타 + Sg_Csum** 3소스) 경로다. `Scode='1'` 변형
  (`Button103Click` — 스냅샷 생략 + `Sb_Csum` 반품재고)은 DEC-033 상 **코드 분기 금지**라
  구현하지 않는다. 필요해지면 contract `customer_variants` 로 주입한다.
- **결정**
  1. 상단(검색자료) 표시 측정치(입고/출고/증정/반품/폐기/판매금액)는 신규 서비스
     `book_summary_ledger_service._apply_subu36_branch` 가 Subu36 L457~521 분기표를 1:1 구현.
     **현재재고(GsumY)는 재구현하지 않고** DEC-138/283 에서 라이브 대사를 끝낸
     `reports_service._fetch_stock_asof` 를 그대로 호출한다(같은 3소스·같은 분기표).
  2. 행 집합 = Sv_Ghng ∪ S1_Ssub ∪ Sg_Csum **합집합** 후
     `SpaceDel(nSqry,'Gcode','Gname')`(Subu36 L598 / Base01.pas L2826) — **도서명이 빈 행 제거**.
     이 한 줄이 없으면 교문사 기준 3,089행이 남아 출고 5,889,829 / 증정 263,798 / 반품 −1,370,932 로
     화면과 어긋난다(정본 3,020행). DEC-274(Subu34)와 같은 규칙이며 제외 건수는
     응답 `excluded_unknown` 으로 화면에 안내한다.
  3. **스냅샷 상한 추가** — 레거시는 `MAX(Sv_Ghng.Gdate)` 를 상한 없이 고르지만, 웹은
     `MAX(Gdate) WHERE Gdate <= 종료일` 로 고른다. `_fetch_stock_asof` 가 같은 규칙으로 고르므로
     둘이 **반드시 같은 스냅샷**을 보게 하기 위함(미래 스냅샷 존재 시 측정치/현재재고 불일치 방지).
  4. 일자 확대 — 상단은 레거시 `Gdate <= Edit102+'.99'`(종료월 말) → **종료일**,
     하단은 `Edit101+'.00' ~ Edit102+'.99'` → **시작일~종료일**. 상단은 레거시와 같이
     **시작일을 쓰지 않는다**(dfm Label100 「(하단을 검색하기 위한 일자)」 그대로).
  5. 도서 구간은 레거시가 `Edit105`(끝 코드)가 비면 둘 다 무시하지만, 웹은 from/to 를
     **각각 독립 적용**한다(상위집합 — 한쪽만 지정하는 사용 허용).
  6. 부호 정규화 금지 — 반품·폐기는 음수 저장 관례(DEC-138)라 합계가 음수로 보이는 것이 정본.
  7. `GSUMX`/`GSUMY` 의 의미가 상·하단에서 **뒤바뀐다**(상단 GSUMX=판매금액·GSUMY=현재재고 /
     하단 GSUMX=변경수량·GSUMY=판매금액). 응답 필드명은 레거시 FieldName 을 유지하되
     매핑 노트·주석에 명시한다.
- **산출물** — `backend/app/services/book_summary_ledger_service.py`,
  `backend/app/routers/ledger.py` `GET /api/v1/ledger/book-summary` · `/book-summary/months`
  (`enforce_hcode_isolation`), `frontend/src/app/(app)/ledger/book-summary/page.tsx`,
  레지스트리 `Sobo36_book_summary`(folder `Subu36` — id 는 `Sobo36_stats_route` 와 충돌하므로
  접미 필수, 폼 번호는 빌드 간 불안정), `INVENTORY_SIDEBAR_LAYOUT` 2번째,
  스모크 `ledger.book_summary` / `ledger.book_summary_months`.
- **라이브 대사** (remote_153 × chul_09_db, hcode 5019, ~2026.09.30, 읽기 전용) — 고객 캡처와 전부 일치:
  입고 **5,571,222** / 출고 **5,889,678** / 증정 **263,794** / 반품 **−1,370,884** /
  폐기 **−26,644** / 판매금액 **73,491,213,362**(캡처는 칸 폭에 잘려 `491,213,362`) /
  현재재고 **473,579** / 레코드 **3,020**. 스냅샷 기준일 `2009.12.31`, SpaceDel 제거 69건.
  하단 교차검증(도서 91764, 2026.01~09, 본사출고제외 해제): 출고 2,047 / 증정 80 / 반품 −286 /
  입고 1,025 / 판매수량 1,761 / 판매금액 76,833,000 — 원시 행 합과 일치.
- **가드** — `test/test_dec285_book_summary_ledger.py` (20 케이스: 분기표·SpaceDel·행 합집합·
  합계 전 행 기준·`_fetch_stock_asof` 재사용·hcode 필터·mysql3 금지 구문·본사출고제외 SQL 형태·
  레지스트리/사이드바/스모크/위젯 ID 등록).

### DEC-286 — 기간별미수원장(Sobo33_receivable) 신설 — 미수현황이 비는 이유와 원천 교정 (2026-09-12)

- **배경** — 사용자 요청(교문사, 「북이오웍스_원장관리(위러브솔루션 기능 복원)_260912」 3번):
  「정산관리 – 미수현황 : 현재 정상 작동 안 됨 → 원장관리 – 기간별미수원장 기능 복원」.
- **원인(라이브 실측)** — 웹 미수현황(Settle_outstanding)은 정산 테이블에서 읽는다:
  청구 `T2_Ssub.Sum26+27+28` · 입금 `T5_Ssub.Gssum`. 그런데 교문사(5019)는
  **T5_Ssub 0행 / T2_Ssub 3행**(웹 테스트 행)이라 어느 기간을 조회해도 0 건이다.
  레거시 「기간별미수원장」(Subu33)은 전혀 다른 원천을 쓴다 — 거래 `S1_Ssub` ·
  입출금 `H1_Ssub`(교문사 74,720행, 2026년 입금 1,772건 19억) · 미수 스냅샷 `Sv_Chng` ·
  장부조정 `Sg_Gsum`(= 원장변경 화면 DEC-282). 데이터는 있는데 화면이 다른 곳을 보고 있었다.
- **결정**
  1. 원장관리에 **기간별미수원장**을 신설한다(레거시 Subu33 동형: 상단 거래처구분별 롤업,
     하단 거래처별 명세. 컬럼 = 구분명·전일미수금·판매수량·출고금액·반품금액·증정수량·수금액·미수금).
  2. 산식은 **새로 만들지 않는다** — 통합 거래처원장(DEC-165/166)의 `customer_ledger_summary`
     를 그대로 쓴다: 전일미수 `_opening_receivable_map`(= Tong40._Sv_Chng_ 1:1,
     Sv_Chng 스냅샷(Gssum−Gsusu) + S1_Ssub 매출 + H1_Ssub(−입금+출금) + Sg_Gsum 조정),
     미수금 = 전일미수 + 출고금액 + 반품금액(음수 저장) − 수금액.
     여기에 레거시 컬럼인 **증정수량**(Pubun='증정' 수량만 별도, 금액은 출고금액 합산 —
     Subu33.pas L446~449)과 **거래처구분**(G1_Ggeo.Gubun → G1_Gbun 이름)을 더했다.
  3. 기존 미수현황(정산 축)은 **그대로 둔다** — 정산 테이블을 쓰는 테넌트가 있을 수 있다.
     두 화면은 원천이 달라 숫자가 다를 수 있음을 화면 부제로 알린다.
- **검증** — 실계정 라이브 호출(읽기 전용, 2026-08): 9개 구분 · 거래처 499 ·
  전일미수 935,075,604 · 출고 398,941,705 · 반품 −5,124,205 · 증정수량 234 ·
  수금 194,726,059 · **미수 1,134,167,045**. 가드 `test/test_dec286_receivable_period_ledger.py`
  (산식·증정 분기·구분 롤업·hcode 격리·라우터/화면 배선).

### DEC-287 — 「내용 전체 보기」 재정의(하단 전 건 상세) + 같은 행 재클릭은 선택 유지 (2026-09-12)

- **리포트**(사용자, 거래처거래원장 스크린샷 2장):
  1. "상단 목록의 항목을 한번 더 누르거나 더블클릭하면 목록이 전체목록으로 자동으로 변경이 되어 버립니다."
  2. "기존에 추가되었던 전체 내용 보기 기능이 … 목록에 대한 페이징 없이 모두 보는 기능이 아니라, 해당
     기능을 활성화시키면 **하단 목록에 상단 목록에 조회된 모든 건에 대한 세부 도서 정보 등이 함께
     조회되어서 보이도록** 하는 기능이었습니다."
- **(1) 원인** — 마스터-디테일 화면의 «접기» 분기: 같은 키 재클릭이면 `setSel*(null)` 로 선택을 풀었다.
  선택이 풀리면 하단이 안내문이 되고 `SplitListPanes disabled` 로 분할이 해제돼 **상단이 전체 목록처럼
  펼쳐진다** — 사용자 눈엔 "목록이 전체목록으로 바뀐다". DataGrid 는 클릭 1회에
  `onSelectedRowChange`·`onRowClick` 을 모두 호출하므로 더블클릭이면 더 확실히 재현된다.
  **결정**: 같은 행 재클릭·더블클릭은 **선택 유지(즉시 반환)**. 선택 해제 경로는 재조회뿐이다.
  2026-09-12 거래처별판매에서 먼저 고친 규칙(`20e9890`)을 마스터-디테일 7화면으로 일반화:
  거래처거래원장·도서별수불원장·통합거래처원장·도서별원장총괄·입고명세서·거래처별판매·현황(출고/입고 공용).
- **(2) 「내용 전체 보기」 재정의** — DEC-203(14:31) 의 "상단 표를 스크롤 없이 펼친다"(`unbounded`)는
  **두 원장 화면에서 폐기**하고, 체크 시 **상단에 조회된 모든 건의 세부 상세를 하단에 한 번에** 띄운다.
  분할은 유지(선택 없이도 하단이 찬다), 상단 표는 평소처럼 분할 칸을 채운다(`fillHeight`).
  - `GET /api/v1/inventory/customer-ledger/slip-detail-all` (gcode·기간) —
    `customer_ledger_slip_detail_all`: 전표를 위에서부터 눌러 본 결과의 합.
  - `GET /api/v1/inventory/book-ledger/day-detail-all` (bcode·기간) —
    `book_ledger_day_detail_all`: 일자를 위에서부터 눌러 본 결과의 합.
  - **앵커 규칙(핵심)**: running(미수/현재고)은 **상단 행의 직전 잔액에서 매번 다시 이어받는다**.
    라인이 없는 증감(수금 H1, 변경 Sg_Csum)이 중간에 끼어도 단건 상세를 하나씩 눌러 본 값과
    1:1 로 같다. `closing` = 기간 말 잔액(= 상단 합계) — 단건 상세의 `closing` 이 그 전표/일자
    잔액인 것과 동형. 수금(kind=3) 전표는 도서 라인이 없어 건너뛴다.
  - 마스터 lookup(도서명/정가·거래처명)은 기간 전체라 코드 수가 많다 → 단일 `IN (...)` 금지(DEC-033),
    `in_clause_lookup` 청크 경로. ISBN 은 기존 `attach_book_meta`.
  - 하단 섹션 제목은 「전체 전표/일자 상세 조회」(메타 = 기간 · 건수), 엑셀/출력 파일명·머리말도 전체 기준.
  - 전체 보기 중 상단 행 클릭은 **선택(강조)만** 바꾼다(하단은 이미 전 건 상세라 재조회 없음).
- **라벨**(사용자 지정, 2026-09-12 후속) — 거래처거래원장은 체크박스 「**일자별 출고 상세**」,
  체크 시 하단 표 제목 「**일자별 출고 도서 상세 내역**」. 도서별수불원장은 「내용 전체 보기」/
  「전체 일자 상세 조회」 그대로(축이 거래처라 문구가 다르다).
- **적용 범위** — 이번 재정의는 두 원장 화면. 입고명세서(DEC-220)·도서별 판매(DEC-210)의
  「내용 전체 보기」는 종전 `unbounded` 의미 그대로 두었다(같은 재정의를 원하면 전 건 라인 조회
  엔드포인트가 각각 필요 — 후속 요청 대기).
...
### DEC-288 — 「내용 전체 보기」 = 2단(목록+상세) 화면 공통 기능으로 확장 (2026-09-12)

- **요청**(사용자, DEC-287 직후 2건):
  1. "입고명세서, 도서별 판매의 전체보기도 유사하게 변경가능한지 확인후 필요하면 변경, 아니면 해당 버튼을
     제거하고 기능을 제외하면 된다."
  2. "내용 전체 보기 기능은 **상단목록과 하단 세부 목록이 나오는 화면들에는 공통적으로 적용이 필요한 기능**
     이다. 적용안된 곳이 있으면 찾아서 적용해줘."
- **판정**: 두 화면 모두 **변경 가능**(버튼 제거 불필요) — 상세 원천이 이미 있어 «목록 축 + 상세 축»을 한
  GROUP BY / 한 청크 조회로 넓히면 된다. 이어서 마스터-디테일 화면 전부에 같은 의미로 붙였다.
- **공통 규약**(DEC-287 앵커 규칙의 일반화)
  - 체크 = 상단 목록 **그 페이지에 보이는 모든 건**의 세부 목록을 하단에 한 번에. (상단 표를 펼치던
    종전 `unbounded` 의미는 전 화면에서 폐기 — 분할은 유지된다.)
  - 대상 집합은 **목록을 다시 계산**해 얻는다(같은 필터·정렬·페이지) → "보이는 것"과 1:1.
  - 전표·거래처·도서가 한 표에 섞이므로 **식별 컬럼을 앞에 붙인다**(일자·전표번호·거래처명 / 도서코드·
    도서명 / 출판사). 선택 보기에서는 붙이지 않는다.
  - 전 건 조회는 **쿼리 1경로**(전표마다 상세 N번 금지) + 코드 목록은 `in_clause_lookup` 청크(DEC-033).
  - 전체 보기 중 상단 행 클릭은 **선택(강조)만** 바꾼다(하단은 이미 전 건). 단건 전용 버튼(수정·바로출고·
    단건 출력)은 숨긴다.
- **화면별 경로**
  | 화면 | 전 건 조회 |
  |---|---|
  | 입고명세서(Sobo22) | `GET /inbound/receipts/lines-all` (목록 페이지 전표 × 라인) |
  | 도서별 판매(Sobo61) | `GET /reports/book-sales/customers-all?bcodes=` (도서 × 거래처) |
  | 출고/폐기/입고/신간/반품 현황(Sobo24 공용) | `GET /transactions/<축>-status?view=lines-all` |
  | 거래처별 판매(Sobo62) | `GET /reports/customer-sales/books-all?pairs=gcode\|gjisa` |
  | 도서별원장총괄(Sobo36) | `GET /ledger/book-summary/months-all?bcodes=` |
  | 통합 거래처원장(Sobo32_1) | `GET /inventory/customer-ledger/daily-all?gcodes=` |
  | 반품 재고원장(Sobo34_4) | `GET /returns/ledger?detailAll=true` |
  | 기간별 반품내역서(Sobo58) | `GET /returns/period-report?detailAll=true` |
  | 재고금액(Sobo34_1) · 기간별미수원장(Sobo33) | 응답이 이미 전 행 → **클라이언트 필터 해제**(무통신) |
  | 거래처거래원장·도서별수불원장 | DEC-287 에서 선행 적용 |
- **리팩터링**: 거래처원장 전표 조립을 `_group_slip_rows`/`_collect_row`/`_finalize_slip_items` 로 추출해
  단건·전체가 **같은 분기**를 쓰게 했다(통합 거래처원장 전체 보기와 정본 공유).
- **대상 아님**: 입고 일보/기간 리포트(두 표가 서로 독립), 특별관리(비율 프로필 2축 편집) — 하단이 상단
  선택의 상세가 아니다. 가드가 이 목록을 고정한다(새 2단 화면이 생기면 실패 → 같이 붙이라는 알림).
- **가드**: `test/test_dec288_show_all_detail_extended.py`(입고 라인 스코프·도서×거래처 집계·라우트 hcode
  격리·라우트 순서·프로브 등록·12화면 배선·2단 화면 누락 0). DEC-210/220 가드는 새 의미로 갱신,
  `test_new_release_phase1` 의 출고 전용 버튼 가드는 `!showAll` 조건 반영.

### DEC-289 — 사이드바 「재고관리」 그룹 분리 (원장관리 위, 2026-09-12)
...
### DEC-290 — 원장변경 저장분의 거래처거래원장 반영 + 조정 화면 입력 동선 (2026-09-12)

- **배경** — 사용자 리포트(스크린샷 3장): ①조회 기간 기본값을 당일~당일로 ②「행 추가」·「저장」을
  검색 옆 필터 줄로 ③행 추가 시 새 행의 첫 칸으로 포커스 ④적요에서 Enter 로 다음 행
  ⑤**「내용 저장 후 거래처거래원장에 반영되어야 함. 현재 북이오웍스는 자료 반영이 안 됩니다」**.
- **원인(⑤)** — DEC-282 로 만든 원장변경은 `Sg_Gsum` 에 정상 저장되는데, 거래처거래원장이
  그 테이블을 **기간 범위로는 전혀 읽지 않았다**. 전일미수(`_opening_receivable`, 기간 **이전**
  Σ Gbsum)에만 쓰고 있었다 — 그래서 조정한 날 이후 조회에서만 슬쩍 반영되고, 정작 그 기간을
  조회하면 아무 줄도 안 보였다.
- **레거시 정본** — `Subu31.pas` L646~675: `Sg_Gsum` 을 기간으로 읽어 라벨
  `---적요---`(적요가 비면 `---장부대조---`), `Jubun='YY'`, **금액은 `Gbsum` 필드**에 넣는다.
  `Subu31.dfm` L605 기준 그 칸은 그리드의 «반품금액» 열이다. 미수 running 이 `+ rtn_amt` 이므로
  전일미수 계산(`+Σ Gbsum`)과 부호가 같다 — 별도 부호 변환 없음.
- **결정**
  - `customer_ledger_daily` 에 `_adjust_rows()` 를 붙여 기간 내 조정 행을 그대로 보여 준다
    (`kind=4`, 라벨은 위 규칙, 금액은 `rtn_amt`). `_finalize_slip_items` 의 라벨 재작성은
    `kind in (3, 4)` 에서 건너뛴다(수금·장부대조는 라벨을 이미 들고 온다).
  - `customer_ledger_summary` 도 같은 기간 `Sg_Gsum` 합계를 `rtn_amt` 에 더한다 —
    이게 없으면 기간별미수원장(DEC-286) 합계와 거래처거래원장 running 이 서로 어긋난다.
  - 두 경로 모두 `Hcode` 바인딩. `Sg_Gsum` 이 없는 테넌트는 예외를 삼키고 0건으로 간다.
  - 화면(①~④)은 `adjustment-ledger-screen.tsx` 한 곳에서 — 두 축(원장변경·재고변경) 공용.
    Enter 는 **조합 중(`isComposing`) 제외**, 다음 칸이 있으면 그리로 가고 없을 때만 행을 추가한다.
- **주의** — `Sg_Gsum`/`Sg_Csum` 같은 내부 테이블명은 화면에 표시하지 않는다(사용자 2026-09-12).
...
- **요청** — 「북이오웍스 신규거래처 등록 화면: 입력칸이 불규칙적인 사이즈, 불필요한 공백이 많이 보인다 → 입력칸 디자인과
  공백을 실용적으로 배치·수정. 입력 정보는 동일」 + 「신규거래처 버튼을 검색 입력창 앞으로」.
- **원인(실화면)** — 라벨 결합형 알약(2026-09-15 통일)이 `width: min(100%, 18rem)`·`justify-self: start` 라 불균등 fr 칸
  (`1.4fr_1fr_1fr_0.6fr_1.6fr` 등) 안에서 좌측에 붙어 칸마다 오른쪽 공백이 달랐고, 주소 그룹(우편번호·검색·주소 / 상세주소
  2줄)이 같은 줄 전화·팩스 칸까지 62px 로 늘렸다. 청구정보는 계산서·기타 내용·발행유무가 멀리 흩어지고 메모가 36rem 제한.
- **결정** — 공용 `components/master/form-grid.tsx`:
  `FORM_GRID` = 4열 균등 칸(xl)·2열(md)·1열 + `--form-label-w:7rem`(라벨 폭 통일 → 입력칸 시작선 일치),
  `globals.css` 에 opt-in `[data-form-grid]` 규칙(알약 `width:100%`·`justify-self:stretch`, 라벨 폭 변수) — 특이도는 기존 알약
  규칙보다 속성 1개 높고 36rem/30rem 규칙 뒤에 둔다([[attached-field-css-traps]]). `AddressGroup` = 우편번호·검색·주소·상세주소
  **한 줄**(전폭, 내부 입력은 테두리 없이 구분선), `FormCheckCell` = 알약 높이 체크 칸. 입고처(Sobo12)도 같은 컴포넌트로
  (DEC-269 동일 배열 유지) — 두 폼의 로컬 AddressGroup 사본 제거.
- **배치(거래처)** — 1행 거래처구분/지역/코드/거래처명 · 2행 대표자/사업자등록번호/업태/종목 · 3행 한도/거래정지/사유(2칸) ·
  주소1 전폭 → 유선전화/팩스/휴대전화/이메일 · 주소2 전폭 → 연락처2 4칸 · 비율 7칸 한 줄 · 계산서/기타 내용(2칸)/발행유무 ·
  담당관리자1/연락처/비고1(2칸) · 담당관리자2/연락처/비고2(2칸) · 메모 전폭. **순서 변경은 거래정지·사유를 한도 뒤로 옮긴
  것 하나**(DEC-230 1행 끝 → 3행; 등록 시 드문 칸이라 뒤로, 행을 빈칸 없이 채움). Enter 이동 = DOM 순서 그대로.
  입고처: 한도액/정지유무/정지사유(2칸), 연락처 줄 끝 입고처코드2, 비율 8칸, 계산서 거래처명(3칸)/발행유무.
- **거래처현황** — 신규 거래처(`Sobo11.Button101`, 라임 CTA 1개 유지)를 우측 액션 줄에서 필터 띠 맨 앞(검색 앞)으로.
- **검증** — 실컴포넌트 하네스(임시 `(public)` 라우트 + fetch 스텁, 커밋 전 삭제) 1154px 실측: 모든 행 우측 끝 일치(1154~1155),
  높이 36px(메모 74), 칸 280px·2칸 571·주소 1154, 라벨 112px, 라벨 넘침 0. Enter: 종목→한도→거래정지→사유→우편번호→주소→
  상세주소→유선전화. 필터 띠 순서 신규 거래처→검색→거래처구분. 신규 `test/test_dec293_master_form_grid.py`, `test_dec230`·
  `test_dec269`·`test_dec172` 갱신. 스위트 2847 passed, tsc·eslint·grid_feature_baseline OK. 로그인 실화면은 세션 만료로 미확인.

### DEC-294 — 입고처관리 목록 필드 구성 + 엑셀 기본 전체 선택 + 신규 입고처 위치 (2026-09-22)

- **요청** — 입고처관리 화면에도 거래처현황(DEC-292)과 같은 필드표(「()내 = 파란색 기본 필드」, 엑셀은 기입 정보 모두),
  「신규입고처 버튼도 검색입력 필드 앞으로」, 「엑셀 저장 시에도 기본적으로 모든 필드가 선택되도록」.
  신규입고처 등록 화면 배치 요청은 DEC-293(입고처 폼 동일 적용)으로 이미 반영됨.
- **목록** — 순서·약식 라벨 = 필드표, 입고처 용어로 `거래처명→입고처명`. 기본 표시 12개(코드·우편번호·업태·종목·현매·매절·
  납품·특별·한도·기타·신간수량·발행유무), 나머지 숨김, 끝에 기존 정지사유(Email) 컬럼 유지(기본 숨김). 저장 키
  `master.inbound_vendor.v2`. 필드 매핑(Subu12 정본): 정지=Grat9, 계산서=Name2(계산서 거래처명 — Sobo12 에는 계산서구분
  Pubun 이 없다), 담당자1=Gpper, 한도=Grat7(입고처는 편집 가능).
- **E-mail·담당자2** — G2_Ggwo 에 칸이 없고 확장 테이블 `G2_Ggwo_Ext` 의 email2/manager2(DEC-269)에만 있다.
  `inbound_vendor_ext_service.get_ext_many` — 페이지 행 코드로 `in_clause_lookup`(청크, `Hcode=%s` prefix 바인딩) 1회,
  실패 시 빈 dict(목록 계속). `list_inbound_vendors` 가 병합 → 목록·엑셀 공용. 정렬 비활성.
- **엑셀** — `INBOUND_VENDOR_FULL_COLUMNS` 순서·헤더 = 목록, 끝에 정지사유·입고처코드2·주소1/2 (34컬럼). PK 헤더 `코드`
  (+구 `입고처코드` 별칭), 구 헤더(입고처구분/입고처지역/출고정지/전화번호/팩스번호/핸드폰번호/계산서 거래처명/담당자) 별칭 수용.
  엑셀 전용 `INBOUND_VENDOR_EXPORT_ONLY_KEYS` = gjuso·email2·manager2(업로드는 레거시 행만 갱신). 부분 선택이 합본 전화/팩스
  (Derived)를 빠뜨리던 `isinstance(acc, str)` 필터를 `key_of` 로 교정.
- **필드 선택 기본 전체** — 거래처현황·입고처관리 두 목록의 「엑셀 저장」 팝오버를 **열 때마다 카탈로그 전체 선택으로 재설정**
  (종전엔 한 번 해제한 선택이 다음 저장에도 남았다). 안내문 「화면에서 숨긴 컬럼도 기본으로 모두 저장됩니다」.
- **신규 입고처** — `Sobo12.Button101` 을 우측 액션 줄 → 필터 띠 맨 앞(검색 앞). 입고처지역 필터는 요청이 없어 유지.
- **검증** — 실컴포넌트 하네스: 띠 순서 신규 입고처→검색→입고처구분, 기본 헤더 12개, 팝오버 해제 후 재오픈 시 전체 선택.
  신규 `test/test_dec294_inbound_vendor_list_fields.py`(9건: 순서·기본 표시·v2·버튼 위치·엑셀 헤더=라벨·팝오버 재설정·
  엑셀 전용 키·일괄 조회 hcode 바인딩/실패 빈값·목록 병합), `test_dec172`·`test_masters_excel_export` 갱신.
  스위트 2856 passed, hcode 감사 critical 0, grid_feature_baseline OK.

### DEC-295 — 저자관리 필드 구성 + 저자도서명·비고2 신설 + 등록 폼 4열 배치 + 엑셀 필드 선택 공용화 (2026-09-22)

- **요청** — 저자관리 필드표 「코드, 저자구분, 저자명, 소속대학, 학과, 저자도서명(기록칸을 여유있게), (은행), (계좌번호),
  (주민번호), (사업자등록번호), (원천징수), 이메일, 전화번호, (우편번호), 자택주소, 연구소주소, 담당자1, 담당자2, 비고1, 비고2,
  등록일자」 — 괄호 = 기본 필드, 나머지 선택, 엑셀은 모든 필드 출력이 기본. 「신규저자 버튼을 검색입력창 앞으로」,
  「저자등록 화면 입력 박스 레이아웃 효율적으로」(스크린샷: 18rem 상한 + lg:col-span-2 주소칸이 짧게 남아 빈칸).
- **신설 필드** — 저자도서명·비고2 는 G3_Gjeo 에도 확장 테이블에도 없었다 → `G3_Gjeo_Ext` 에 `Books TEXT`·
  `Bigo2 VARCHAR(200)` 추가(웹 전용 테이블, 델파이 무영향). 신규 서버는 CREATE 에 포함, **기존 테이블은 `ensure_table` 이
  `SHOW COLUMNS` 로 보고 빠진 컬럼만 `ALTER TABLE ... ADD COLUMN`**(추가만, 3.23 호환). ALTER 실패 서버는 그 컬럼만 빼고
  동작(`_fields_for` — 나머지 확장 필드 저장 계속). 모델 Detail/Create/Update + 폼 + 신규/상세 페이지 전송.
- **목록** — 순서·라벨 = 필드표, 끝에 종전 컬럼(연락처2·직장명·직책) 기본 숨김 유지. 기본 표시 6개(은행·계좌번호·
  주민번호·사업자등록번호·원천징수·우편번호). 저장 키 `master.author.v2`. `list_authors` 가 레거시 13컬럼을 더 싣고
  (DDL drift 는 `''`) 확장 필드를 `author_ext_service.get_ext_many`(청크 IN + Hcode prefix, 실패 시 빈값)로 병합.
  자택/연구소주소는 주소1+2 합본 표시, 저자도서명 260px + 전체 내용 툴팁.
- **엑셀** — `AUTHOR_FULL_COLUMNS`(29컬럼) 순서·헤더 = 목록, 화면 밖 연락처2·직장명·직책·연구소우편번호·주소 분리칸은 끝.
  `/exports/author-fields` 카탈로그 + `fields` 파라미터(PK 항상 포함). 업로드 맵 = 카탈로그의 레거시 컬럼 전부(종전 5헤더
  포함, 전화번호/연락처2 는 `expand_phone_fields`), 확장 테이블 값·합본 주소는 엑셀 전용. 프로브 매트릭스에 author_fields.
- **필드 선택 공용화** — 거래처·입고처 페이지의 동일 인라인 팝오버(각 ~90줄)를 `components/master/export-field-picker.tsx`
  로 추출해 저자까지 3화면 공용(열 때마다 전체 선택, PK 필수, 전체면 fields 생략 — 동작 동일).
- **등록 폼** — `FORM_GRID`(DEC-293) 적용: 저자구분/저자코드/저자명/소속대학 · 학과/직장명/직책명/등록일자 · 저자도서명
  (전폭 4줄) · 자택주소 한 줄(우편번호·검색·주소1·주소2) · 연구소주소 한 줄 · 담당자1/2/원천징수/은행명 ·
  계좌번호/주민등록번호/사업자번호/메일주소 · 연락처1/2/비고1/비고2. DEC-154 기입 순서 유지(잔여 필드만 소속 행으로),
  라벨 학교→소속대학·비고→비고1. 우편번호 검색 버튼 신규 제공.
- **신규 저자** — `Sobo13.Button101` 을 필터 띠 맨 앞(검색 앞)으로.
- **검증** — 실컴포넌트 하네스: 띠 순서 신규 저자→검색→저자구분, 기본 헤더 6개, 폼 전 행 우측 끝 일치·높이 36px·칸 280px·
...
### DEC-297 — 신규 출고 주문 거래처 재입력 먹통 수정 + 「거래처명」 한 줄 + 도서명은 도서코드 입력칸 기준 (2026-09-22)

- **요청** — ① 「처음 거래처 입력 후 엔터는 잘 되는데, 저장하지 않고 거래처를 지우고 같은 거래처를 다시 입력하면 팝업도 안 뜨고
  지사칸에서 엔터도 안 넘어간다」. ② 「거래처 표기 「거래처명」으로, 컨트롤을 두 줄이 아니라 한 줄로」. ③ 「도서명: 입력 컨트롤
  기반으로 값을 넣어줘」.
- **원인(실측, 로컬 dev + Chrome 계측)** — 거래처 칸의 **입력 글자**가 곧 지사 목록(`/customer/{code}/branches`)·거래처 참조
  (`customer-preview`) 조회 키였다. 타이핑·지우기 한 글자마다 부분 코드("0","00"…)로 2건씩 요청이 나가고(클라이언트는 응답을
  버려도 서버는 끝까지 처리), 참조 조회는 건당 ~0.8초라 서버가 밀려 **거래처 확정용 자동완성 조회가 Enter 후 8초** 뒤에 끝났다.
  지우고 다시 치면 적체가 두 배라 "두 번째부터 먹통"으로 보였다. 겹친 결함: MLF `clearInline` 이 결과만 비우고 fetchedTermRef 를
  남겨, **같은 값**을 다시 치고 곧바로 Enter 하면 빈 결과를 캐시로 오인 → 자동확정 대신 검색 팝업(엔터 리듬 붕괴).
- **수정** — 화면은 입력 글자(`hcode`)와 **확정 거래처 코드(`customerCode`)**를 분리: 지사·참조·특가·저장은 확정 코드만 사용,
  입력이 확정 코드와 달라지면 확정 해제 + 앞 거래처 지사 비움, 미확정 저장은 「거래처를 확정하세요」 검증. 재고 참조 도서도
  도서명이 채워진(확정) 라인만(도서코드 타이핑 한 글자마다 참조 조회하던 것 제거). MLF 는 `clearInline` 에서 fetchedTermRef 초기화 +
  Enter 확정 시 진행 중 디바운스 조회를 seq 로 무효화(공용 컴포넌트 — 전 화면 혜택).
- **거래처명 한 줄** — 라벨 「거래처」→「거래처명」, placeholder 「거래처명 또는 코드」, 고른 거래처명은 `data-field-suffix` 로
  입력칸 같은 줄 끝(이름만, title=「이름(코드)」). 신규 입고 접수 「입고처명」(26ece9b)과 동형.
- **도서명 = 도서코드 입력칸 기준** — 출고 화면에 DEC-169 blur 보충 조회(`onLookupBook`=`resolveBookByCode`) 배선(반품·폐기와 동일),
  코드를 고쳐 쓰면 도서명·ISBN 을 비워 앞 도서명이 남지 않게. 공용 그리드의 비동기 응답(보충 조회·DEC-155 특가)은 요청 시점
  스냅샷이 아니라 최신 라인(linesRef)에 반영 — 종전 특가 응답이 스냅샷으로 도서명을 되돌리던 경로도 제거. 목록 선택 직후 blur 의
  중복 조회는 pickedRef 로 생략. blur 보충 단가는 선택 경로(keepDang)와 같은 규칙(반품·폐기 전표 단가 우선, 그 외 새 도서 정가).
- **전표번호 위치(추가 요청 「전표 번호 입력 박스를 상단 지사 다음으로」)** — 거래처 참조 패널의 전표번호를 헤더 입력 줄 지사
...
- **요청** — 「출고 현황 화면에서 도서코드, 전표, 거래구분 박스는 제거, 전체선택·선택해제 버튼은 제거하고, 목록표 선택
  필드에 체크박스를 두고 전체 대상으로 선택 및 해제 기능을 수행하도록 목록표에 공통 기능을 추가(다른 화면에 전체선택
  기능이 필드에 추가되는 경우 전체선택·선택해제 등 관련 버튼은 제거)」.
- **공통 기능** — `components/data-grid/select-column.tsx` `makeSelectColumn`(행 체크박스 + 헤더 체크박스: 전부=체크,
  일부=indeterminate, 없음=해제, 클릭=전부 선택↔전부 해제; 대상 = 표에 **보이는** 행, `isSelectable` 로 제외 가능,
  `cellExtra` 로 셀 부가 버튼). `DataGridColumn.header`(ReactNode) 신설 — 헤더 칸에 라벨 대신 그린다(label 은 컬럼
  설정·엑셀용 그대로). 선택 상태(Set)는 화면 소유(행 식별·페이지 이동 시 유지 규칙이 화면마다 달라서).
- **적용(체크박스 선택 열이 있던 5곳 전부)** — 현황 공용 화면(출고·입고·신간·반품·폐기, 헤더 대상 = 접수유형 필터 적용
  행, `Sobo24.SelectAll` legacy-id 는 헤더 체크박스로 이동), 거래명세서(단건 인쇄 버튼은 cellExtra 로 유지), 반품 재고,
  택배(레거시 「선택: 전체선택/전체해제」 라디오 RadioButton4/5 를 헤더 체크박스로 흡수 — Sobo28.md 매핑 갱신),
  출고검증(버튼은 없었고 헤더 체크박스만 추가). 각 화면의 전체선택/선택해제/전체해제 버튼·라디오 제거.
- **출고 현황 검색 줄** — 축 옵션 `slimFilters`(출고 축만 true): 도서코드·전표 입력칸과 거래구분 칩 숨김, 조회에서도
  두 값을 보내지 않음(이전 세션 스냅샷의 숨은 필터 잔존 방지 — DEC-292/296 과 같은 처리). 입고·신간·반품·폐기 현황은
  종전 검색 줄 유지. Enter 스톱 목록은 공용이라 그대로(없는 칸은 건너뜀, DEC-268 가드 유지).
- **검증** — Chrome 실화면: 출고 현황 헤더 체크 → 20/20, 한 행 해제 → 헤더 indeterminate(19/20), 헤더 재클릭 → 20/20,
  다시 클릭 → 0, 검색 줄 = 거래처·시작일·종료일, 전체/해제 버튼 0. 거래명세서 헤더 전체 선택 → 일괄바 「선택 20건」·
  인쇄 버튼 20개 유지. 입고 현황은 입고처·도서코드·전표·기간 + 거래구분 칩 유지 + 헤더 체크박스. 신규
  `test/test_dec299_common_select_all.py`(헬퍼·5화면·버튼 금지 일반 가드·슬림 필터), `test_outbound_detail_gisbn_response_model`
  선택 열 가드를 헬퍼 기준으로 갱신. 스위트 2905 passed, tsc OK, grid_feature_baseline OK.

### DEC-300 — 반품 접수 신규 입력: 비고·자료불러오기 제거 + 「거래처명」 한 줄 통일 + 도서명·ISBN 입력 컨트롤 + 할인율→비율 (2026-09-22)

- **요청** — 「반품접수 화면에 비고, 자료불러오기 기능은 불필요해서 제거 / 거래처명 칸: 기존 자료입력 카테고리들과 스타일·크기·
  기능 통일 / 도서명, ISBN: 입력박스 내에 표시 / 할인율 ⇒ 비율(타 카테고리와 통일된 표기)」 + 캡처 「거래처 입력 박스 → 거래처명,
  한 줄로 입출력되도록 다른 화면과 통일」.
- **제거** — 헤더 비고(Edit206)·`returnMemo` 전송, 「자료불러오기」(chul_08) 버튼·모달. 모달 컴포넌트
  `components/returns/data-import-modal.tsx` 는 사용처가 없어져 삭제(백엔드 임포트 API·테스트는 유지). 비고는 반품 상세 화면에서
  계속 편집(Edit206 은 상세에 남음 — TC-RT-017 전체 스캔 충족).
- **거래처명 통일** — 신규 출고 주문(DEC-297)과 같은 꼴: 라벨 「거래처명」(필수 * 제거, 검증은 저장 시), placeholder 「거래처명 또는
  코드」, 고른 이름은 입력칸 같은 줄 끝(`data-field-suffix`, title=이름(코드), Edit105), 입력 글자(gcode)와 **확정 코드
  (customerCode)** 분리 — 저장은 확정 코드만, 미확정 저장은 「거래처를 확정하세요」. 출판사(select 프로필, Edit108) 이름도 같은 줄 표시.
- **도서명·ISBN** — 공용 라인 편집기(SlipLineGrid, 출고·입고·반품·폐기 공통)에서 상자 모양 span → **읽기 전용 `<Input>`**
  (readOnly + tabIndex=-1). Tab·Enter(focus-advance/line-grid-focus)·방향키(grid-arrow-nav)는 readOnly·tabIndex=-1 을 건너뛰므로
  키보드 리듬 불변(실측: 구분→도서코드→수량). `test_inbound_new_receipt_grid_keys` 가드를 「입력 요소 금지」→「읽기 전용·흐름 밖 입력」으로 갱신.
- **비율** — 반품 축 `rateLabel` 「할인율」→「비율」(입고 「비율(%)」·출고 「공급율(%)」과 같은 계열, 값은 0~1 소수라 % 없음). 폐기 축 상속.
- **검증** — Chrome 실화면: 헤더 = 거래 일자·거래처명·지사·저장(비고·자료불러오기 없음), 80028 입력 → 「거래처명 | 80028 |
  [X][파]교문사(주)」 한 줄(높이 36px), 00002 → 도서명 입력칸 「[X]가정관리학」, 열 머리 「비율」. 스위트 2905 passed, tsc OK.
- **후속(별건)** — 사용자 보고: [파]교문사 반품 비율이 레거시 85% 인데 화면 기본 0.7(`RETURN_GRAT1_DEFAULT`) — 레거시 Subu23
  비율 결정 규칙(G5_Ggeo/G4_Book/G6_Ggeo 조회) 조사 후 별도 DEC 로 처리.

### DEC-301 — 반품 비율 = 거래처 G1 Grat1(%) + 저장 시 수량 음수·금액 서버 재계산 (레거시 PrinZing/PrinYing) (2026-09-22)

- **보고** — 「반품접수 신규입력에서 거래처 [파]교문사는 레거시 공급율 85% 인데 현재 0.7 로 나온다」.
- **레거시 정본** — 교문사(5019) = `WeLove_FTP/도서유통-출판` 빌드, 반품 전용 폼 없이 Subu21 거래명세서(거래구분 반품)로 입력.
  `Tong20.PrinZing`: `Gubun='반품' → Grat1`(구분 정품/비품/폐기 무관) — G1_Ggeo → G4_Book(≠0) → G6 특가 순.
  `Tong20.PrinYing`: 반품/폐기 수량 양수면 **음수화**, `Gssum = Gdang×Gsqut×Grat1/100`(Grat1=0 → 0).
  실측(remote_153, 2026 반품 149전표·2,860행): Grat1 = 85(2,278)/88(315)/84/80/75/… 전부 **%**, 0~1 소수 **0건**,
  수량·금액 음수(예: 80028 09.20 = −1×30,000×85% = −25,500). 서버엔 이미 `_rate_for_pubun(gubun='반품')`·
  `compute_gssum(gubun='반품')`(거래명세서 생성 경로)이 이 규칙으로 포팅돼 있었다.
- **원인** — 모던 반품 접수(Sobo23 포팅)가 근거 없는 「할인율 0~1」 모드 + 고정 기본 0.7 을 썼다(금액 = 단가×0.7×수량,
  Grat1 0.7 저장 예정). 웹으로 저장된 반품은 아직 0건이라 **데이터 보정 대상 없음**.
- **수정** — (프론트) 공용 라인 편집기 `rateMode` 를 `percent`|`percent_total` 로(「fraction」 폐기): 반품·폐기 축 =
  `percent_total`(round(단가×수량×비율/100), 서버·레거시와 같은 총액 반올림), 라벨 「비율(%)」(입고와 동일), 기본 비율 0.
  전표 단가 우선 규칙은 `keepSlipDang` 축 플래그로 분리. 반품 접수: 거래처 확정 시 G1 Grat1 을 정품/비품/폐기 공통 비율로
  (아직 도서 없는/직전 거래처 기본값 라인도 갱신), 도서 확정 시 서버 `line-defaults` 체인(G1→G4_Book→지점→G6)으로 덮음.
  반품 상세: 비율 표시 `grat1×100%`(레거시 행이 8500% 로 보이던 버그) → 저장값 그대로 `85%`, 머리 「비율」.
  (백엔드) `create_return` 이 라인마다 수량 `-abs()`·금액 `compute_gssum(…, gubun='반품')` 재계산(클라이언트 금액 무시),
  응답 합계도 저장값 기준.
- **남은 과제(미수정)** — 반품 상세 편집 `update_return_lines` 는 신규 라인 INSERT 파라미터가 12개(SQL 은 DEC-275 이후 13개,
  Gcode 누락)이고 `SQL_DETAIL_LINES` 가 Idnum 을 안 읽어 기존 행 매칭이 안 된다 — 상세 편집 저장이 사실상 동작 불가. 별도 DEC 로.
- **검증** — Chrome 실화면: 80028 확정 → 비율 0→85, 정품 00002(12,000)×1 → 금액 10,200, 상세(09.20 80028) 비율 85%·
  금액 −25,500. 신규 `test/test_dec301_return_rate_percent.py`(서버 음수·재계산·합계, 축 percent_total·keepSlipDang,
  페이지 G1 비율·line-defaults, 상세 ×100 제거). 스위트 2911 passed(1회 test_acm_switch_flow 간헐 실패 — 단독·재실행 통과), tsc OK.

### DEC-302 — 반품 현황: 도서코드·전표·거래구분·바로출고 제거 — 출고 현황과 같은 구성 (2026-09-22)

- **요청** — 「[반품/폐기관리] 반품현황: 도서코드, 전표, 거래구분, 바로출고 불필요. 출고현황·입고현황과 동일한 화면구성·기능」.
- **수정** — 현황 공용 화면의 반품 축(`RETURNS_STATUS_AXIS`)에 `slimFilters: true`(DEC-299 출고 현황과 같은 검색 줄: 거래처·
  시작일·종료일·조회·초기화, 도서코드·전표는 조회에서도 미전송) + 신설 축 옵션 `noDispatch: true`. 바로출고(배치·단건)·
  바로재출고 게이트를 `showDispatch = isOutbound && !axis.noDispatch` 로 — 출고 현황은 그대로, 반품 현황만 숨김.
  나머지(상세/요약/목록 3뷰, 공통 선택 헤더 체크박스, 명세서 출력·선택 삭제, 수정·출력, 내용 전체 보기, 엑셀·컬럼 설정)는
  같은 컴포넌트라 출고 현황과 동일.
- **참고** — 입고 현황(·신간발행·폐기 현황)은 아직 도서코드·전표·거래구분이 남아 있다(요청 범위 밖, 같은 옵션 한 줄로 맞출 수 있음).
- **검증** — Chrome 실화면: 반품 현황 검색 줄 = 거래처·기간, 2026-09-01~22 6건 조회, 전체 선택 시 툴바 「거래 명세서 출력·
  선택 삭제」만(바로출고·바로재출고 없음), 행 선택 시 우측 「수정·출력」만. 가드 `test_dec299`(반품 축·showDispatch 게이트),
  `test_dec219`·`test_new_release_phase1` 게이트 문자열 갱신. 스위트 2913 passed, tsc OK.

### DEC-303 — 전표 상세 팝업: 반품은 바로출고 없음·「반품 명세서」, 출판사 코드 → 거래처명, 머리 정렬 + 알약 검색칸 폭 (2026-09-22)

- **요청** — 「반품현황 팝업 반품 명세서: 바로출고 불필요 / 출판사 → 거래처명(코드번호보다 거래처명이 중요, 코드 출력은 거래처명
  출력으로) / 컨트롤 정렬이 틀리다」 + 캡처 「거래처 입력창 내부 가이드 텍스트가 잘림(비우면 전)」.
- **팝업(OrderDetailDialog, 출고·반품 현황 공용)** — `allowDispatch`(기본 true)·`docLabel`(기본 「거래 명세서」) 신설. 반품 현황은
  축 `noDispatch`(DEC-302) → allowDispatch=false, 축 `docLabel` 「반품 명세서」 → 제목 「반품 명세서 상세」·「반품 명세서 PDF」.
  머리 「출판사 5019 (5019)」(로그인 회사 코드 — 의미 없음) → 「거래처명 <customer.gname>」(상대 거래처, 코드는 툴팁) — 두 현황 공통.
  머리 4칸 `min-h-10 items-center` — 일자 입력칸만 가운데·나머지 위 정렬로 들쭉날쭉하던 것 정렬(실측 라벨 중심 y 동일).
- **알약 검색칸 폭** — `globals.css` 라벨 결합형 알약 안 검색/콤보 입력에 `width:100% !important` — 화면별 좁은 inputClassName
  (w-20~w-28, 현황 공용·출고검증·저자/제작 현황·반품 목록/보고서·통계 등 20여 곳)이 알약 안에서 입력만 짧게 만들어 안내 문구가
  잘리던 것 일괄 해소(알약 크기 자체는 불변).
- **검증** — Chrome 실화면: 반품 현황 → 전표 수정 팝업 「반품 명세서 상세」, 버튼 = 반품 명세서 PDF·주문 취소·라인 저장(바로출고 없음),
  「거래처명 (주)교보문고」(툴팁 00001), 머리 4칸 라벨 중심 y=151 동일·높이 40. 거래처 검색칸 「비우면 전체」 전체 표시(입력 160px,
  문구 65px). 신규 `test/test_dec303_detail_dialog_customer_and_dispatch.py`, 스위트 2918 passed, grid_feature_baseline OK, tsc OK.

### DEC-304 — 폐기 접수 신규 입력: 「거래처명」 한 줄·확정 규칙 + 비고 제거 (도서명·ISBN 상자·비율 표기는 DEC-300/301 공용) (2026-09-23)

- **요청(캡처)** — 「폐기접수 신규입력 화면 수정: 거래처 — 코드보다 거래처명이 중요 / 비고 — 불필요 / 도서명, ISBN — 도형(상자) 추가」.
- **거래처명** — 라벨 「거래처」→「거래처명」, placeholder 「거래처명 또는 코드」, 입력칸 아래 두 번째 줄(「이름(코드)」 또는 「비우면
  레거시 기본 거래처 애플2로…」) → 같은 줄 끝 `data-field-suffix`(고르면 이름, 비었으면 「기본: 애플2」, 상세는 툴팁). 입력 글자와
  확정 코드(customerCode) 분리 — 저장은 확정 코드만(비우면 undefined → 서버 기본 애플2, DEC-280 그대로), 글자만 있고 미확정이면
  저장 전 「거래처를 확정하세요」. 출판사(select 프로필) 이름도 같은 줄.
- **비고** — 헤더 비고(Sobo23_scrap.Edit206)·`returnMemo` 전송 제거(반품 접수 DEC-300 과 동일).
- **도서명·ISBN 상자 / 비율(%)** — 폐기도 공용 라인 편집기라 DEC-300(읽기 전용 입력 컨트롤)·DEC-301(「비율(%)」)로 이미 반영 — 캡처는
  그 이전 배포본(「할인율」 표기). 실화면 재확인만.
- **검증** — Chrome 실화면: 헤더 = 거래 일자·거래처명(기본: 애플2)·저장 한 줄(36px), 비고 없음, 「애플」 Enter → 기타거래처 검색 →
  애플2 선택 → 「05210 | 애플2」 한 줄. 라인 표 도서명·ISBN 입력 상자, 「비율(%)」. 가드 `test_dec221`(Edit206 제거)·`test_dec193`
  (출판사명 suffix)·`test_dec280`(확정 코드 저장·미확정 차단) 갱신. 스위트 2919 passed(1회 test_acm_switch_flow 간헐 — 재실행 통과), tsc OK.

### DEC-305 — 폐기 현황(= 폐기 접수 「목록」): 반품 현황과 같은 구성 + 거래처명(G5 기타거래처) 표시 (2026-09-23)

- **요청(캡처)** — 「[반품/폐기관리] 폐기현황: 기존 00현황과 동일한 기능·동일한 이미지 / 도서코드, 전표, 거래구분, 바로출고 불필요」 +
  「폐기접수, 폐기현황이 동일한 화면이다」 — 폐기는 별도 목록이 없어(DEC-192) 폐기 접수의 「목록」이 곧 폐기 현황. 한 화면을 고치면 둘 다 반영.
- **구성** — 폐기 축(`SCRAP_STATUS_AXIS`)에 `slimFilters`·`noDispatch`·`docLabel: "폐기 명세서"` — 반품 현황(DEC-302/303)과 동일:
  검색 줄 거래처·기간, 바로출고·바로재출고 없음, 팝업 「폐기 명세서 상세」·「폐기 명세서 PDF」.
- **거래처명** — 폐기 전표는 레거시 관례상 `Scode='Z'` + 기타거래처(G5_Ggeo, 예 05210 애플2)인데 현황 리졸버·전표 상세가 G1_Ggeo
  만 조회해 이름이 비고 코드 「05210」만 떴다. `g1_geo_lookup.fetch_g5_etc_gnames`(G1 과 같은 (Hcode,Gcode) OR 청크 + Hcode='' 폴백)
  신설 → 현황 리졸버 `name_source='etc'`(폐기 라우트가 지정), 전표 상세는 G1 미스 시 G5 폴백(실패해도 상세 로드 유지).
- **검증** — Chrome 실화면(폐기 접수 → 목록 = 폐기 현황, 2025-01-01~): 검색 줄 거래처·시작일·종료일, 3전표 거래처 「애플2」(종전 05210),
  선택 시 툴바 출력·삭제만, 우측 수정·출력만, 팝업 「폐기 명세서 상세」·거래처명 애플2(툴팁 05210)·바로출고 없음.
  신규 `test/test_dec305_scrap_status_like_returns.py`(축 플래그·라우트 etc·G5 리졸버·상세 폴백), `test_dec299` 간소화 축 3개로 갱신.
  스위트 2923 passed, hcode 도메인 감사 critical 0, tsc OK.

### DEC-306 — 「재고금액」→「도서별재고금액」 + 재고 2화면을 원장관리로 이동(「재고관리」 그룹 철회) (2026-09-23)

- **요청** — 「재고금액: 도서별재고금액 명칭변경 및 원장관리 “기간별재고원장” 밑으로 위치이동 / 재고변경: 원장관리 “원장변경” 아래로 위치이동」.
- **변경** — `Sobo34_1_value` caption 「재고금액」→「도서별재고금액」(화면 제목·탭·브레드크럼 공통) + `menuGroup` stock→inventory,
  원장관리 레이아웃에서 `Sobo44_inv`(기간별재고원장) 바로 아래. `Sobo52_adjust`(재고변경)도 inventory 로 옮겨 `Sobo51_adjust`
  (원장변경) 바로 아래. 두 화면이 빠지면서 DEC-289 의 「재고관리」 그룹은 폼 0개 — `STOCK_SIDEBAR_LAYOUT`·`SIDEBAR_LAYOUTS.stock`·
  `MENU_GROUPS` 의 `id:"stock"` 항목을 제거(빈 그룹 자동 숨김 DEC-058 에 의존하지 않고 설정 자체를 정리). 라우트·권한·menuId(NAV-03) 불변.
- **검증** — Chrome 실화면: 사이드바 대메뉴 10개에 「재고관리」 없음, 원장관리 = 거래처거래원장·도서별수불원장·도서별원장총괄·
  기간별미수원장·**기간별재고원장·도서별재고금액**·통합 거래처원장 / 구분선 / **원장변경·재고변경**, 화면 제목·탭 「도서별재고금액」.
  가드 `test_stock_value_sobo34_1`(캡션·위치)·`test_dec137_ledger_axis_book_sales_menu`(그룹 철회·두 위치) 갱신. 스위트 2923 passed, tsc OK.

### DEC-307 — 「통합 거래처원장」 메뉴 감춤 + 도서별재고금액 기준율·반품재고 제로 칸 제거 (2026-09-23)

- **요청(캡처 2건)** — ① 원장관리 「통합 거래처원장」 메뉴 제거, ② (기존 재고금액 = 현 도서별재고금액) 「기준율, 반품재고 제로: 불필요」.
- **통합 거래처원장** — `INVENTORY_SIDEBAR_LAYOUT` 에서만 제거. 도서수불장 2종·기타거래처(Sobo15) 감춤과 같은 처리로
  **레지스트리·라우트(/ledger/customer-integrated)·등가 매트릭스는 보존** — 즐겨찾기/직접 링크는 그대로 열리고 커버리지 지표도 불변.
- **기준율·반품재고 제로** — 필터에서 두 칸(Edit109 / CheckBox3) 제거, 조회는 항상 `rate=100`(정가 100%)·`zeroReturn=false`
  로 보낸다(이전 세션 스냅샷의 값도 복원하지 않음 — DEC-292/296 숨은 필터 방지). Snap 에서 두 키 삭제, Enter 스톱에서 Edit109 제거
  (DEC-116 함정), 안내문 「거래일자를 설정한 뒤 조회」. 금액 컬럼·산식(정가×재고×기준율/100)·백엔드 파라미터는 불변.
- **검증** — Chrome 실화면: 원장관리 메뉴 8개(통합 거래처원장 없음), 도서별재고금액 필터 = 도서명 또는 코드·거래일자·조회,
  2026.09.21 조회 시 첫 행 「32 / 371,500 / 0 / 0 / 32 / 371,500」로 캡처와 동일(기준율 100% 유지). 가드
  `test_stock_value_sobo34_1` 위젯 목록 갱신(Edit109·CheckBox3 부재 + FIXED_RATE 고정) . 스위트 2923 passed, tsc OK.

### DEC-308 — 재고변경 = 레거시 Subu52 정품·비품 두 창(좌우) + 도서 확정 검색 + 도서명 편집 칸 (2026-09-24)

- **요청(캡처 3건)** — 「재고변경: 정상 작업이 안 되고 있습니다 / 카테고리를 전체 이미지와 동일하게, 지금은 너무 퍼져 있습니다」,
  「화면 분할은 좌우로 하더라도 거의 기존 재고변경 레거시 기능과 동일하게」, 「도서명도 편집박스 내에 표시(원본처럼 정품, 비품 재고변경 모두)」.
  덧붙여 도서별재고금액: 상단(도서구분) 정가 칸 제거(레거시도 합산 안 해 항상 공란) + 상·하단 「엑셀 저장」.
- **두 창** — DEC-282 에서 「상단 창만」으로 뺐던 하단 비품 창(Panel003/Panel004/DBGrid201)을 복원. 레거시 Button201Click =
  `Sg_Csum` `Scode='C'`, 원장재고 = DBGrid201KeyPress 의 Tong40 `Gbqut`(본사 반품재고) → 백엔드 축 `book_return`
  (`ledger_value="return_stock_asof"` = `inventory_service._fetch_return_stock_asof(axis_like='%A%')`). 화면은 같은 `AdjustmentPane`
  두 개를 `SplitListPanes orientation="horizontal"` 로 좌우 배치, 창마다 필터·검색·행 추가·저장이 따로(레거시 동일). 원장변경은 창 하나 그대로.
- **도서 확정 검색(레거시 Edit104→Seek40→Edit103)** — 검색칸 = 도서 룩업(인라인 Enter: 1건 자동/여러 건 팝업). 확정하면 즉시
  `gcode` 정확 일치로 조회(`list_adjustments(gcode=)` 신설)하고, 그 도서·조회 종료일·원장재고가 채워진 입력 줄(레거시 삽입행 `*`)을
  붙여 대조재고 칸으로 포커스. 이 줄은 `pristine` — 사용자가 건드리기 전엔 저장 대상이 아니다. 「행 추가」도 확정 도서로 시작.
- **도서명 편집 칸** — 도서 축은 도서명 칸이 룩업 입력(레거시 GNAME cbsEllipsis) — 도서명을 치고 Enter/검색으로 확정하면 코드·이름·원장재고가
  함께 들어간다. 코드 칸은 코드만(확정 시 이름 보충). 코드 칸에 이름을 친 채 저장하면 마스터에 없는 코드가 들어가던 경로는
  «새로 넣거나 바꾼 코드는 이름이 확인된 것만» 저장 검증으로 막았다(추정 원인 — 「정상 작업이 안 됨」 재현은 로그인 필요로 미확인).
- **폭** — 두 창이 반씩 쓰므로 컬럼 최소폭을 레거시 비율로 축소(거래일자 150·코드 90·도서명 220·수치 90·적요 140).
- **실데이터(교문사 5019, remote_153)** — Sg_Csum 1066행 전부 Scode A(비품 C 0건), 반품재고는 400종 중 1종만 비0 → 비품 창은 대부분 빈 목록이 정상.
  원장재고 자동값은 레거시 저장값과 «그 행 자체의 변경수량» 만큼만 차이(저장 후 재계산이라 자기 행 포함) — 신규 입력값은 레거시와 일치.
- **가드** — 신규 `test/test_dec308_stock_adjustment_two_panes.py`, `test_dec282`(축 3개), `test_dec290`(필터 줄 = `filterBar`) 갱신,
  스모크 매트릭스 `ledger.adjustments.book_return` 등록, 매핑 노트 `Sobo52_adjust.md` §6 갱신.
- **후속(같은 날) — 정렬·엑셀 저장 + 실화면 보정** — 사용자 「목록표 정렬 기능 추가, 엑셀 저장하기 기능 추가」. 조정 그리드 7컬럼 `sortable`
  (`useClientSort`). 편집 그리드라 **저장된 행은 서버 원본 값으로 정렬**(입력 중 행이 움직이지 않게), **미저장 새 행은 항상 맨 아래**
  (행 추가·입력 줄 포커스가 «마지막 행» 을 잡는 전제 유지). 「엑셀 저장」 = `gridRowsToExport(표시 컬럼−삭제 칸, 정렬된 행)` + 합계 행,
  파일명 `재고변경_정품_<기간>`. 원장변경도 같은 컴포넌트라 함께 적용.
  Chrome 실화면(교문사)에서 잡은 두 가지: ① 검색칸 Enter 한 번에 조회 3회 — 필터 줄 `advanceFilterOnEnter` 가 검색 버튼을 또 눌렀다 →
  도서 룩업 검색칸의 Enter 는 그 칸이 처리(확정=검색, 빈값=전체 검색)하고 필터 스톱 처리에서 제외(조회 1 + 원장값 1 로 확인).
  ② 거래일자 칸(달력 버튼)이 코드 칸을 덮어 「2988」→「88」 — 거래일자 최소폭 172. 확인: 2988/3046 확정 검색 → 입력 줄·원장재고 16 자동,
  대조 10 입력 → 변경 −6·미저장 1건(저장은 운영 DB 라 하지 않음), 대조재고 헤더 오름/내림 정렬, 엑셀 요청 본문(7컬럼·정렬 순서·합계) 가로채 확인.

### DEC-309 — 원장관리 「도서별원장총괄」 메뉴 감춤 (2026-09-24)

- **요청** — 「원장 관리에서 도서별 원장 총괄 메뉴 제거」.
- **변경** — `INVENTORY_SIDEBAR_LAYOUT` 에서 `Sobo36_book_summary`(DEC-285) 항목만 제거. 통합 거래처원장(DEC-307)·도서수불장 2종과
  같은 처리로 **레지스트리·라우트(/ledger/book-summary)·API·등가 매트릭스는 보존** — 직접 링크·즐겨찾기는 그대로 열리고 커버리지 지표 불변.
  (같은 줄에 붙어 있던 「도서별원장총괄」 라벨 주석이 기간별미수원장 줄에 잘못 달려 있던 것도 정리.)
- **검증** — Chrome 실화면(교문사): 원장관리 = 거래처거래원장·도서별수불원장·기간별미수원장·기간별재고원장·도서별재고금액 / 원장변경·재고변경,
  「도서별원장총괄」 없음. 가드 `test_dec285_book_summary_ledger` 를 «레이아웃에 없음» 으로 갱신(레지스트리 엔트리 가드는 유지).

### DEC-310 — 기간별재고원장 검색 줄을 거래처거래원장 디자인으로 통일 (2026-09-24)

- **요청(캡처 2건)** — 「기간별 재고원장 검색 라인을 다른 화면과 통일되게」, 「이런 형식으로 배열 및 검색 버튼 크기도 맞추기」(거래처거래원장 헤더).
- **변경** — 라벨 위 입력 4칸(도서명 또는 코드·시작일·기준일·조회) → 거래처거래원장(Sobo32)과 같은 한 줄:
  제목 옆(`filtersBelow={false}`) 「거래 일자 [시작 ~ 기준]」 알약 | 구분선 | 「도서명 | 코드 또는 도서명」 알약 + 「검색」
  (`h-10 min-w-24 rounded-2xl`). 관리자 출판사 칸도 같은 알약. 위젯 id(Sobo44.Edit101/102/103·dxButton1) 불변.
- **Enter 흐름** — 스톱 순서를 화면 순서(기간 → 도서 → 검색)로. 도서칸이 검색 버튼 바로 앞이 되어 확정 후 «다음 칸» 이 없으므로
  거래처거래원장처럼 **도서 확정 = 곧바로 조회**(인라인 Enter·팝업 공통), 빈값 Enter = 전체 조회. 필터 줄 Enter 처리가 검색 버튼을
  또 누르지 않게 도서칸은 제외(재고변경 DEC-308 과 같은 중복 조회 함정).
- **검증** — Chrome 실화면: 두 화면 헤더 캡처 비교(배치·알약·구분선·버튼 크기 동일), 3046 입력 Enter → 조회 1회, 현재고 16.
  신규 `test_dec310_inventory_status_search_line.py`(기준 화면과 같은 블록·Enter 순서·단일 조회), `test_dec200` 래퍼 가드를 data-enter-scope 로 갱신.

### DEC-311 — 거래처거래원장 엑셀 = 화면(값·모양) + 일자별 출고 상세에 반품수량·판매수량·판매금액 (2026-09-24)

- **요청** — 「화면출력과 엑셀 저장 값이 맞는지」 → 「일자별 출고 도서 상세 내역 엑셀도 화면과 동일하게」, 「우측 일자별 출고 상세보기 —
  반품수량, 판매수량, 판매금액 확인불가, 추가요청」(레거시 위러브솔루션 하단 표에 없던 판매부수·판매금액을 북이오웍스에 추가).
- **값 검증(변경 전)** — 1015 · 2026.01.01~09.21 에서 엑셀 요청 본문을 가로채 화면과 셀 단위 비교: 상단 26행+합계·하단 63행+합계 **불일치 0**.
  모양만 달랐다(화면 빈칸 → 엑셀 0 이 상단 86칸·하단 63칸, 천 단위 구분 없음).
- **엑셀 모양** — 범용 `/api/v1/export/table-xlsx` 컬럼에 `numFmt`(허용 목록 `#,##0` 등) → `masters_excel._write_list_sheet(number_formats=)`
  가 **숫자 셀에만** 서식 적용(값은 숫자 그대로 → 엑셀 합계·정렬 유지). 거래처거래원장은 `asScreenRow` 로 화면이 0 을 빈칸으로 그리는
  컬럼을 엑셀에서도 빈칸, 미수는 화면처럼 0 표시, 공급률은 소수 가능성 때문에 기본 서식. 다른 화면 export 는 `numFmt` 를 안 보내 불변.
- **상세 컬럼** — 백엔드 `_detail_split(qty, amt, is_rtn)` → `out_qty/rtn_qty/sale_qty/sale_amt`(단건·전체 상세 공통). 화면 순서는 상단 전표 표와 같게
  출고수량·출고금액·반품수량·반품금액·판매수량·판매금액·미수. 종전 「수량」(부호 있는 순수량 = 판매수량)은 중복이라 「출고수량」으로 대체.
  배포 순서 차이 대비 `withSplit` 이 구 응답을 보정(kind 2 또는 반품금액 존재 = 반품).
- **검증** — Chrome 실화면: 하단 합계 출고 269/8,902,130 · 반품 −120/−3,773,700 · 판매 149/5,128,430 · 미수 1,838,520 = 상단 합계와 일치,
  반품 행 −12/−570,240. 엑셀 본문을 «엑셀 표시 문자열» 로 바꿔 화면과 비교 — 상단 26행·하단 63행 불일치 0, 남은 0 은 화면도 0 인 미수 1칸뿐.
  신규 `test_dec311_customer_ledger_export_matches_screen.py`(분해·서식·빈칸·허용 목록·컬럼 순서).

### DEC-312 — 거래처거래원장 「일자별 출고 상세」 체크박스 → 다른 화면과 같은 「내용 전체 보기」 (2026-09-24)

- **요청** — 「타 화면에 존재하는 "내용전체보기" 기능을 이 화면에도, 디자인을 맞춰서」 + 추가 설명 「체크하면 모든 날짜의 세부내역,
  해제하면 선택된 항목의 세부 항목 — 다른 전체내용 보기와 동일」(레거시 툴바 「내용전체 / 내용각각」).
- **판단** — 기능은 DEC-287 에서 이미 있었고 라벨만 사용자 지정(2026-09-12) 「일자별 출고 상세」였다. 같은 기능을 두 개 두지 않고
  체크박스 이름·툴팁을 다른 화면(DEC-288)과 같게 「내용 전체 보기」로 통일. 체크 시 하단 제목 「일자별 출고 도서 상세 내역」은 유지.
- **검증** — Chrome 실화면: 체크 → 우측 63행(전표 19건) 「일자별 출고 도서 상세 내역」, 해제 후 2026.02.25 행 선택 → 「선택 전표 상세 조회」 1행.
  가드 `test_dec203`·`test_dec287` 라벨 갱신. 같은 날 전체 스위트에서 드러난 두 가드도 정리: `test_dec268`(DEC-310 기간별재고원장을
  제목 줄 필터 예외에 추가), `test_dec288`(재고변경 정품/비품 두 창은 목록-상세가 아니라 예외). 스위트 2942 passed.

### DEC-313 — 도서별수불원장: 검색칸 도서명·정가/ISBN 정돈·판매부수·엑셀 전 컬럼(=화면 모양) (2026-09-24)

- **요청** — 「도서검색: 출력화면은 코드보다 도서명이 중요」(+ 캡처 「입력박스에 코드가 아니라 도서명만, 기타 텍스트 출력 불필요」),
  「정가, ISBN 정보 조금 정돈된 느낌」, 「컬럼에 판매부수 추가」, 「엑셀 다운로드 시 컬럼 정보 모두 출력」.
- **검색칸** — 도서를 고르면 입력칸에 **도서명만**(`bookQuery`), 조회 키는 확정 코드(`bcode`) 별도(DEC-297 — 입력 글자를 조회 키로 안 씀).
  검색칸 옆 「도서명 · 정가 · ISBN」 흩어진 글자는 제거.
- **정가·ISBN** — 표 머리(SectionHeader) 제목 = 도서명, 메타 = 「코드 · 정가 ○○원 · ISBN · 기간」 라벨(흐림)+값(진하게) 쌍(`Sobo32.BookInfo`).
- **판매부수** — 상단(일자별)·하단(거래처별) 모두 반품 뒤, `출고 + 반품`(반품 음수 저장, 증정 제외 — 거래처거래원장 판매수량과 같은 정의), 합계 포함.
  실데이터 3226: 출고 706 + 반품 −161 = 545.
- **엑셀** — 컬럼 설정에서 숨긴 칸까지 **전 컬럼**(화면 순서) — 엑셀=표시 필드 규칙의 예외(거래처현황·입고처관리 DEC-292/294 와 같은 쪽).
  인쇄는 보이는 컬럼만. 모양은 DEC-311 방식을 공용 `screenLikeExport` 로(본문 0 → 빈칸, 수량·금액·재고 `#,##0`), **합계 행은 화면 합계줄처럼 0 표시**
  (거래처거래원장도 같은 보정). 파일명·인쇄 머리말은 도서명 먼저 `도서별수불원장_<도서명>(<코드>)`.
- **검증** — Chrome 실화면(3226): 검색칸 「현대인의 생활영양 3판」, 머리 「코드 3226 · 정가 22,000원 · ISBN … · 기간」, 판매부수 합계 545,
  엑셀 요청 본문을 엑셀 표시 문자열로 바꿔 화면과 비교 46행+합계 불일치 0. 신규 `test_dec313_book_ledger_sale_qty_export.py`,
  `test_dec203` 컬럼 목록 갱신. 스위트 2947 passed.

### DEC-314 — 원장변경·재고변경: 「행 추가」 → 표 아래 「라인 추가」 + 검색 줄 한 줄 알약 배치 (2026-09-24)

- **요청(캡처 2건)** — 「첫번째 이미지(라인 표 아래 「라인 추가」)를 참고해 원장변경에 행추가 버튼 대신 다른 화면과 동일한 방식으로,
  검색을 위한 입력 라인도 정렬을 변경해서 공간을 효율적으로」.
- **라인 추가** — 조정 원장 공용 `AdjustmentPane` 의 검색 줄 「행 추가」를 빼고 표 아래 왼쪽에 `variant="outline" size="sm"`
  「라인 추가」(주문·거래명세서 라인 표 `order-line-grid` 와 같은 자리·모양). 동작·위젯 id(`*.DBGrid101/201.INSERT`) 불변, 빈 표 안내문도 「라인 추가」.
- **검색 줄** — 거래처거래원장(DEC-310 기준) 알약: 「거래 일자 [시작 ~ 종료]」 | 「거래처명/도서명 | 입력」 검색 · 저장.
  원장변경은 제목 줄 오른쪽(`filtersBelow={false}`, DEC-268 예외 3), 재고변경 두 창은 창 안 한 줄(이름 알약 10~16rem·버튼 h-9 — 1423px 창에서도 한 줄).
  공용 컴포넌트라 재고변경에도 같이 적용.
- **검증** — Chrome 실화면: 원장변경 제목 줄 한 줄 + 표 아래 「라인 추가」 클릭 → 1행·포커스 거래일자, 재고변경 정품/비품 각 한 줄.
  가드 `test_dec290`(필터 줄에 추가 버튼 없음·표 아래 라인 추가)·`test_dec268`(예외) 갱신. 스위트 2947 passed.

### DEC-315 — 원장관리 세부 화면 한 줄 검색 통일 + 기간별미수원장 상/하단 자동 출력·내용 전체 보기 + 도서별재고금액 상단 구성 (2026-09-24)

- **요청(캡처 2건 + 후속 2건)** — 「검색 부분을 거래처거래원장과 동일하게, 원장관리 세부 화면들은 모두 동일한 레이아웃으로(한 줄 정렬)」,
  기간별미수원장 「상단:구분명, 하단:거래처명 자료가 자동 검색 출력되어야 하는데 상단만 출력」, 「이 화면에도 내용전체보기 기능」,
  「도서별재고금액 화면도 기간별재고원장 화면의 상단 레이아웃처럼 디자인을 통일」.
- **공용 조각** — `components/shared/ledger-search-line.tsx`(`LedgerSearchLine`/`LedgerDatePill`(범위·하루)/`LedgerSearchDivider`/
  `LedgerNamePill`/`LedgerSearchButton`/`SEARCH_PILL_INPUT_CLASS`) — 거래처거래원장 마크업·클래스를 그대로 옮긴 한 곳.
  적용: 기간별미수원장·도서별수불원장·도서별재고금액(제목 줄 오른쪽, DEC-268 예외 4). 거래처거래원장·기간별재고원장(DEC-310)·원장변경/재고변경(DEC-314)은
  이미 같은 모양(인라인). 메뉴에서 감춘 통합 거래처원장·도서별원장총괄은 대상 외. 검색 버튼 문구 「조회」→「검색」 통일, Enter 스톱 = 화면 순서.
- **기간별미수원장** — 조회 직후 화면 정렬 순서의 첫 구분 자동 선택(레거시: 하단은 상단 현재 행 = 첫 행을 따른다) → 상/하단이 함께 뜬다.
  「내용 전체 보기」는 이미 있었으나 **하단 표 머리**에 있어 하단이 뜨기 전엔 안 보였다 → 상단 표 머리로 이동(다른 원장 화면과 같은 자리).
  빈 구분 행 키 `(none)` ↔ 선택 상태 `""` 변환 누락(키보드 선택 시 하단 0건) 수정.
- **도서별재고금액** — 기간별재고원장과 같은 상단: 조회 전 `EmptyHint`, 조회 후 `SectionHeader`(제목·메타) + 액션 알약 「엑셀 다운로드」·「출력」(종전 「엑셀 저장」 아이콘),
  엑셀 수치 `#,##0`. 검색칸은 도서명 표시(DEC-313 규칙)·도서 확정=즉시 조회·빈값 Enter=전체(필터 줄 중복 클릭 제외). 「내용 전체 보기」인데 분류
  미선택이면 하단이 안내문으로 막히던 버그 수정.
- **검증** — Chrome 실화면: 세 화면 제목 줄 한 줄(거래 일자 알약 | 이름 알약 검색), 기간별미수원장 조회 → 첫 구분 「(구분 없음)」 선택·하단 14개 거래처,
  내용 전체 보기 체크 501개/해제 14개, 도서별재고금액 조회 전 안내문 → 조회 후 두 섹션 헤더·전체 보기 3,022종. 신규 `test_dec315_*`,
  `test_dec268`·`test_stock_value_sobo34_1` 갱신. 스위트 2953 passed.

### DEC-316 — 기간별재고원장 좌우 2단(도서구분별/도서별)·내용 전체 보기 + 구분 분할 원장 3화면 «조회 직후 하단 = 전체» 규칙 (2026-09-24)

- **요청(캡처 2건)** — 「기간별재고원장: 도서구분별 집계를 클릭하면 도서구분명으로, 해제하면 도서명 화면으로 바뀜. 좌우 화면으로 다른 원장과 동일하게」,
  「레거시에서 상하 구분되어 출력되는데 동일한 출력방식이 아니다. 다른 화면들과 같이 좌우 목록, 상세, 전체 등을 적용」.
- **레거시 근거(캡처 Subu34)** — 조회 직후 상단 = 분류별, 하단 = **전 도서**(레코드 1/3021, 하단 합계 474,054 = 상단 합계). 즉 하단 기본은 전체,
  분류를 고르면 그 분류(상세).
- **기간별재고원장** — DEC-205 단일 표 + 「도서구분별 집계」 토글(`Sobo34.ClassView`) 제거 → `SplitListPanes` 좌 도서구분별(`DBGrid101`)/우 도서별(`DBGrid201`, 페이저 유지).
  좌측 머리 「내용 전체 보기」(`Sobo34.ShowAll`)·엑셀·출력, 우측 머리 「구분 해제」·엑셀·출력(표마다, 보이는 컬럼, `#,##0`). 검색 줄은 공용 조각(DEC-315)으로.
- **공통 규칙(기간별재고원장·도서별재고금액·기간별미수원장)** — 조회 직후 하단/우측 = 전체(구분 미선택), 구분 클릭 = 그 구분만, 「구분 해제」 = 전체로,
  「내용 전체 보기」 = 선택과 무관하게 전체. 하단은 항상 표시(`secondaryVisible` 제거). DEC-315 에서 넣은 «첫 구분 자동 선택»은 레거시와 달라 철회.
- **빈 구분코드 키** — 구분코드 "" 행은 DataGrid 가 키 없음으로 보고 선택 강조를 못 했다 → `(none)` ↔ "" 변환(두 재고 화면), 제목 「(구분 없음)」.
- **검증** — Chrome 실화면: 기간별재고원장 조회 → 우측 「도서별 (전체)」 3,022종·전재고 합계 481,519 = 좌측 합계, 식품영양학 클릭 → 571종, 구분 해제 → 전체,
  빈 구분 행 선택 강조·「(구분 없음)」 38종. 기간별미수원장 조회 → 「거래처별 (전체)」 501개, 도서별재고금액 → 「전체 · 3,022종」.
  가드 갱신: `test_dec205`(단일 표 규칙 → 좌우 2단), `test_dec200`·`test_dec310`(공용 조각), `test_dec288`(기간별재고원장 추가), `test_dec315`(자동 선택 철회). 스위트 2953 passed.

### DEC-317 — 원장변경 검색칸·코드 칸 거래처 자동검색 + 확정 후 거래처명 표시 (2026-09-24)

- **요청(캡처)** — 「원장변경 화면에 해당 컨트롤 자동검색 기능 등이 작동하지 않는다. 검색결과는 코드가 아니라 거래처명으로」.
- **원인** — 조정 원장 공용 화면에서 검색칸 룩업(`MasterLookupField` 인라인 자동완성)이 도서 축(재고변경)에만 켜져 있었고, 원장변경은
  평문 `<Input>`(글자 부분일치 필터)이었다. 그리드 코드 칸도 원장변경은 팝업만(인라인 자동완성 꺼짐).
- **변경** — 검색칸 룩업을 두 축 공통으로: 입력 → 자동완성, Enter 1건 자동·여러 건 팝업 → 코드 확정(레거시 Edit103) → 곧바로 `gcode` 정확 일치 검색,
  확정 후 칸에는 **거래처명**, 빈값 Enter = 기간 전체, 확정 검색 시 그 거래처가 든 입력 줄(pristine) 추가(재고변경 DEC-308 과 동일).
  원장변경 코드 칸도 인라인 자동완성(확정 시 코드·거래처명 함께). 거래처 인라인 항목의 코드는 **`hcode` 필드**(C2 자동완성 API 명명 — 메모리 MLF 함정)라
  `inlineItemCodeName(kind, item)` 로 축별 추출. 자리표시 「코드 또는 거래처명/도서명」.
- **검증** — Chrome 실화면: 「홍익」 입력 → 자동완성 목록, 선택+Enter → 칸 「[X]하웅문고[홍익대 세종캠]」·검색 1회·입력 줄(2295·거래처명)·대조금액 포커스·저장 비활성(pristine),
  라인 추가 → 코드 칸 1015 Enter → 거래처명 「홍익대[서울]대학서적」(저장은 운영 DB 라 하지 않음). 가드 `test_dec308` 추가. 스위트 2953+1 passed.
- **후속(같은 날, 캡처)** — 「거래처명도 입력박스에 넣어서 보여주세요」 → 원장변경 축도 `searchLookup: true`: 그리드 거래처명 칸 = 룩업 편집 칸
  (재고변경 도서명 DEC-308 과 동일, 코드 칸은 코드 입력), 이름 칸 확정 시 거래처 항목 코드 `hcode` 추출(`inlineItemCodeName`).
  Chrome: 라인 추가 → 거래처명 칸 1015 Enter → 코드 1015·칸 안 「홍익대[서울]대학서적」·대조금액 포커스.
  (자동화 도구의 한글 IME 입력은 조합 잔여 글자가 확정 뒤에 붙는 인공 현상 — 실제 키보드는 조합 중 Enter 무시(isComposing)라 해당 없음.)

### DEC-318 — 룩업 Enter: 다건 중 명칭 정확 일치 1건 자동 확정 + 확정된 칸 Enter 재검색 없음 (2026-09-24)

- **요청(캡처)** — 원장변경 입력 시 「이미 거래처명이 명확하게 검색되어 있는데 다시 검색창이 뜬다. 검색결과 명확하게 맞는게 있으면 검색창이 뜨면 안 된다」
  (예 「(주)교보문고」 — 자동완성 결과 2건: 00001 (주)교보문고 / 90063 [X]단행-(주)교보문고).
- **원인 2가지** — ① DEC-134 는 결과 **1건**일 때만 자동 확정 → 명칭이 정확히 같은 항목이 있어도 다른 부분일치가 있으면 팝업.
  ② 확정 후 칸에 이름을 보이게 한 화면(DEC-313/317)에서 그 칸에 다시 Enter 하면 이름 글자로 재검색 → 팝업.
- **변경(`MasterLookupField`)** — ① 다건이어도 **명칭 전체가 검색어와 같은 항목이 정확히 1개**면 확정(팝업 없음). 코드 정확 일치 다건은 종전대로 팝업
  (DEC-134 한글 단어형 도서코드 「기계」 오확정 방지 유지). ② `confirmed` prop — 칸 글자가 확정된 선택이면 Enter 는 재검색 없이 `onKeyDown`(화면 검색 실행)
  또는 다음 칸. 적용: 원장변경/재고변경 검색칸(확정 코드 = 그 코드로 재검색)·그리드 이름 칸(다음 칸), 도서별수불원장·기간별재고원장·도서별재고금액 도서칸(재조회).
- **검증** — Chrome: 원장변경 라인 추가 → 거래처명 칸 「(주)교보문고」 Enter → 팝업 없이 00001 확정·대조금액 포커스, 같은 칸 재-Enter → 팝업 없이 대조금액.
  가드 `test_dec134`(명칭 정확 1건·confirmed·코드 다건 자동확정 금지). 스위트 2955 passed.

### DEC-319 — 통계 도서별판매·거래처별판매 「내용 전체 보기」 공통 동작 통일 (2026-09-24)

- **요청(캡처)** — 「좌측 목록에서 세부항목을 보여주는 게 우측 창. 도서별판매는 "내용전체보기"로 창이 열렸다 닫혔다, 거래처별판매는 해당 기능이 안 보인다.
  공통기능으로 합리적인 기능으로 통일」.
- **통일 규칙(원장 DEC-316 과 동일)** — 조회하면 우측은 곧바로 **전체**, 좌측 행을 고르면 그 행만, 「선택 해제」 = 다시 전체,
  「내용 전체 보기」 = 선택과 무관하게 전체(행별 식별 컬럼 포함). 우측 창은 항상 표시(`secondaryVisible` 제거 — 열렸다 닫혔다 없음).
- **도서별판매** — `effectiveAll = showAll || detail === null`: 선택이 없으면 기존 전 건 경로(`bookSalesCustomersAll`, 목록 도서 전부의 거래처별)를
  그대로 쓴다. 우측 「닫기」 → 「선택 해제」, 선택 안내 칸 제거.
- **거래처별판매** — 이미 레거시 T00=1(조회 직후 하단 = 전체 거래처 도서별)이었다. 「내용 전체 보기」가 **좌측 표 도구줄**(페이저·엑셀·컬럼설정과 한 줄)에
  있어 좁은 창에서 눌려 가려졌다 → 도서별판매와 같은 **두 창 위 한 줄**로 이동. 우측 「선택 해제」 추가(→ 전체 거래처 재조회).
- **검증** — Chrome 실화면: 도서별판매 조회 → 우측 「도서별 거래처 상세 내역 · 도서 100종 · 121행」(체크 없이), 행 선택 → 그 도서 1행, 선택 해제 → 121행.
  거래처별판매 → 체크박스·엑셀 저장이 두 창 위 줄에 보임, 우측 「전체 거래처」, 행 선택 → 영진전문대[대구]북앤북스, 선택 해제 → 전체 786행.
  가드 `test_dec319_stats_show_all_unified.py` 신설, `test_dec210`·`test_dec288` 갱신. 스위트 2958 passed.

### DEC-320 — 통계관리 하위 화면 한 줄 검색 통일 + 모든 목록표 「엑셀 저장」 (2026-09-24)

- **요청(캡처 2건)** — 「거래처거래원장 검색 요소 레이아웃처럼 도서별판매 같은 통계관리 하위 화면들의 레이아웃을 통일하고, 모든 목록표에 대해서
  엑셀다운로드·엑셀저장 등을 없으면 추가하고 있으면 엑셀저장으로 통일 — 통계화면 하위 공통 요청」.
- **대상 11화면** — 도서별판매·거래처별판매·도서별년말집계·월별통계·거래처통계(목록)·도서통계(목록) (자체 필터) +
  기간별 매출 분석·거래처별 판매 분석·도서 회전율·분기/반기 손익·출판사통계 (공용 `StatsFilterBar`). 년/월(통계)는 링크 허브(검색·표 없음)라 제외.
- **검색 줄** — 원장관리 공용 조각(`ledger-search-line`, DEC-315)으로 제목 줄 오른쪽 한 줄(`filtersBelow={false}`, DEC-268 예외 5):
  「거래 일자 [시작 ~ 종료]」 | 이름·범위 알약(거래처/도서 코드 범위는 한 알약 안 시작 ~ 끝) · 집계/년/분기/비교/SCode 등은 라벨 알약 · 「검색」.
  공용 `StatsFilterBar` 는 한 번 고쳐 5화면에 적용(카드형 → 알약, 필터 바를 PageHeader 안으로), Enter 스톱 순서도 화면 순서(계정 → 기간 → 범위 → 집계·분기 → 검색)로.
  위젯 id 전부 유지. 버튼 문구 「조회」→「검색」.
- **엑셀 저장** — 통계 화면 라벨은 전부 「엑셀 저장」(원장관리는 「엑셀 다운로드」 유지 — 이번 요청 범위 밖). 없던 표: 도서별판매 우측 거래처별 내역
  (`Sobo61.Button_ExportDetail`)·거래처별판매 우측 도서별 상세(`Sobo62.Button_ExportDetail`) — 화면 보이는 컬럼·정렬·합계, 수치 `#,##0`;
  분기/반기 손익 분기 비교 표(`Button_ExportCompare`, 시트1 분기 비교 + 시트2 월별 같은 파일).
- **작업 방식** — 공용 필터 바·필터 바 5화면·분기 비교 엑셀은 직접, 자체 필터 6화면은 보조 에이전트 3개(파일 2개씩, 테스트·커밋 제외)로 병렬 후 통합 검증.
- **검증** — Chrome 실화면: 11화면 제목 줄 캡처(거래 일자 알약 | 범위/이름 알약 · 집계/분기 알약 · 검색), 기간별 매출 분석 검색 → 32행,
  도서별판매 조회 후 엑셀 저장 2개(상단·우측). 신규 `test_dec320_stats_search_line_excel.py`(11화면 한 줄·필터 바 알약·Enter 순서·표마다 엑셀 저장·「엑셀 다운로드」 없음),
  `test_dec140`·`test_dec268` 갱신. 스위트 2961 passed.

### DEC-321 — 통계 2표 화면: 표마다 제목 줄에 그 표의 버튼(어느 표의 엑셀 저장인지) + 거래처별판매 「전체」·「지점별검색」 확인 (2026-09-24)

- **요청(캡처 2건)** — 도서별수불원장처럼(표 제목 줄에 내용 전체 보기·엑셀·출력) 되어야 하는데, 도서별판매는 「내용 전체 보기 · 엑셀 저장」이
  두 창 위에 떠 있어 «어떤 엑셀저장인지 보기 어렵다».
- **변경** — 도서별판매·거래처별판매: 두 창 위 공용 줄 제거 → 좌측 표 `SectionHeader`(「도서별 판매 목록」/「거래처별 판매 목록」 · 기간 · 건수)에
  내용 전체 보기·컬럼 설정·엑셀 저장, 우측 표도 같은 `SectionHeader`(선택 해제·컬럼 설정·엑셀 저장). 분기/반기 손익: 「분기 비교」·「월별 손익」 각각 제목 줄.
  버튼은 원장 화면과 같은 알약(`SECTION_ACTION_BUTTON_CLASS`), 라벨 「엑셀 저장」(DEC-320) 유지.
- **상단 페이저** — 이미 전 화면에서 제거된 정책(`DataGridPager variant="full"` = 0×0 자리표시자, 페이지 이동은 표 하단)이라 공용 줄과 함께 걷음.
  낡은 가드 `test_stats_optional_paging`(«표 위 페이저») 를 현 정책(표 하단 pager prop)으로 갱신.
- **거래처별판매 「전체」·「지점별검색」 확인(사용자 질문)** — 「내용 전체 보기」와 **중복 아님**, 유지:
  「전체」 = 거래처 **필터**(체크 시 거래처명 칸 무시하고 전 거래처 조회, 칸에 입력하면 자동 해제 — 빈 칸과 같은 효과),
  「지점별검색」(레거시 Subu62 CheckBox1) = **좌측 목록 묶음 단위**(끔 = 거래처 합산, 켬 = 거래처×지점 행),
  「내용 전체 보기」 = **우측 상세 표시**(목록 거래처 전부의 도서별을 거래처별로).
- **검증** — Chrome: 도서별판매 「도서별 판매 목록 · 785종 [내용 전체 보기][엑셀 저장]」 | 「도서별 거래처 상세 내역 · 100종·121행 [엑셀 저장]」,
  거래처별판매 「거래처별 판매 목록 · 202곳」 | 「도서별 상세 · 전체 거래처」, 분기/반기 손익 「월별 손익 · 2026년 3분기 [엑셀 저장]」.
  `test_dec319` 가드를 «표마다 머리» 로 갱신. 스위트 2961 passed.

### DEC-322 — 도서별판매·거래처별판매 「전체」 체크박스 제거(지점별검색 유지) (2026-09-24)

- **요청(캡처)** — 「전체 등 기능에 대해서 불필요한 전체는 제거, 지점별 등 필요 기능은 남겨줘」(DEC-321 확인 결과: 「전체」 = 이름 칸 비움과 같은 효과).
- **변경** — `Sobo61.Chk_AllBooks`·`Sobo62.Chk_AllCustomers` 체크박스·Enter 스톱 제거. 상태 `bookAll`/`customerAll` 은 파생값(`!코드.trim()`) —
  칸이 비면 전체. 구 세션 스냅샷의 `bookAll/customerAll: true`(코드 무시)는 빈 칸으로 복원해 동작 불변. 거래처별판매 「지점별검색」(`CheckBox1`, 레거시 Subu62)은 유지.
- **검증** — Chrome: 거래처별판매 검색 줄 = 거래 일자 | 거래처명 · 지점별검색 · 검색(전체 없음), 빈 칸 조회 202곳(종전과 동일),
  도서별판매 = 거래 일자 | 도서명 · 검색, 빈 칸 조회 785종. 가드 `test_sales_team_a_batch` 갱신. 스위트 2961 passed.

### DEC-323 — 도서별년말집계: 검색 통일·SCode/집계 단위/하단 상세(드릴다운) 제거·컬럼명 통일·판매 추가·엑셀 전 필드 (2026-09-24)

- **요청(캡처 + 후속)** — 「검색창 디자인 및 기능 통일 / Scode 필터, 집계 단위, 하단 상세내역: 불필요 / 컬럼명 통일: 년도, (코드), 도서명, (ISBN), 정가,
  입고수량, 출고수량, 증정수량, 반품수량, 판매수량, 출고금액, 반품금액, 판매금액」, 「엑셀 저장 시 모든 필드 저장 기본, () 부분 기본」,
  「첨부 이미지에 제거 요청한 컨트롤 2가지 제거」, 「(드릴다운) 이전 화면으로 되돌아갈 방법이 없다」.
- **검색** — 「거래 월 [시작 ~ 종료]」 | 「도서명」 한 칸(다른 통계 화면과 같은 동작: 칸에 도서명, 확정 = 그 도서 1종(시작=끝) 곧바로 조회, 비우면 전체,
  확정 칸 Enter = 재조회 DEC-318) · 검색. SCode(`CheckBox2`)·집계 단위(`GrainGroup`)·도서코드 끝(`Edit105`) 제거, 년 집계 고정(백엔드 grain 파라미터는 유지).
- **드릴다운 제거** — 행 클릭 → 월별(하단 상세) 전환·「년도 표로」 삭제. 배포본에서 드릴다운 뒤 돌아갈 길이 없던 문제도 함께 해소(구 세션의 month/parentBcode 스냅샷은 무시하고 년 표).
- **컬럼** — 요청 순서·명칭 그대로, 폐기수·폐기액 제외, 판매수량(출고+반품)·판매금액(출고액+반품액) 추가(반품 음수 저장 — 도서별판매와 같은 정의).
  괄호(코드·ISBN) = 기본 표시(DEC-292 사용자 정의) — 컬럼 설정 저장 키 `reports.year-end-book.v2` 로 올려 전 컬럼 기본 표시.
  백엔드 `_YEAR_END_BOOK_SORT_KEYS` 에 sale_qty/sale_amt 추가·행에 적재 → 판매 컬럼도 전체 정렬(정렬 가능 컬럼 수 종전과 같은 11).
- **엑셀** — 컬럼 설정에서 숨긴 칸까지 전 컬럼(화면 순서, `allColumns`), 판매 파생은 서버 `_YEAR_END_DERIVED_FIELDS` 가 계산. 표 제목 줄(DEC-321)에 엑셀 저장.
  엑셀=표시 필드 규칙의 예외(거래처현황·입고처관리·도서별수불원장과 같은 «전 필드» 쪽).
- **검증** — Chrome: 헤더 13컬럼 요청 순서 일치, 2,133행, 행 클릭 드릴다운 없음, 판매수량 헤더 정렬(내림 4,925 → 4,714 …), 엑셀 요청 columns 13개.
  신규 `test_dec323_year_end_book_simplified.py`, `test_dec142`·`test_dec198` 갱신, grid 기준선 재생성. 스위트 2966 passed.

### DEC-324 — 거래처별 판매 분석: 차트 = 표 순서·상위 N 선택·수량/금액 이중 축·막대별 라벨 (2026-09-24)

- **요청(캡처 + 후속)** — 「거래처코드 1개 조회는 거래처거래원장으로 가능 → 여러 거래처 비교분석이면 1) 막대그래프 및 정보 위치랑 일치하게 조정,
  2) 상위 10위만 출력되어 있는데 이 조건은 선택으로」, 「차트에 수량도 보일 수 있도록 범례 조정」. (검색창 디자인은 DEC-320 으로 완료.)
- **원인** — ① 차트는 현재 페이지를 **금액으로 따로 재정렬**해 상위 10을 그렸고 표는 최종거래일 순 → 막대 1번 ≠ 표 1행.
  recharts 기본 X 라벨이 겹치면 건너뛰어 막대와 라벨 위치가 어긋나 보였다. ② `TOP_N = 10` 상수. ③ 수량·금액이 한 축이라 금액(수천만)에 눌려 수량 막대가 바닥.
- **변경** — 막대 = **표의 현재 정렬 순서 앞 N행**(막대 1번 = 표 1행, 헤더 정렬을 바꾸면 차트도 따라감), 기본 정렬 = 출고금액 내림차순(서버 정렬).
  차트 머리 「표시」 선택 10/20/30/50/전체(현재 페이지), 제목 「거래처 비교 — 표 순서(정렬 기준) 상위 N곳」.
  공용 `StatsBarChart` 옵션 추가(다른 화면 불변): `secondaryAxisThousand`(금액 오른쪽 축·천원), `showAllLabels`(막대마다 라벨, 기울임·말줄임). `ChartCard` 에 `actions`.
  recharts 3.8 에서 막대 애니메이션이 끝나지 않아 막대가 비는 현상 → 막대 `isAnimationActive={false}`(통계 막대 차트 공통).
- **검증** — Chrome: 조회 → 표 1~3행 (주)교보문고·(주)예스이십사·납품거래처 = 막대 1~3번, 수량(왼쪽 축)·금액(오른쪽 축) 막대 모두 표시(20개),
  상위 20 → 40막대, 전체 → 200막대(100행). 신규 `test_dec324_customer_analysis_chart.py`. 스위트 2968 passed.

### DEC-325 — 도서 회전율 산식 = 판매수량 ÷ 평균재고(회) + 판매·재고 컬럼 + 차트=표 순서·상위 N 선택 (2026-09-25)

- **요청(캡처)** — 「하단에 "판매" 컬럼 추가 / 회전율 %는 모두 0.0000 — 수식 반영 확인 / 여러 도서 비교분석이면 막대그래프 및 정보 위치 일치 /
  상위 10위만 → 선택」. 산식은 사용자 선택(AskUserQuestion): **판매 ÷ 평균재고(회)**.
- **원인** — 종전 회전율 = 출고 ÷ 입고, 입고 0 이면 0 → 기간(1개월) 안 입고가 없는 대부분 도서가 0.0000.
- **산식** — 회전율(회) = 판매수량 ÷ ((기초재고 + 기말재고) ÷ 2). 판매수량 = 출고 + 반품(음수 저장), 판매금액 = 출고액 + 반품액.
  기초 = 시작일 전일, 기말 = 종료일 정품재고 — `reports_service._fetch_stock_asof`(기간별재고원장·도서별수불원장과 같은 3소스). 평균재고 ≤ 0 → null(공란, 정렬 맨 뒤).
  전체 회전율 = Σ판매 ÷ Σ평균재고. 실데이터(교문사 08.25~09.24): 3428 판매 651·기초 0·기말 393 → 3.31회, 전체 0.06회(총재고 ≈ 35만).
- **화면** — 컬럼 판매수량·판매금액·기초재고·기말재고·회전율(회)(소수 2자리) 추가, 기본 정렬 회전율 내림차순, 요약 「총 판매수량」「전체 회전율(회)」.
  차트 = 표 순서 앞 N행(DEC-324 규칙), 표시 10/20/30/50/전체, 회전율(왼쪽 축)·판매수량(오른쪽 축, `secondaryAxisRaw` 신설), 막대별 라벨.
  공용 `_sort_items` 는 None 값을 방향과 무관하게 맨 뒤로. 엑셀 기본 컬럼 목록에도 판매·재고·회전율(회).
- **검증** — 서비스 실데이터 호출(위 수치), 신규 `test_dec325_book_turnover_formula.py`(산식·판매·공란 정렬·화면 컬럼/차트),
  `test_dec169`·`test_stats_reports_sort_params` 재고 목 추가. 스위트 2971 passed. 화면 캡처는 로그인 만료로 재로그인 후 확인 예정.

### DEC-326 — 분기/반기 손익 점검: 월 단위 청구·입금 원천 병합 + 분기 추세선 + 도서별 판매 기여도 + 청구 구성 파이 (2026-09-25)

- **요청** — 「분기/반기 분석 화면도 개선·공통 수정 확인 / 분기 비교는 막대 외 선 그래프 — 추세선 / 도서별 기여도 등 추가 정보」.
- **데이터 버그(교문사 5019)** — ① 청구: 기간에 T2_Ssub 행이 하나라도 있으면 전 기간을 T2 로 채워, 자리표시 T2 행(0)이 있는 달(2026.03·07~09)은
  0, T2 행 없는 달(2026.01~02)은 누락. → **월 단위**: T2 Sum28 ≠ 0 이면 그 값, 아니면 출고−반품 실시간 파생(`settlement_service.live_period_summary`).
  ② 입금: T5_Ssub 0행이라 전 분기 입금 0. → 월 단위 «T5 값 우선, 없으면 H1_Ssub 수금(입금−출금)»(`receipts_by_month_h1`, 거래처거래원장과 같은 원천).
  분기 월별 표는 분기의 모든 달을 채운다(자료 없는 달 0).
- **대사** — 레거시 입출금전표-거래처(2025.09.25~2026.09.25) 합계칸 「248,533,317」 = 조회 상한 **2000행** 합 2,248,533,317 의 앞자리 「2,」가
  칸 폭에 잘린 표시. 기간 전체는 2,550행 2,726,389,386. 웹 월별 입금(예: 2025.10 = 223,765,946)과 H1_Ssub 월 합계 일치.
- **화면** — 분기 추세 선그래프 `StatsTrendLineChart`(청구·입금·손익 실선 + 최소제곱 점선 추세선, linear), 월별 막대 `showAllLabels`,
  파이는 「청구 구성(입금·잔액)」 — 종전 「손익(+)」 조각은 잔액(=청구−입금)과 같은 값의 중복이라 제거, 라벨 천단위+비중(%), recharts 파이 `isAnimationActive={false}`(빈 파이).
  「도서별 판매 기여도」 표(순위·도서·판매수량·판매금액·기여도%·누적%) 상위 50 + 엑셀 저장 — 전 구간 1회 `GET /api/v1/stats/book-contribution`
  (판매 = 출고+반품, 도서별판매 `get_book_sales` 동일 집계, 분모 = 판매금액 순합). 요청의 「지급 기여도」는 판매 기여도로 해석.
- **검증** — 실데이터: 청구 Q4 347,153,361 · Q1 1,748,198,127 · Q2 −69,550,377(반품 시즌) · Q3 1,022,119,758, 입금 590.6M·1,023.6M·545.1M·503.3M,
  Q3 기여도 합계 × 1.1 = Q3 청구. Chrome: 추세선 6개·파이 2조각·월 라벨 12개·기여도 50행. 신규 `test_dec326_quarterly_summary_review.py`,
  `test_dec085` 월 채움 반영. 스위트 2978 passed. probe `stats.book_contribution` 등록.

### DEC-327 — 월별통계 용어 직관화 + 차트 5종 (2026-09-25)

- **요청** — 「컬럼명 날짜/매출수량합계/매출금액합계 — "트렌드"는 어떤 의미?」 → 「직관적 용어로 변경」, 「다양한 차트 형식과 함께」.
- **용어** — 수량합/금액합 → **매출수량 합계/매출금액 합계**(표 머리·하단 합계). 「트렌드」는 증감 추세가 아니라 금액 비교 막대였으므로
  **「매출 비교 (최고 월=100%)」** 로 바꾸고 막대 옆에 비율(%)을 표시(기준 = 현재 표의 최대 월, 음수 달은 막대 없이 비율만).
  막대 열은 화면 보조 표시라 엑셀 저장 컬럼에서 제외(`test_dec198` 가드는 `.filter(c.id !== "trend")` 허용으로 갱신).
- **차트** — 표 위 「월별 매출 차트」 카드 + 「차트」 선택(뷰어별 localStorage 기억): 매출금액·수량(막대, 수량 오른쪽 축) /
  매출 추세선(선 + 최소제곱 점선) / 전월 대비 증감(막대, 음수 아래) / 누적 매출(면적, 신규 `StatsCumulativeAreaChart`) /
  월별 매출 비중(원형 — 상위 7개월 + 「기타 N개월」, 라벨=월(비중%), `StatsPieChart labelKind="name"` 신설). 표 정렬과 무관하게 월 오름차순.
- **검증** — Chrome(교문사 2025.09~2026.09, 13개월): 5종 모두 렌더(막대 26·추세선 2·증감 12·면적 1·원형 8조각), 선택 기억 확인.
  신규 `test_dec327_monthly_stats_labels.py`. 스위트 2982 passed, grid baseline OK.

### DEC-328 — 거래처통계(목록) 컬럼 재구성·반품율·총합 (2026-09-25)

- **요청(문서 캡처)** — 「(예상) 위러브솔루션 거래처/구분별판매 자료 / 검색창 타 카테고리와 동일(→ DEC-320 기 적용, 제외) /
  컬럼: 구분·출고수량·출고금액·증정수량·반품수량·반품금액·수금액·판매부수·판매금액·반품율 (최종거래일 — 화면 미출력·엑셀 출력) / 하단부 총합」.
- **반품율 정본** — 레거시 `Subu69_2.pas`(메뉴 「거래처판매율」 Menu609_2 → TSobo69_2) L602~621:
  행 = −반품수량 ÷ 출고수량 × 100(출고 0 → 0), Footer = Σ(−반품) ÷ Σ출고 × 100(행 평균 아님). 웹은 소수 1자리.
  `reports_service._return_rate` · 행 `return_rate`(정렬 화이트리스트 추가) · totals `return_rate`.
- **화면** — 컬럼 순서 거래처코드·거래처명·구분(G1_Gbun 이름, 페이지 행 부착이라 정렬 불가)·출고수량·출고금액·증정수량·반품수량·반품금액·
  수금액·판매부수·판매금액·반품율(%). 최종거래일은 `EXPORT_ONLY_COLUMNS` 로 엑셀 끝에만. 기본 정렬 최종거래일→**거래처코드 오름차순**
  (보이지 않는 열 기준 정렬 회피). 하단 「총합」 행(DataGrid totals).
- **버그 수정** — `stats_service.get_customer_analysis` 합계가 현재 페이지 행만 더하던 것을 검색 결과 전체(`get_customer_sales` totals)로.
  `CustomerSalesResponse.totals` 타입 `dict[str,int]` → `dict[str,int|float]`(반품율 소수로 거래처판매 화면 검증 실패 방지).
- **검증** — Chrome(교문사 2026.06.25~09.25, 226곳 3페이지): 교보문고 출고 6,095·반품 −212·판매부수 5,883·반품율 3.5,
  총합 출고 38,017·증정 2,272·반품 −942·수금 509,400,888·판매 924,956,090·반품율 2.5(=942÷38,017). 엑셀 요청 가로채기 200 +
  컬럼 13개(최종거래일 끝). 거래처판매(/reports/customer-sales) 합계 정상. 신규 `test_dec328_stats_customer_list.py`,
  `test_dec197` 합계 키 갱신. 스위트 2988 passed.

### DEC-329 — 년/월(통계) 세분화 판매 6종 이관: 공용 매트릭스 서비스·화면 + 설정 (2026-09-25)

- **요청** — 「레거시 년/월 통계의 세분화 화면(도서별·거래처판매 × 월별/일별/년·월)은 어디에 반영됐나」 → 미이관 확인 → 「제안 방식으로 6개 모두 진행」.
  출판 빌드 전용 폼이라 폼↔화면 매트릭스(유통 트리 기준)·ACC-MENU-YM 계획에서 빠져 있었다(월별·일별 4종은 계획에도 없음).
- **레거시 정본** — `WeLove_FTP/도서유통-출판/MySQL/Subu79_1~4.pas`, `도서유통-출판/Subu73·74.pas`:
  · 79_1/2(월별)·79_3/4(일별): 「출력조건」 1개(판매·출고·반품·증정·폐기·입고·판매유형별 수량/금액 25종)를 월·일 열로 피벗.
    조건별 WHERE(도서축 Scode≠Y·(Z 비폐기|X|Y), 거래처축 Scode=X/Y 등)를 `_book_measure_where`/`_customer_measure_where` 로 그대로 이식.
  · 73/74(년/월): 기간 ≤3개(YYYY·YYYY.MM, 년도엔 1/4~4/4분기·1~2/3~4분기 — Seek_Date) × 출고·반품·판매(=출고+반품)·판매금액.
    74 는 「수금조회」(H1_Ssub 입금−출금, 입고처는 반대)를 기간별 **수금액 열**로 상시 표시(판매 없는 수금 거래처도 행).
  · 보기(ComboBox1): 도서 = 분류/저자 × 도서 표시, 거래처 = 구분/지역(Jubun)/담당(Gpper) × 거래처 표시. 「본사출고제외」 = 00001 출고 제외.
- **설계** — `app/services/sales_matrix_service.py` 한 곳(`GET /api/v1/stats/sales-matrix`, admin.stats.sales, enforce_hcode_isolation)
  + `components/stats/sales-matrix-screen.tsx` 한 화면 + 라우트 6개(설정만, DEC-028 코드 분기 금지):
  `/year-month-stats/{book,customer}-sales-{monthly,daily}`, `/year-month-stats/sales-by-{book,customer}-monthly`(ACC-MENU-YM-03/04 경로).
  form-registry 6건(Sobo79_1~4·Sobo73·Sobo74, 통계관리 메뉴), 「년/월(통계)」 허브 링크, 매트릭스 보조 루트에 `도서유통-출판` 추가 → 6건 MATCH.
- **레거시와 다른 점(의도)** — ① 레거시 열은 «월 번호/일 번호»라 해·달을 넘기면 같은 번호가 합쳐졌다 → 실제 연월·일자 순 열,
  상한 월별 24개월·일별 31일(같은 해/같은 달 조회는 동일 결과). ② 도서구분(본사/창고, Ocode) 토글은 chul_09 Ocode 혼재(DEC-084)로 미이관.
  ③ 거래처구분/도서분류 교차 필터는 API(custClass/bookClass)만 — 화면은 단일 거래처/도서 교차 필터.
- **화면** — 두 줄 검색(1줄 기간·출력조건·보기·검색 / 2줄 코드 범위·교차 필터·본사출고제외), 본문 0 빈칸·합계 행,
  클라이언트 정렬·컬럼 설정, 엑셀 = 화면 컬럼·순서 + 합계 행(#,##0, 본문 0 빈칸).
- **검증** — 실데이터(교문사 5019, remote_153): 도서×월 1,125행(2026.01~09, 합계 91,999, 2초) · 거래처×월 286행 · 도서×일 · 거래처×일 164행 ·
  도서 년/월(2025 vs 2026 1~2분기) 1,294행 · 거래처 년/월 342행(2025 수금 2,979,920,064). Chrome 6화면·사이드바 메뉴·워크스페이스 열기,
  엑셀 요청 가로채기(1,294행+합계). 신규 `test_dec329_sales_matrix.py`(13), `test_dec200` 래퍼 6건. 스위트 3001 passed.
  probe `stats.sales_matrix` 2건. hcode 감사: 신규 서비스 info 만(critical/warn 0).
- **보강(같은 날, 사용자 「엔터로 차례로 입력 — 엔터 누락 부분 존재」)** — 스톱 목록이 기간·출력조건·보기·검색만 있어
  2줄 필터(도서/거래처 범위·교차 필터·본사출고제외)와 거래처축 구분·비교 분기 칸이 빠졌다. 화면 순서 전 칸 + 마지막 검색(자동 조회),
  비교 기간의 분기/반기 칸은 년도(4자리)일 때만 스톱(비활성 칸 포커스 멈춤 방지). 월 입력 폭 w-40(달력 아이콘 밖으로 삐져나옴),
  값 열 최소폭 92(월 머리 「2026.0」 잘림). Chrome 실키 Enter 검증: 월별 9스톱 → 검색 자동 조회, 년/월(거래처) 11스톱(빈 기간3 분기 건너뜀),
  거래처 코드 입력 후 Enter = 확정 + 다음 칸.
- **보강 2(같은 날, 사용자 「화면 오류」 캡처)** — 워크스페이스(사이드바 포함) 폭에서 년/월 비교 1줄이 `xl:flex-nowrap` + 오른쪽 정렬로
  왼쪽 밖으로 넘쳐 「기간1」 알약이 잘렸다. 공용 `LedgerSearchLine` 에 `wrap` 선택 prop(기본 false — 기존 화면 불변)을 두고
  매트릭스 화면 두 줄에 적용 → 폭이 모자라면 다음 줄로. Chrome 워크스페이스에서 잘림 없음 확인. `test_dec315` 는 기본 클래스 유지 검사로 갱신.

### DEC-330 — 도서통계(목록) 컬럼 재구성·판매금액·반품율·총합·엑셀 전 필드 (2026-09-25)

- **요청(문서 캡처, 「이전 수정한 (목록) 화면처럼」 = DEC-328)** — 검색창 통일(DEC-320 기 적용, 제외) / 컬럼: 구분·도서명·입고·출고·증정·반품·
  폐기수량·출고금액·반품금액·판매금액·반품율 / (코드)(ISBN)(최종거래일) = 컬럼 선택에 / 엑셀 = 해당 컬럼 모두 / 하단 총합.
- **반품율 정본** — 레거시 `Subu69_1.pas`(메뉴 「도서별판매율」) L517~520: −반품수량 ÷ 출고수량 × 100(출고 0 → 0), 합계 = Σ(−반품) ÷ Σ출고.
  `reports_service.get_book_sales` 행에 `sale_amt`(출고액+반품액)·`return_rate`(정렬 화이트리스트 추가), totals 에도 두 값.
  `BookSalesRow/BookSalesTotals` 모델에 필드 추가(response_model 절단 방지 — DEC-198 유형), 엑셀 허용 키에 추가.
- **화면** — 구분(도서분류 sname)·도서명·수량 5종·금액 3종·반품율(%). 코드·ISBN·최종거래일은 `defaultHidden`(컬럼 설정에서 켬),
  prefs 키 `stats.book.v2`(예전 저장 설정이 기본 숨김을 덮지 않게), 정가 제외. 기본 정렬 최종거래일→도서코드. 하단 「총합」(서버 totals).
  엑셀 = 전 필드(숨긴 선택 컬럼 포함, 화면 순서) — `test_dec198` 목록에서 빼고 전용 가드(DEC-323 년말집계와 같은 예외).
- **수량 의미** — 레거시 Subu61 분기 그대로: 반품수량엔 Scode X 폐기 포함, 폐기수량 = Scode Z 폐기, 출고금액엔 증정 금액 포함.
- **검증** — Chrome(교문사 2026.06.25~09.25, 972종): 총합 입고 30,533·출고 38,015·증정 2,272·반품 −942·판매 924,898,090·반품율 2.5.
  엑셀 요청 가로채기 200 + 14컬럼(도서코드·ISBN·최종거래일 포함). 도서별판매(/reports/book-sales)는 합계 키 명시 매핑이라 불변.
  신규 `test_dec330_stats_book_list.py`, `test_dec197`/`test_sales_team_a_batch` 갱신. 스위트 3007 passed.

### DEC-331 — 「년/월(통계)」 허브 메뉴 감춤 (2026-09-25)

- **요청** — 「년/월 통계메뉴 제거」. 하위 세분화 판매 6종(DEC-329)이 통계관리 메뉴에 직접 올라가 허브 카드 페이지는 중복.
- **변경** — `FormMeta.sidebarHidden`(선택) 신설 + 사이드바 필터. `MenuYearMonthStats` 에만 적용. 통계관리는 전용 사이드바 레이아웃이 없는
  그룹이라 DEC-309(레이아웃에서 항목 제거) 대신 플래그로 같은 효과 — **레지스트리·라우트(/year-month-stats)·ACC-MENU-NAV-07·등가 매트릭스는 보존**
  (직접 링크·열려 있던 워크스페이스 탭·즐겨찾기는 그대로 열림, 커버리지 지표 불변). `getFormsByMenu` 소비처는 사이드바 1곳.
- **검증** — Chrome 워크스페이스: 통계관리 = … 월별통계·거래처통계(목록)·도서통계(목록)·세분화 6종·출판사통계, 「년/월(통계)」 없음.
  신규 `test_dec331_year_month_hub_menu_hidden.py`. 스위트 3010 passed.

### DEC-332 — 출판사통계 개선: 목록 화면 공통 컬럼·총합·반품율 + 계정 수에 맞춘 차트 (2026-09-25)

- **요청** — 「출판사 통계화면 개선해줘」(캡처: 출판사 1곳뿐인 「상위 10 출판사」 막대, 출고수 막대 안 보임, 「잔량」 컬럼).
- **문제** — ① 「잔량」 = 실제로 증정(GJQUT) 오라벨(도서통계 A3 정정과 같은 유형). ② 판매금액·반품금액·반품율·총합 없음.
  ③ 출판사 계정(교문사)은 항상 1행이라 비교 차트가 막대 1개, 금액·수량 한 축이라 수량 막대가 바닥.
- **변경** — `get_publisher_stats` 행에 gift_qut·disposal_qut·return_sum·sale_qut·sale_amt·return_rate(레거시 판매율 식, 출고 0 → 0),
  totals 에 합계·합계 반품율(Σ−반품÷Σ출고). `stock_qut` 는 호환 유지. 화면 컬럼 = 거래처·도서통계(목록)와 같은 체계
  (출판사·코드·도서수·입고·출고·증정·반품·폐기수량·출고금액·반품금액·판매금액·반품율), 최종거래일 선택 컬럼(기본 숨김, prefs `stats.publisher.v2`)
  + 엑셀 끝에 포함(DEC-328 규칙), 기본 정렬 판매금액 내림차순, 「총합」 행, 요약 카드 5개(출판사·도서·판매수량·판매금액·반품율).
  차트: 출판사 여럿(관리자·유통) = 표 순서 상위 10/20/전체 판매금액(천원)·판매수량(오른쪽 축), 1곳 = 「수량 구성」(입고·출고·증정·반품·폐기·판매).
- **검증** — Chrome(교문사 08.25~09.25): 출고 21,589·증정 772·반품 −182·판매 532,715,355(=537,162,010−4,446,655)·반품율 0.8, 수량 구성 차트.
  엑셀 요청 가로채기 200 + 13컬럼(최종거래일 끝). 신규 `test_dec332_publisher_stats.py`. 스위트 3014 passed.

### DEC-333 — 출판사통계 메뉴 = 물류사(총판) 계정 전용 + 메뉴 감춤 플래그 통합 (2026-09-25)

- **요청** — 「출판사 통계화면 메뉴는 물류사(총판) 계정으로 로그인하였을 때만 보여주면 된다」.
- **변경** — `Sobo43_stats_route` 에 기존 `distributorOnly: true`(2026-07-24 총판 전용 게이팅, `isDistributorViewer`)만 추가 — 라우트·권한 불변.
  출판사 계정은 항상 자기 1곳뿐이라 비교 화면 의미가 없음(DEC-332 의 «수량 구성» 차트는 총판이 한 출판사만 조회할 때도 쓰임).
- **정리** — DEC-331 에서 새로 만든 `sidebarHidden` 은 기존 `hiddenFromMenu`(+`hiddenReason`, 2026-09-04 규약)와 중복이라 제거하고
  「년/월(통계)」 허브를 `hiddenFromMenu` 로 옮겼다(사이드바 필터 원복). 동작 동일.
- **검증** — Chrome(교문사 = 출판사 계정): 통계관리 메뉴에 출판사통계·년/월(통계) 없음, 세분화 6종 유지. 가드 `test_dec331_*` 갱신. 스위트 3015 passed.

### DEC-334 — 컬럼 설정 창 위/아래 자동 배치 + 「전체 선택」 (2026-09-25)

- **요청(캡처)** — 「표시할 컬럼 창이 화면에 전체 나타나지 않는다 — 화면 위치에 따라 위·아래로 보이게, 필드 선택 창 상단에 일괄선택」.
- **원인** — `GridColumnSettings` 패널이 항상 트리거 아래(top = rect.bottom+4)·고정 높이(h-80)라 표가 화면 아래쪽이면 잘렸다.
- **변경(공용 1곳, 호출부 74곳 prop 불변)** — 아래 남은 공간 ≥ 320 이거나 아래가 위보다 넓으면 아래, 아니면 트리거 **위로**(bottom 기준) 펼치고
  `maxHeight` = 실제 남은 공간(최소 160, 목록은 안에서 스크롤). 목록 위 「전체 선택」 체크: 전부 표시 ↔ 첫 컬럼만 남기고 해제(최소 1개 규칙),
  일부 선택 = indeterminate. 기존 `toggleHidden`(함수형 갱신)을 반복 호출해 저장 경로 불변.
- **검증** — Chrome: 출판사통계(표가 아래) → 위로 펼침(323~643, 뷰포트 893), 거래처통계(목록)(표가 위) → 아래로. 전체 선택 → 13컬럼, 다시 → 출판사만,
  「전체 초기화」로 원복. 신규 `test_dec334_column_settings_placement.py`. 스위트 3017 passed.

### DEC-335 — 기초관리: 신규 거래처·입고처 이름 맨 앞 / 입고처관리 검색 줄 통일·지역 제거 / 저자 소속대학·학과 = 직장명·직책 (2026-09-25)

- **요청(문서 캡처 1~3)** — ① 신규거래처 화면 거래처명 맨 앞. ② 입고처관리: 입고처지역 검색칸 삭제, 검색창 디자인을 거래처관리와 통일,
  신규 입고처 입고처명 맨 앞. ③ 저자관리: 「소속대학·학과」와 기존 「직장명·직책」이 같은 내용 → 직장명·직책 명칭을 소속대학·학과로.
- **①② 폼** — 거래처·입고처 상세 폼(신규·수정 공용) 1행 = 이름/구분/지역/코드. legacyId(Edit105 등)·바인딩 불변.
- **② 목록 검색 줄** — 거래처현황의 로컬 `AttachedFilterField` 를 `components/shared/attached-filter-field.tsx` 로 옮겨(마크업 불변) 입고처관리도 사용:
  「검색 | 코드, 입고처명, 대표자」 + 「입고처구분」 알약. 지역 입력칸 제거, 예전 세션에 저장된 지역값은 복원하지 않음(보이지 않는 필터 방지).
- **③ 저자** — 실데이터(교문사 1,236명): Gname(직장명) 1,029건·Gjice(직책) 476건 vs Gscho(웹 「소속대학」, 레거시 「출신학교」) 4건(전화번호 등 오입력)·
  웹 확장 학과(dept) 0건. → 소속대학 = Gname, 학과 = Gjice(목록·상세·검색 필터 라벨·엑셀 헤더). Gscho 는 레거시 원래 이름 「출신학교」 선택 컬럼,
  dept 는 목록·폼·엑셀에서 제외(확장 테이블 컬럼·서비스는 보존). 엑셀 가져오기는 종전 헤더(직장명·직책·직책명)도 별칭으로 인식. 엑셀 29 → 28컬럼.
- **검증** — Chrome: 신규 거래처·신규 입고처 1행 이름 첫 칸, 입고처관리 검색 줄 = 거래처현황 알약(지역 없음), 저자 목록 소속대학 「전 서울대학교」·학과 「가정대학 의류학과」.
  가드 갱신(test_dec230/294/154/295/masters_excel_export). 스위트 3017 passed.

### DEC-336 — 도서관리: 검색 줄 통일 + 컬럼 재구성 + 확장 필드(상태·자료제공·기타·인지유무) + 엑셀 전 필드 (2026-09-25)

- **요청(문서 캡처 4)** — ① 검색창 디자인 통일. ② 기본 컬럼 = 구분·코드·전자책(유무)·도서명·저자명·ISBN·정가·재고·서가위치·위탁·발행일·
  상태(정상·일시품절·품절·절판)·정지사유·자료제공(수기입력 — 상세에 없음, 추가)·기타·비고, 하늘색(선택 컬럼) = 도서종류·인지유무·쪽수·판수·
  현매·매절·납품·특별·한도·기타·신간수량·원가·매입가·덩이·그램·재고금액·도서구분·본사재고·본사비품·창고정품·창고비품·등록일. 엑셀 = 기입 정보 전부.
- **사용자 결정(질문 응답)** — 상태 = 새 칸 + 기존 기록으로 초기값 / 자료제공 = 자유 입력 / 기타 = 새 메모칸 / 도서종류 = 기존 「묶음」(Gbjil).
- **저장소** — G4_Book 에 빈 컬럼이 없어(DEC-068) 사이드 테이블 `G4_Book_Ext`(Hcode, Gcode PK; Status·Supply·EtcMemo·Stamp) 신설
  (`book_ext_service` — CREATE IF NOT EXISTS·REPLACE INTO, 부분 갱신 병합, 전부 비면 행 삭제, 상태 4종 외 422). 델파이 무관.
- **상태 초기값** — 확장 행에 상태가 없으면 **읽을 때만** 추정(DB 일괄 기록 없음): 판형·도서처리 '절판'→절판, '품절'→품절, '재고없음'→일시품절,
  재고절판 플래그→절판, 그 외 정상. 교문사 실데이터: 판형 칸 절판 700·재고없음 93, 도서처리 품절 2.
- **화면** — 목록 기본 16컬럼(사용자 순서), 선택 컬럼(prefs `master.book.v3`, 기본 숨김) — 사용자 표기(본사재고·본사비품·창고정품·창고비품),
  판형·도서코드2·비율구분·등록번호·단위·세액유무·재고절판·출고정지도 선택 컬럼으로 보존. 확장 컬럼은 페이지 행 병합이라 정렬 없음.
  전자책 = 상세 「전자책 여부」(Bigo3) — 목록 포함. 검색 줄 = `AttachedFilterField`(검색 | 코드, 도서명, ISBN · 도서구분 · 발행일 기간).
  상세 폼: 도서종류(묶음 칸), 상태 선택, 자료제공·기타(메모), 인지유무 체크, 비율 「기타」 → 「기타(비율)」(메모와 구분).
- **엑셀** — BOOK_COLUMNS 에 전자책·상태·자료제공·기타메모·인지유무 추가, 묶음 → 도서종류, 비율 기타 → 기타(비율). 가져오기는 새 헤더와 종전 헤더
  (묶음·기타) 모두 인식, 확장 필드(기타메모 등)는 가져오기 대상 아님(메모 글자가 비율 칸에 들어가는 사고 방지).
- **미반영** — 선택 컬럼 「신간수량」은 도서 마스터에 해당 필드가 없어(거래처 신간수량 Gqut1 만 존재) 제외 — 필요 시 확장 필드로 추가.
- **검증** — Chrome: 목록 16컬럼·상태 추정(00001 절판, 00015 일시품절 ← 판형 「재고 없음」), 상세 폼 새 필드, 엑셀 요청 가로채기 200.
  운영 DB 에는 조회 시 빈 `G4_Book_Ext` 테이블 생성만(행 기록 없음). 신규 `test_dec336_book_list_ext_fields.py`, dec148/151/296 가드 갱신. 스위트 3024 passed.

### DEC-337 — 「택배관리」 대메뉴 제거 (2026-09-25)

- **요청** — 「택배관리 메뉴는 제거해줘」.
- **변경** — 택배관리 그룹(`delivery`)의 유일 화면 `Sobo28_delivery`(/shipping/courier)에 `hiddenFromMenu`(+사유) — 사이드바는 빈 그룹을
  그리지 않으므로 대메뉴째 사라진다. 라우트·레지스트리·권한·등가 매트릭스 보존(직접 링크는 열림, DEC-309/331 과 같은 방식).
- **검증** — Chrome 사이드바: 기초·입고·출고·반품/폐기·원장·정산·통계·거래관리·발송비/입금(택배관리 없음). 신규 `test_dec337_delivery_menu_hidden.py`. 스위트 3026 passed.
- **보강(같은 날, 사용자 캡처 「이 부분 디자인을 다른 화면과 유사하게」)** — 도서관리 「발행일」 알약 안 날짜칸이 테두리 있는 입력+달력 버튼 상자로
  겹쳐 보였다 → `DateFieldYMD embedded`(거래처거래원장 LedgerDatePill 과 같은 테두리 없는 모양). Chrome 확인.
