...
- **보강(2026-07-20 사용자 리포트)**: 무지사 거래처 선택 시 빈 지사(직접 입력) 폴백
  칸에 포커스가 멈춤 → `showGjisa=false`(지사 목록 없음)면 지사를 건너뛰고 첫 라인
  구분으로 바로 포커스(거래명세서 신규 DEC-097 동작과 통일). 지사 직접 입력은
  마우스/Tab 접근. 실검증: 월드북센타(무지사)→구분 직행 ✅, 교보문고(지사有)→지사
  select 유지 ✅.
- **결정자**: 사용자 (2026-07-20)
