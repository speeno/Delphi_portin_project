...
- [ ] MariaDB **실접속 검증** — 연결 정보는 확보했으나 현재 접속 불가. 방화벽·VPN·허용 IP·3306·DB 인스턴스·계정 권한 점검 후 `schema_extractor` 재시도.
...
- DB: MariaDB — 호스트·자격 증명 확보(`.env`). **접속 검증 진행 중**(네트워크/권한). 통과 후 스키마 추출·#4.
- 빌드/구동: 환경별 확인 필요
