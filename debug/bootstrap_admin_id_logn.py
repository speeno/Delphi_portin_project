...
------
  ...
  python debug/bootstrap_admin_id_logn.py --server-id remote_138 --apply \
      --gcode admin --gpass admin123 --hcode 0000 --gname "관리자"

환경 변수 (--옵션 미지정 시 폴백)
---------------------------------
  BLS_ADMIN_GCODE   관리자 로그인 ID    (기본 admin)
  BLS_ADMIN_GPASS   관리자 비밀번호     (기본 admin123)
  BLS_ADMIN_HCODE   소속 코드           (기본 0000 — 4자리, 레거시 슈퍼유저 동등 DEC-056)
  BLS_ADMIN_GNAME   시스템 별칭         (기본 관리자)
  BLS_ADMIN_HNAME   표시 이름           (기본 시스템관리자)

비고
----
- ``hcode`` 는 ``app/services/auth_service.py::_resolve_role_and_permissions`` 의 슈퍼유저
  분기(``hcode_norm == "0000"``) 와 정확히 동일한 4자리 영을 기본값으로 사용한다.
- 과거 5자리(``00000``)로 부트스트랩된 환경은 다음 SQL 1건을 수동 실행:
    UPDATE Id_Logn SET hcode='0000' WHERE gcode='admin' AND hcode='00000';
  또는 ``BLS_ADMIN_USER_IDS=admin`` 환경변수로 즉시 우회 가능.
