...
class MasterCrudStaticContract(TestCase):
    def test_router_exposes_customer_and_book_post_delete(self):
        ...
        for token in (
            ...
            '@router.get("/inbound-vendors/{gcode}"',
            '"/inbound-vendors",',
            '"/inbound-vendors/{gcode}",',
            '@router.get("/inbound-vendor-categories"',
            '"/inbound-vendor-categories",',
            '"/inbound-vendor-categories/{gcode}",',
            '@router.get("/authors/{gcode}"',
            '"/authors",',
            '"/authors/{gcode}",',
            '@router.get("/author-categories"',
            '"/author-categories",',
            '"/author-categories/{gcode}",',
            '@router.get("/etc-customers/{gcode}"',
            '"/etc-customers",',
            '"/etc-customers/{gcode}",',
            '@router.get("/etc-customer-categories"',
            '"/etc-customer-categories",',
            '"/etc-customer-categories/{gcode}",',
            '@router.get("/customer-categories"',
            '@router.post("/customer-categories"',
            '@router.patch("/customer-categories/{gcode}"',
            '@router.get("/customer/{gcode}/branches"',
            '"/customer/{gcode}/branches"',
            "create_customer_branch",
            "update_customer_branch",
            "delete_customer_branch",
            '@router.delete("/customer-categories/{gcode}"',
            '@router.post("/book"',
            '@router.delete("/book/{gcode}"',
            '@router.get("/book-categories"',
            '@router.post("/special"',
            '@router.delete("/special/{row_id}"',
            '"/book-categories",',
            '"/book-categories/{gcode}",',
    ...
    def test_frontend_client_exports_create_delete_methods(self):
        ...
        for token in (
            ...
            "categoryList",
            "InboundVendorDetail",
            "InboundVendorCreatePayload",
            "InboundVendorUpdatePayload",
            "AuthorDetail",
            "AuthorCreatePayload",
            "AuthorUpdatePayload",
            "EtcCustomerDetail",
            "EtcCustomerCreatePayload",
            "EtcCustomerUpdatePayload",
            "bookCreate",
            "bookDelete",
            "bookCategoryList",
            "bookCategoryCreate",
